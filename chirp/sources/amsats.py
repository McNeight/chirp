...
DATA_URL = (
    "https://raw.githubusercontent.com/palewire/"
...
SATNOGS_TRANS_URL = (
    "https://db.satnogs.org/api/transmitters/" "?alive=True&status=active"
)
...
class RadioAmateurSatellites(base.NetworkResultRadio):
    ...
    def item_to_memory(self, item):
        ...
        try:
            ...
            first_freq = "".join(c for c in first_freq if c.isdigit() or c == ".")
            mem.freq = chirp_common.parse_freq(first_freq)
        except (ValueError, IndexError, errors.InvalidDataError) as e:
            LOG.warning(
                "Unable to parse downlink freq %r for %s: %s", downlink_str, name, e
            )
            return None
        ...
        if uplink_str:
            try:
                ...
                first_uplink = "".join(
                    c for c in first_uplink if c.isdigit() or c == "."
                )
                ...
                LOG.debug(
                    "Unable to parse uplink freq %r for %s: %s", uplink_str, name, e
                )
    ...
    def do_fetch(self, status, params):
        ...
        for item in data:
            try:
                ...
                LOG.warning("Failed to convert satellite %s: %s", item.get("name"), e)
                continue
...
class SatNOGS(base.NetworkResultRadio):
    ...
    def _fetch_all(self, status, url, label):
        ...
        while next_url:
            status.send_status(
                "Fetching %s (page %i)..." % (label, page), len(results) % 100
            )
            try:
    ...
    def do_fetch(self, status, params):
        ...
        transmitters = self._fetch_all(status, SATNOGS_TRANS_URL, "transmitters")
