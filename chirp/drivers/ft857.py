...
class FT857Radio(ft817.FT817Radio):
    ...
    CROSS_MODES_REV = dict(
        list(zip(list(CROSS_MODES.values()), list(CROSS_MODES.keys())))
    )
    ...
    _CALLSIGN_CHARSET = [
        chr(x)
        for x in list(range(ord("0"), ord("9") + 1))
        + list(range(ord("A"), ord("Z") + 1))
    ] + [" ", "/"]
    _CALLSIGN_CHARSET_REV = dict(
        list(zip(_CALLSIGN_CHARSET, list(range(0, len(_CALLSIGN_CHARSET)))))
    )
    _BEACON_CHARSET = _CALLSIGN_CHARSET + ["+", "."]
    _BEACON_CHARSET_REV = dict(
        list(zip(_BEACON_CHARSET, list(range(0, len(_BEACON_CHARSET)))))
    )
    ...
    SPECIAL_MEMORIES_REV = dict(
        list(zip(list(SPECIAL_MEMORIES.values()), list(SPECIAL_MEMORIES.keys())))
    )
    ...
    def get_settings(self):
        ...
        extended.append(rs)
        rs = RadioSetting(
            "ars_144", "144 MHz ARS", RadioSettingValueBoolean(_settings.ars_144)
        )
        basic.append(rs)
        rs = RadioSetting(
            "ars_430", "430 MHz ARS", RadioSettingValueBoolean(_settings.ars_430)
        )
        basic.append(rs)
        ...
        rs = RadioSetting(
            "disable_amfm_dial",
            "AM&FM Dial",
            RadioSettingValueList(options, current_index=_settings.disable_amfm_dial),
        )
        panel.append(rs)
        rs = RadioSetting(
            "am_mic", "AM mic gain", RadioSettingValueInteger(0, 100, _settings.am_mic)
        )
        basic.append(rs)
        ...
        rs = RadioSetting(
            "apo_time",
            "APO time",
            RadioSettingValueList(options, current_index=_settings.apo_time),
        )
        basic.append(rs)
        ...
        rs = RadioSetting(
            "arts_beep",
            "ARTS beep",
            RadioSettingValueList(options, current_index=_settings.arts_beep),
        )
        basic.append(rs)
        rs = RadioSetting(
            "arts_id", "ARTS ID", RadioSettingValueBoolean(_settings.arts_id)
        )
        extended.append(rs)
        st = RadioSettingValueString(
            0, 10, safe_charset_string(self._memobj.arts_idw, self._CALLSIGN_CHARSET)
        )
        st.set_charset(self._CALLSIGN_CHARSET)
        rs = RadioSetting("arts_idw", "ARTS IDW", st)
        extended.append(rs)
        st = RadioSettingValueString(
            0, 40, safe_charset_string(self._memobj.beacon_text1, self._BEACON_CHARSET)
        )
        st.set_charset(self._BEACON_CHARSET)
        rs = RadioSetting("beacon_text1", "Beacon text1", st)
        extended.append(rs)
        st = RadioSettingValueString(
            0, 40, safe_charset_string(self._memobj.beacon_text2, self._BEACON_CHARSET)
        )
        st.set_charset(self._BEACON_CHARSET)
        rs = RadioSetting("beacon_text2", "Beacon text2", st)
        extended.append(rs)
        st = RadioSettingValueString(
            0, 40, safe_charset_string(self._memobj.beacon_text3, self._BEACON_CHARSET)
        )
        st.set_charset(self._BEACON_CHARSET)
        ...
        rs = RadioSetting(
            "beacon_time",
            "Beacon time",
            RadioSettingValueList(options, current_index=_settings.beacon_time),
        )
        extended.append(rs)
        ...
        rs = RadioSetting(
            "beep_tone",
            "Beep tone",
            RadioSettingValueList(options, current_index=_settings.beep_tone),
        )
        panel.append(rs)
        rs = RadioSetting(
            "beep_vol",
        ...
        rs = RadioSetting(
            "cat_rate",
            "CAT rate",
            RadioSettingValueList(options, current_index=_settings.cat_rate),
        )
        basic.append(rs)
        options = ["CAT", "Linear", "Tuner"]
        rs = RadioSetting(
            "cat_lin_tun",
            "CAT/LIN/TUN selection",
            RadioSettingValueList(options, current_index=_settings.cat_lin_tun),
        )
        extended.append(rs)
        ...
        rs = RadioSetting(
            "clar_dial_sel",
            "Clarifier dial selection",
            RadioSettingValueList(options, current_index=_settings.clar_dial_sel),
        )
        panel.append(rs)
        rs = RadioSetting(
            "cw_auto_mode",
        ...
        rs = RadioSetting(
            "cw_bfo",
            "CW BFO",
            RadioSettingValueList(options, current_index=_settings.cw_bfo),
        )
        cw.append(rs)
        options = ["FULL"] + ["%i ms" % (i * 10) for i in range(3, 301)]
        val = (_settings.cw_delay + _settings.cw_delay_hi * 256) - 2
        rs = RadioSetting(
            "cw_delay", "CW delay", RadioSettingValueList(options, current_index=val)
        )
        cw.append(rs)
        ...
        rs = RadioSetting(
            "cw_key_rev",
            "CW key reverse",
            RadioSettingValueList(options, current_index=_settings.cw_key_rev),
        )
        cw.append(rs)
        rs = RadioSetting(
            "cw_paddle", "CW paddle", RadioSettingValueBoolean(_settings.cw_paddle)
        )
        cw.append(rs)
        ...
        rs = RadioSetting(
            "cw_pitch",
            "CW pitch",
            RadioSettingValueList(options, current_index=_settings.cw_pitch),
        )
        cw.append(rs)
        ...
        rs = RadioSetting(
            "cw_qsk",
            "CW QSK",
            RadioSettingValueList(options, current_index=_settings.cw_qsk),
        )
        cw.append(rs)
        rs = RadioSetting(
            "cw_sidetone",
        ...
        rs = RadioSetting(
            "cw_speed",
            "CW speed",
            RadioSettingValueList(options, current_index=_settings.cw_speed),
        )
        cw.append(rs)
        ...
        rs = RadioSetting(
            "cw_training",
            "CW training",
            RadioSettingValueList(options, current_index=_settings.cw_training),
        )
        cw.append(rs)
        ...
        rs = RadioSetting(
            "cw_weight",
            "CW weight",
            RadioSettingValueList(options, current_index=_settings.cw_weight),
        )
        cw.append(rs)
        ...
        rs = RadioSetting(
            "dcs_inv",
            "DCS inv",
            RadioSettingValueList(options, current_index=_settings.dcs_inv),
        )
        extended.append(rs)
        options = ["Fine", "Coarse"]
        rs = RadioSetting(
            "dial_step",
            "Dial step",
            RadioSettingValueList(options, current_index=_settings.dial_step),
        )
        panel.append(rs)
        rs = RadioSetting(
            "dig_disp",
        ...
        packet.append(rs)
        rs = RadioSetting(
            "dig_mic", "Dig gain", RadioSettingValueInteger(0, 100, _settings.dig_mic)
        )
        packet.append(rs)
        ...
        rs = RadioSetting(
            "dig_mode",
            "Dig mode",
            RadioSettingValueList(options, current_index=_settings.dig_mode),
        )
        packet.append(rs)
        rs = RadioSetting(
            "dig_shift",
        ...
        packet.append(rs)
        rs = RadioSetting(
            "dig_vox", "Dig vox", RadioSettingValueInteger(0, 100, _settings.dig_vox)
        )
        packet.append(rs)
        ...
        rs = RadioSetting(
            "disp_color",
            "Display color mode",
            RadioSettingValueList(options, current_index=_settings.disp_color),
        )
        panel.append(rs)
        rs = RadioSetting(
            "disp_color_arts",
            "Display color ARTS set",
            RadioSettingValueInteger(0, 1, _settings.disp_color_arts),
        )
        panel.append(rs)
        rs = RadioSetting(
            "disp_color_band",
            "Display color band set",
        ...
        rs = RadioSetting(
            ...
            RadioSettingValueList(options, current_index=_settings.disp_mode),
        )
        panel.append(rs)
        ...
        rs = RadioSetting(
            ...
            RadioSettingValueList(options, current_index=_settings.dsp_bpf),
        )
        cw.append(rs)
        options = [
            "100 Hz",
        ...
        rs = RadioSetting(
            "dsp_mic_eq",
            "Dsp mic equalization",
            RadioSettingValueList(options, current_index=_settings.dsp_mic_eq),
        )
        basic.append(rs)
        rs = RadioSetting(
            "dsp_nr",
        ...
        rs = RadioSetting(
            "fm_mic", "FM mic gain", RadioSettingValueInteger(0, 100, _settings.fm_mic)
        )
        basic.append(rs)
        rs = RadioSetting(
            "home_vfo",
            "Enable HOME to VFO moving",
        ...
        rs = RadioSetting(
            "lock_mode",
            "Lock mode",
            RadioSettingValueList(options, current_index=_settings.lock_mode),
        )
        panel.append(rs)
        rs = RadioSetting(
            "mem_group", "Mem group", RadioSettingValueBoolean(_settings.mem_group)
        )
        basic.append(rs)
        options = [
            "CW SIDETONE",
        ...
        rs = RadioSetting(
            "mem_vfo_dial_mode",
            "Mem/VFO dial mode",
            RadioSettingValueList(options, current_index=_settings.mem_vfo_dial_mode),
        )
        panel.append(rs)
        rs = RadioSetting(
            "mic_scan", "Mic scan", RadioSettingValueBoolean(_settings.mic_scan)
        )
        basic.append(rs)
        options = ["NOR", "RMT", "CAT"]
        rs = RadioSetting(
            "mic_sel",
            "Mic selection",
            RadioSettingValueList(options, current_index=_settings.mic_sel),
        )
        extended.append(rs)
        options = ["SIG", "CTR", "VLT", "N/A", "FS", "OFF"]
        rs = RadioSetting(
            "mtr_arx_sel",
            "Meter receive selection",
            RadioSettingValueList(options, current_index=_settings.mtr_arx_sel),
        )
        extended.append(rs)
        options = ["PWR", "ALC", "MOD", "SWR", "VLT", "N/A", "OFF"]
        rs = RadioSetting(
            "mtr_atx_sel",
            "Meter transmit selection",
            RadioSettingValueList(options, current_index=_settings.mtr_atx_sel),
        )
        extended.append(rs)
        rs = RadioSetting(
            "mtr_peak_hold",
        ...
        st = RadioSettingValueString(
            0,
            4,
            safe_charset_string(self._memobj.op_filter1_name, self._CALLSIGN_CHARSET),
        )
        st.set_charset(self._CALLSIGN_CHARSET)
        ...
        st = RadioSettingValueString(
            0,
            4,
            safe_charset_string(self._memobj.op_filter2_name, self._CALLSIGN_CHARSET),
        )
        st.set_charset(self._CALLSIGN_CHARSET)
        ...
        rs = RadioSetting(
            "pg_a",
            "Programmable key MFq:A",
            RadioSettingValueList(
                self.PROGRAMMABLEOPTIONS, current_index=_settings.pg_a
            ),
        )
        extended.append(rs)
        rs = RadioSetting(
            "pg_b",
            "Programmable key MFq:B",
            RadioSettingValueList(
                self.PROGRAMMABLEOPTIONS, current_index=_settings.pg_b
            ),
        )
        extended.append(rs)
        rs = RadioSetting(
            "pg_c",
            "Programmable key MFq:C",
            RadioSettingValueList(
                self.PROGRAMMABLEOPTIONS, current_index=_settings.pg_c
            ),
        )
        extended.append(rs)
        rs = RadioSetting(
            "pg_acc",
            "Programmable mic key ACC",
            RadioSettingValueList(
                self.PROGRAMMABLEOPTIONS, current_index=_settings.pg_acc
            ),
        )
        extended.append(rs)
        rs = RadioSetting(
            "pg_p1",
            "Programmable mic key P1",
            RadioSettingValueList(
                self.PROGRAMMABLEOPTIONS, current_index=_settings.pg_p1
            ),
        )
        extended.append(rs)
        rs = RadioSetting(
            "pg_p2",
            "Programmable mic key P2",
            RadioSettingValueList(
                self.PROGRAMMABLEOPTIONS, current_index=_settings.pg_p2
            ),
        )
        extended.append(rs)
        rs = RadioSetting(
            "pkt1200",
            "Packet 1200 gain level",
            RadioSettingValueInteger(0, 100, _settings.pkt1200),
        )
        packet.append(rs)
        rs = RadioSetting(
            "pkt9600",
            "Packet 9600 gain level",
        ...
        rs = RadioSetting(
            "pkt_rate",
            "Packet rate",
            RadioSettingValueList(options, current_index=_settings.pkt_rate),
        )
        packet.append(rs)
        rs = RadioSetting(
            "proc_level",
        ...
        rs = RadioSetting(
            "scan_mode",
            "Scan mode",
            RadioSettingValueList(options, current_index=_settings.scan_mode),
        )
        basic.append(rs)
        rs = RadioSetting(
            "scan_resume",
        ...
        rs = RadioSetting(
            "sql_rf_gain",
            "Squelch/RF-Gain",
            RadioSettingValueList(options, current_index=_settings.sql_rf_gain),
        )
        panel.append(rs)
        rs = RadioSetting(
            "ssb_mic",
        ...
        rs = RadioSetting(
            "tot_time",
            "Time-out timer",
            RadioSettingValueList(options, current_index=_settings.tot_time),
        )
        basic.append(rs)
        ...
        rs = RadioSetting(
            "tuner_atas",
            "Tuner/ATAS device",
            RadioSettingValueList(options, current_index=_settings.tuner_atas),
        )
        extended.append(rs)
        rs = RadioSetting(
            "tx_if_filter",
            "Transmit IF filter",
            RadioSettingValueList(self.FILTERS, current_index=_settings.tx_if_filter),
        )
        basic.append(rs)
        rs = RadioSetting(
            "vox_delay",
        ...
        basic.append(rs)
        rs = RadioSetting(
            "vox_gain", "VOX Gain", RadioSettingValueInteger(1, 100, _settings.vox_gain)
        )
        basic.append(rs)
        rs = RadioSetting(
            "xvtr_a",
            ...
            RadioSettingValueInteger(
                -4294967295,
                4294967295,
                self._memobj.xvtr_a_offset * (-1 if _settings.xvtr_a_negative else 1),
            ),
        )
        ...
        rs = RadioSetting(
            ...
            RadioSettingValueInteger(
                -4294967295,
                4294967295,
                self._memobj.xvtr_b_offset * (-1 if _settings.xvtr_b_negative else 1),
            ),
        )
        extended.append(rs)
        ...
        rs = RadioSetting(
            "xvtr_sel",
            "Transverter function selection",
            RadioSettingValueList(options, current_index=_settings.xvtr_sel),
        )
        extended.append(rs)

        rs = RadioSetting(
            "disp", "Display large", RadioSettingValueBoolean(_settings.disp)
        )
        panel.append(rs)
        rs = RadioSetting("nb", "Noise blanker", RadioSettingValueBoolean(_settings.nb))
        panelcontr.append(rs)
        options = ["Auto", "Fast", "Slow", "Off"]
        rs = RadioSetting(
            "agc", "AGC", RadioSettingValueList(options, current_index=_settings.agc)
        )
        panelcontr.append(rs)
        ...
        rs = RadioSetting(
            "pwr_meter_mode",
            "Power meter mode",
            RadioSettingValueList(options, current_index=_settings.pwr_meter_mode),
        )
        panelcontr.append(rs)
        rs = RadioSetting("vox", "Vox", RadioSettingValueBoolean(_settings.vox))
        panelcontr.append(rs)
        rs = RadioSetting("bk", "Semi break-in", RadioSettingValueBoolean(_settings.bk))
        cw.append(rs)
        rs = RadioSetting("kyr", "Keyer", RadioSettingValueBoolean(_settings.kyr))
        cw.append(rs)
        options = ["enabled", "disabled"]
        rs = RadioSetting(
            "fst", "Fast", RadioSettingValueList(options, current_index=_settings.fst)
        )
        panelcontr.append(rs)
        options = ["enabled", "disabled"]
        rs = RadioSetting(
            "lock", "Lock", RadioSettingValueList(options, current_index=_settings.lock)
        )
        panelcontr.append(rs)
        rs = RadioSetting(
            "scope_peakhold",
        ...
        rs = RadioSetting(
            "scope_width",
            "Scope width (channels)",
            RadioSettingValueList(options, current_index=_settings.scope_width),
        )
        panelcontr.append(rs)
        rs = RadioSetting(
            "proc", "Speech processor", RadioSettingValueBoolean(_settings.proc)
        )
        panelcontr.append(rs)
        ...
        for element in settings:
            ...
            try:
                ...
                try:
                    LOG.debug(
                        "Setting %s(%s) <= %s"
                        % (setting, getattr(obj, setting), element.value)
                    )
                except AttributeError:
                    LOG.debug("Setting %s <= %s" % (setting, element.value))
                if setting == "arts_idw":
                    self._memobj.arts_idw = [
                        self._CALLSIGN_CHARSET_REV[x] for x in str(element.value)
                    ]
                elif setting in [
                    "beacon_text1",
                    "beacon_text2",
                    ...
                    setattr(
                        self._memobj,
                        setting,
                        [self._BEACON_CHARSET_REV[x] for x in str(element.value)],
                    )
...
class FT857USRadio(FT857Radio):
    ...
    def get_memory(self, number):
        if number in self.SPECIAL_60M:
            ...
            and number < 0
            and (self.SPECIAL_MEMORIES_REV[number] in list(self.SPECIAL_60M.keys()))
        ):
