...
class AR8200Radio(chirp_common.LiveRadio):
    ...
    def _encode_number(self, region, bank, chan):
        ...
        if region == "vfo":
            ...
            if bank not in self._SCAN_BANKS or chan >= self._scan_bank_size(bank):
                return None
            ...
            if bank == "V":
                ...
                offset = self._SCAN_MEMORIES + self._SEARCH_BANKS.index(bank) * 50
                size = 50
    ...
    def _command_update_cache(self, cmd, lines=10, varlist=False, retries=5):
        return self._command(
            cmd=cmd, lines=lines, cache=True, varlist=varlist, retries=retries
        )
    ...
    def _scan_bank_size(self, bank):
        ...
        if bank.upper() not in self._scan_bank_sizes:
            res = self._command(
                "MW{}".format(bank.upper()), "MW [A-J]:([1-9]0) [a-j]:([1-9]0)"
            )
    ...
    def _get_ol_list_uncached(self):
        ...
        for n in range(1, 48):
            res = self._command_cached(
                "OL{:02d}".format(n), "OL([0-4][0-9]) RF(0[0-9]{7}00)"
            )
    ...
    def _invalidate_memory(
        self, region=None, bank=None, chan=None, number=None, emptyonly=False
    ):
    ...
    def _get_memory_unlocked(self, number):
        ...
        rs = RadioSetting("selectindex", "Select Scan Index", extra_selectindex)
        mem.extra.append(rs)
        ...
        if region is None:
            raise errors.InvalidMemoryLocation("No such memory {}".format(number))
        ...
        if region == "vfo":
            ...
            if not res:
                ...
                if int(res["auto"]):
                    ...
                    radio_mode = [
                        mode[0] for mode in self._MODES if mode[1] == int(res["mode"])
                    ][0]
                ...
                if res:
                    extra_adjust.set_value(int(res[1]) / 1000 if res[2] == "+" else 0)
            ...
            res = self._command_cached(
                "QM{:1d}".format(chan), "QM([0-9]) RF([0-9]{10})"
            )
            if not res:
                mem.empty = True
            else:
            ...
            if not res:
                ...
                if not self._SKIP_SELECT:
                    ...
                    if (bank, chan) in gr_list:
                        ...
                        extra_selectindex.set_value(gr_list.index((bank, chan)))
                    else:
    ...
    def validate_memory(self, memory):
        ...
        if region in ("scan", "search"):
            ...
            if (
                memory.tuning_step == 0.0
                or memory.tuning_step >= 1000.0
                or (int(memory.tuning_step * 1000) % 50) != 0
            ):
                msg = chirp_common.ValidationWarning(
        ...
        if region == "search" and "upper" in memory.extra:
            ...
            if memory.freq > upper:
                msg = chirp_common.ValidationWarning(
                    "Upper search frequency is not greater than lower " "frequency"
                )
                msgs.append(msg)
    ...
    def _set_memory_unlocked(self, memory):
        ...
        if region == "vfo":
            ...
            if memory.mode == "Auto":
                ...
                mode = [
                    mode[1] for mode in self._MODES if mode[0] == memory.extra["am"]
                ][0]
            else:
                mode = [mode[1] for mode in self._MODES if mode[0] == memory.mode][0]

            att = int(memory.extra["att"].value) if "att" in memory.extra else 0
            duplex = memory.duplex if memory.duplex in ("+", "-") else ""
            offset = (
                ol_list.index(memory.offset)
                if duplex and memory.offset in ol_list
                else 0
            )
            adjust = (
                float(memory.extra["adjust"].value) if "adjust" in memory.extra else 0
            )
            limit = int(memory.extra["limit"].value) if "limit" in memory.extra else 0
            ...
            raise errors.InvalidDataError("Quick memory locations are read-only")
            ...
            limit = int(memory.extra["limit"].value) if "limit" in memory.extra else 0
            protect = (
                int(memory.extra["protect"].value) if "protect" in memory.extra else 0
            )
            select = (
                int(memory.extra["select"].value) if "select" in memory.extra else 0
            )
            name = memory.name
            mp = 1 if memory.skip == "S" else 0

            offsetstr = (
                "OF{:02d}{} ".format(offset, duplex) if not self._SKIP_SELECT else ""
            )
            res = self._command(
                (
                    "MX{}{:02d} RF{:010d} MP{:1d} ST{:06d} MD{:1d} AT{:1d} "
                    "{}AU{:1d} TM{:12}".format(
                        bank, chan, freq, mp, step, mode, att, offsetstr, auto, name
                    )
                ),
                "(?:$|{:c}{:c})".format(0x13, 0x11),
            )
            if not res:
                raise errors.InvalidDataError("Radio refused {}".format(memory.number))
            ...
            if not self._SKIP_SELECT:
                ...
                if not select and (bank, chan) in gr_list:
                    self._command("GD{:02d}".format(gr_list.index((bank, chan))))
            ...
            lower = memory.freq
            upper = (
                int(float(memory.extra["upper"].value) * 1000000)
                if "upper" in memory.extra
                else lower
            )
            auto = int(memory.mode == "Auto")
            step = int(memory.tuning_step * 1000)
            ...
            res = self._command(
                ...
                "OF{:02d}{:1} AU{:1d} TT{:12}".format(
                    bank, lower, upper, step, mode, att, offset, duplex, auto, name
                )
            )
            if not res:
                raise errors.InvalidDataError("Radio refused {}".format(memory.number))
    ...
    def _get_settings_unlocked(self):
        ...
        scanbankprotect = RadioSettingGroup("scanbankprotect", "Scan Bank Protect")
        top = RadioSettings(main, vfo, freq, scan, search, scanbankprotect)

        res = self._command("VR", "VR([0-9]+)")
        rs = RadioSetting(
            "VR", "Firmware Version", RadioSettingValueString(0, 4, res[1])
        )
        ...
        rs = RadioSetting(
            "AF", "Automatic Frequency Control", RadioSettingValueBoolean(int(res[1]))
        )
        rs.set_apply_callback(lambda x: self._command("AF{}".format(int(x.value))))
        main.append(rs)

        res = self._command("AP", "AP([0-9][.][0-9])")
        rs = RadioSetting(
            "AP",
            "Automatic Power Off (hours)",
            RadioSettingValueFloat(0, 9.5, float(res[1]), 0.5, 1),
        )
        rs.set_apply_callback(lambda x: self._command("AP{}".format(int(x.value))))
        main.append(rs)

        res = self._command("DT", "DT([01])")
        rs = RadioSetting(
            "DT", "Display Frequency Text", RadioSettingValueBoolean(int(res[1]))
        )
        rs.set_apply_callback(lambda x: self._command("DT{}".format(int(x.value))))
        main.append(rs)

        res = self._command("LB", "LB([0-9]{2})")
        rs = RadioSetting("LB", "LCD Contrast", RadioSettingValueInteger(0, 31, res[1]))
        rs.set_apply_callback(lambda x: self._command("LB{:02d}".format(int(x.value))))
        main.append(rs)
        ...
        rs = RadioSetting(
            "OM", "Opening Message", RadioSettingValueString(0, 48, res[2])
        )
        rs.set_apply_callback(lambda x: self._command("OM2 {}".format(str(x.value))))
        main.append(rs)

        res = self._command("PA", "PA([0-9]{2})")
        rs = RadioSetting(
            "PA", "Power Save Delay (seconds)", RadioSettingValueInteger(0, 99, res[1])
        )
        rs.set_apply_callback(lambda x: self._command("PA{:02d}".format(int(x.value))))
        main.append(rs)

        res = self._command("PI", "PI([1-9].[05])")
        rs = RadioSetting(
            "PI",
            "Power Save Interval (seconds)",
            RadioSettingValueFloat(1.0, 9.5, float(res[1]), 0.5, 1),
        )
        rs.set_apply_callback(
            lambda x: self._command("PI{:02d}".format(int(float(x.value) * 10)))
        )
        main.append(rs)
        ...
        rs = RadioSetting(
            "TI", "Priority Interval (seconds)", RadioSettingValueInteger(1, 19, res[1])
        )
        rs.set_apply_callback(lambda x: self._command("TI{:02d}".format(int(x.value))))
        main.append(rs)

        res = self._command("VL", "VL([0-9])")
        rs = RadioSetting("VL", "Beep Volume", RadioSettingValueInteger(0, 9, res[1]))
        rs.set_apply_callback(lambda x: self._command("VL{}".format(int(x.value))))
        main.append(rs)

        res = self._command("WP", "WP([01])")
        rs = RadioSetting("WP", "Write Protect", RadioSettingValueBoolean(int(res[1])))
        rs.set_apply_callback(lambda x: self._command("WP{}".format(int(x.value))))
        main.append(rs)
        ...
        rs = RadioSetting(
            "DA", "Dial Audio Squelch", RadioSettingValueInteger(0, 255, res[1])
        )
        rs.set_apply_callback(lambda x: self._command("DA{:03d}".format(int(x.value))))
        vfo.append(rs)

        res = self._command("DB", "DB[ +]([012][0-9]{2})")
        rs = RadioSetting(
            "DB", "Dial Level Squelch", RadioSettingValueInteger(0, 255, res[1])
        )
        rs.set_apply_callback(lambda x: self._command("DB{:03d}".format(int(x.value))))
        vfo.append(rs)

        res = self._command("DD", "DD([0-9][.][0-9])")

        rs = RadioSetting(
            "DDFF", "Dial Search Hold", RadioSettingValueBoolean(res[1] == "FF")
        )
        vfo.append(rs)

        value = float(res[1]) if res[1] != "FF" else 0
        rs = RadioSetting(
            "DD", "Dial Delay (seconds)", RadioSettingValueFloat(0, 9.9, value, 0.1, 1)
        )
        vfo.append(rs)

        res = self._command("DP", "DP([0-9]{2})")
        rs = RadioSetting(
            "DP", "Dial Pause (seconds)", RadioSettingValueInteger(0, 60, res[1])
        )
        rs.set_apply_callback(lambda x: self._command("DP{:02d}".format(int(x.value))))
        vfo.append(rs)

        VT_MAP = [("Off", 0), ("On, auto-store to bank J", 1), ("On, erase bank J", 2)]
        res = self._command("VT", "VT([012])")
        rs = RadioSetting(
            "VT", "VFO Auto-store", RadioSettingValueMap(VT_MAP, int(res[1]))
        )
        rs.set_apply_callback(lambda x: self._command("VT{}".format(int(x.value))))
        vfo.append(rs)
        ...
        for n in range(0, 48):
            rs = RadioSetting(
                ...
                RadioSettingValueFloat(0.0, 999.9999, ol_list[n] / 1000000, 0.0001, 4),
            )
            rs.value.set_mutable(n > 0 and n < 20)
            rs.set_apply_callback(
                lambda x, y: self._command(
                    ("OL{:02d} {:010d}" "".format(y, int(float(x.value) * 1000000)))
                ),
                n,
            )
            rs.set_warning(
                "Memories must be refreshed after changing " "offset frequencies."
            )
            freq.append(rs)
        ...
        sel = int(res[1])
        rs = RadioSetting(
            "GM", "Selected Scan Group", RadioSettingValueInteger(0, 9, sel)
        )
        rs.set_apply_callback(lambda x: self._command("GM{}".format(int(x.value))))
        grp.append(rs)
        ...
        for bank in self._SCAN_BANKS:
            ...
            rs = RadioSetting(
                "TB{}".format(bank),
                "Bank {} Name".format(bank),
                RadioSettingValueString(0, 8, res[2]),
            )
            rs.set_apply_callback(
                lambda x, y: self._command("TB{}{:8}".format(y, str(x.value))), bank
            )
            grp.append(rs)
        ...
        for bank in self._SCAN_BANKS[::2]:
            rs = RadioSetting(
                "MW{}".format(bank),
                "Bank {}/{}".format(bank, bank.lower()),
                RadioSettingValueMap(MW_MAP, self._scan_bank_size(bank)),
            )
            rs.set_apply_callback(
                lambda x, y: self._command(
                    "MW{}{:2d}".format(y, int(x.value)), timeout=600
                ),
                bank,
            )
            rs.set_warning(
                "Resizing scan banks takes a significant amount of "
        ...
        sel = int(res[1])
        rs = RadioSetting(
            "GS", "Selected Search Group", RadioSettingValueInteger(0, 9, sel)
        )
        rs.set_apply_callback(lambda x: self._command("GS{}".format(int(x.value))))
        grp.append(rs)
        ...
        for group in range(10):
            grp = RadioSettingSubGroup(
                "GS{}".format(group), "Search Group {}".format(group)
            )
            search.append(grp)
        ...
        for bank in self._SCAN_BANKS[::2]:
            ...
            for b, v in list(zip((bank, bank.lower()), res)):
                rs = RadioSetting(
                    "WM{}".format(b),
                    "Scan Bank {} Protect".format(b),
                    RadioSettingValueBoolean(v),
                )
                rs.set_apply_callback(
                    lambda x, y: self._command("WM{}{}".format(y, int(x.value))), b
                )
    ...
    def _set_settings_unlocked(self, settings):
        ...
        if (
            setdict["vfo"]["DDFF"].value.changed()
            or setdict["vfo"]["DD"].value.changed()
        ):
        ...
        for group in range(1, 10):
            ...
            if any(
                parent["BM{}{}".format(group, bank)].value.changed()
                for bank in self._SCAN_BANKS
            ):
        ...
        for group in range(1, 10):
            ...
            if (
                parent["SD{}FF".format(group)].value.changed()
                or parent["SD{}".format(group)].value.changed()
            ):
            ...
            if any(
                parent["BS{}{}".format(group, bank)].value.changed()
                for bank in self._SEARCH_BANKS
            ):
