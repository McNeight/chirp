...
VOLUMES.update({"Selectable": 0x30, "Current": 0xFF})
VOLUMES_REV = {v: k for k, v in VOLUMES.items()}

MIN_VOL_PRESET = {"Preset": 0x30, "Lowest Limit": 0x31}
...
def decode_dtmf(array):
    return "".join(
        "%x%x" % (int(i) & 0x0F, (int(i) & 0xF0) >> 4) for i in array if i != 0xFF
    )
...
def do_ident(radio):
    ...
    if ident[:6] not in (radio._model,):
        ...
        variants = {b"\x06": "K, K1, K3 (450-520 MHz)", b"\x07": "K2, K4 (400-470 MHz)"}
        if model == "P3180":
...
def do_download(radio):
    ...
    for block in range(0, 0xBF + 1):
        ...
        if cmd == b"Z":
            ...
            LOG.error("Radio sent %r (%02x), expected W(0x57)" % (cmd, chr(cmd)))
        ...
        if chunk and _chksum != ord(chksum):
            ...
            raise errors.RadioError(
                "Checksum failure while reading block. " "Check serial cable."
            )
...
def do_upload(radio):
    ...
    for addr in range(addr_base, radio._memsize, 0x40):
        ...
        ack = radio.pipe.read(1)
        if ack != b"\x06":
            LOG.error("Radio refused address 0x%02x with %r" % (block_addr, ack))
            raise errors.RadioError("Radio refused data block")
...
class KenwoodTKx180Radio(chirp_common.CloneModeRadio):
    ...
    def match_model(cls, filedata, filename):
        return filename.endswith(".dat") and filedata[15:21] == cls._model
    ...
    def probe_layout(self):
        ...
        for i, addr in enumerate(zone_addresses):
            ...
            if zone.zonetype != 0x31:
                LOG.error(
                    "Zone %i is type 0x%02x; " "I only support 0x31 (conventional)"
                )
                raise errors.RadioError(
    ...
    def expand_mmap(self, zone_sizes):
        ...
        for zone_number, count in enumerate(zone_sizes):
            ...
            if zone_number < len(old_zones):
                ...
                if old_count != count:
                    LOG.debug(
                        "Zone %i going from %i to %i" % (zone_number, old_count, count)
                    )
                ...
                dest_zone.zoneinfo.name = ("Zone %i" % (zone_number + 1)).ljust(12)
    ...
    def shuffle_zone(self):
        ...
        memories = [
            (i, raw_memories[i].number) for i in range(0, self.raw_zoneinfo.count)
        ]
        current = memories[:]
        ...
        if current == memories:
            ...
            return
        raw_data = [raw_memories[i].get_raw(asbytes=False) for i, n in memories]
    ...
    def get_prompts(cls):
        ...
        rp.info = (
            "This radio is zone-based, which is different from how "
        ...
        rp.experimental = (
            "This driver is very experimental. Every attempt "
    ...
    def get_memory(self, number):
        ...
        mem.extra.append(
            MemSetting(
                "pttid",
                "PTTID",
                RadioSettingValueMap(PTTID_SETTINGS.items(), int(_mem.pttid)),
            )
        )
    ...
    def set_memory(self, mem):
        ...
        if _mem is None:
            ...
            _mem = self._get_raw_memory(mem.number)
            if _mem is None:
                raise errors.RadioError("Internal error after " "memory allocation")
    ...
    def _pure_choice_setting(self, settings_key, name, choices, default="Off"):
        ...
        s = RadioSetting(
            ...
            RadioSettingValueList(
                ui_choices,
                get_choice(self._memobj.settings, settings_key, choices, default),
            ),
        )
        s.set_apply_callback(
            set_choice, self._memobj.settings, settings_key, choices, default
        )
        return s
    ...
    def _get_common1(self):
        ...
        common1.append(
            self._pure_choice_setting(
                "sublcd", "Sub LCD Display", SUBLCD, default="None"
            )
        )
        ...
        clockfmt = RadioSetting(
            "clockfmt",
            "Clock Format",
            RadioSettingValueList(CLOCKFMT, current_index=settings.clockfmt),
        )
        ...
        datefmt = RadioSetting(
            "datefmt",
            "Date Format",
            RadioSettingValueList(DATEFMT, current_index=settings.datefmt),
        )
        ...
        inverted_flags = [
            ("power_switch_memory", "Power Switch Memory"),
        ...
        it = RadioSetting(
            "it", "Ignition Timer (s)", RadioSettingValueInteger(10, 28800, _it)
        )
    ...
    def _get_common2(self):
        ...
        def apply_ponmsgtext(setting):
            settings.pon_msgtext = str(setting.value)[:12].strip().ljust(12, "\x00")

        common2.append(
            self._pure_choice_setting(
                "pon_msgtype", "Power On Message Type", STARTUP_MODES
            )
        )
        ...
        for value, name in volumes.items():
            ...
            volume = RadioSetting(
                "settings.%s" % value,
                name,
                RadioSettingValueList(VOLUMES.keys(), VOLUMES_REV.get(int(setting), 0)),
            )
        ...
        voloffset = RadioSetting(
            "tvo", "Tone Volume Offset", RadioSettingValueInteger(-5, 5, _voloffset)
        )
        ...
        volpreset = RadioSetting(
            ...
            "Minimum Volume Type",
            RadioSettingValueList(
                MIN_VOL_PRESET.keys(), MIN_VOL_PRESET_REV[_volpreset]
            ),
        )
    ...
    def _get_common3(self):
        ...
        rs = MemSetting(
            "settings.battsave",
            "Battery Save",
            RadioSettingValueMap(BATTSAVE_SETTINGS.items(), settings.battsave),
        )
        group.append(rs)
        ...
        rs = MemSetting(
            "settings.battwarn",
            "Battery Warning",
            RadioSettingValueMap(BATTWARN_SETTINGS.items(), settings.battwarn),
        )
        group.append(rs)
        ...
        rs = MemSetting(
            "settings.battstatus",
            "Battery Status",
            RadioSettingValueInvertedBoolean(not settings.battstatus),
        )
        group.append(rs)
        ...
        rs = MemSetting(
            "settings.pttid_type",
            "PTTID Type",
            RadioSettingValueMap(PTTID_TYPES.items(), settings.pttid_type),
        )
        group.append(rs)
        ...
        rs = RadioSetting(
            "pttid_bot", "Beginning of Transmit", RadioSettingValueString(0, 16, code)
        )
        ...
        rs = RadioSetting(
            "pttid_eot", "End of Transmit", RadioSettingValueString(0, 16, code)
        )
    ...
    def _get_zones(self):
        ...
        for i in range(len(self._zones)):
            ...
            name = MemSetting(
                ...
                "Name",
                RadioSettingValueString(0, 12, _name, mem_pad_char="\x00"),
            )
            zone.append(name)
            ...
            rs = MemSetting(
                "zone%i.zoneinfo.bcl_override" % i,
                "BCL Override",
                RadioSettingValueInvertedBoolean(not _zone.bcl_override),
            )
            zone.append(rs)
    ...
    def _get_ost(self, parent):
        ...
        for i in range(0, 40):
            ...
            ost = RadioSettingSubGroup("ost%i" % i, "OST %i" % (i + 1))

            cur = str(_ost.name).rstrip("\x00")
            name = MemSetting(
                "ost_tones[%i].name" % i,
                "Name",
                RadioSettingValueString(0, 12, cur, mem_pad_char="\x00"),
            )
            ost.append(name)
            ...
            if _ost.rxtone == 0xFFFF:
                ...
                if cur not in tones:
                    ...
                    tones.sort()
            rx = RadioSetting(
                "rxtone%i" % i, "RX Tone", RadioSettingValueList(_tones(), str(cur))
            )
            ...
            if _ost.txtone == 0xFFFF:
                ...
                if cur not in tones:
                    ...
                    tones.sort()
            tx = RadioSetting(
                "txtone%i" % i, "TX Tone", RadioSettingValueList(_tones(), str(cur))
            )
    ...
    def _get_keys(self):
        ...
        sec_functions = {
            k: v
            for k, v in (model_functions | BUTTON_FUNCTIONS).items()
        ...
        rs = MemSetting(
            "selector_knob",
            "Selector Knob",
            RadioSettingValueMap(KNOB_MODE.items(), self._memobj.selector_knob),
        )
        group.append(rs)
        ...
        rs = MemSetting(
            "keypad_type",
            "Keypad Type",
            RadioSettingValueMap(KEYPAD_TYPE.items(), self._memobj.keypad_type),
        )
        group.append(rs)
        ...
        rs = MemSetting(
            "keypad_op",
            "Keypad Operation",
            RadioSettingValueMap(KEYPAD_OP.items(), self._memobj.keypad_op),
        )
        group.append(rs)
        ...
        rs = MemSetting(
            "list_selector_key",
            "List Selector Key",
            RadioSettingValueInvertedBoolean(not self._memobj.list_selector_key),
        )
        group.append(rs)

        for key, index in BUTTONS.items():
            rs = MemSetting(
                "buttons[%i].primary" % index,
                key,
                RadioSettingValueMap(pri_functions.items(), buttons[index].primary),
            )
            pri.append(rs)

            rs = MemSetting(
                "buttons[%i].secondary" % index,
                key,
                RadioSettingValueMap(sec_functions.items(), buttons[index].secondary),
            )
            sec.append(rs)
        group.append(pri)
        group.append(sec)
    ...
    def get_settings(self):
        ...
        common = RadioSettingGroup("common", "Common", common1, common2, common3)
        ...
        for element in settings.apply_to(self._memobj):
            if element.get_name() == "_zonecount":
                ...
                if len(self._zones) > new_zone_count:
                    ...
                    self.expand_mmap(
                        zone_sizes + ([0] * (new_zone_count - len(self._zones)))
                    )
    ...
    def get_sub_devices(self):
        ...
        for i, _ in enumerate(self._zones):
            ...
            zone_cls = tk280.TKx80SubdevMeta.make_subdev(
                self,
                KenwoodTKx180RadioZone,
                zone,
                to_copy,
                VARIANT="Zone %s" % (str(zone.zoneinfo.name).rstrip("\x00").rstrip()),
            )
