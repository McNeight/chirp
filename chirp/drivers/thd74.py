...
DUPLEX = ["", "+", "-"]
TUNE_STEPS = [5.0, 6.25, 8.33, 9.0, 10.0, 12.5, 15.0, 20.0, 25.0, 30.0, 50.0, 100.0]
...
class KenwoodGroup(chirp_common.NamedBank):
    ...
    def get_name(self):
        name = self._model._radio._memobj.names[GROUP_NAME_OFFSET + self._index].name
...
@directory.register
class THD74Radio(chirp_common.CloneModeRadio, chirp_common.IcomDstarSupport):
    VENDOR = "Kenwood"
    ...
    def get_raw_memory(self, number):
        return repr(self._get_raw_memory(number)) + repr(self._memobj.flags[number])
