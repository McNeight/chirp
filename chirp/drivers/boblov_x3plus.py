...
@directory.register
class BoblovX3Plus(chirp_common.CloneModeRadio, chirp_common.ExperimentalRadio):
    ...
    VOXDELAY_LIST = ["1 seconds", "2 seconds", "3 seconds", "4 seconds", "5 seconds"]
    X3P_POWER_LEVELS = [
    ...
    def get_memory(self, number):
        ...
        if mem.freq == 0 or (rmem.rxfreq.get_raw(asbytes=False) == "\xff\xff\xff\xff"):
    ...
    def get_settings(self):
        ...
        rs = RadioSetting(
            "squelchlevel",
            "Squelch level",
            RadioSettingValueInteger(minval=0, maxval=9, current=cur.squelchlevel),
        )
        basic.append(rs)
        rs = RadioSetting(
            "timeouttimer",
        ...
        rs = RadioSetting(
            ...
            "Voice prompt",
            RadioSettingValueList(
                options=self.VOICE_LIST, current=self.VOICE_LIST[cur.voiceprompt]
            ),
        )
        basic.append(rs)
        rs = RadioSetting(
            ...
            "Vox level",
            RadioSettingValueList(
                options=self.VOXLEVEL_LIST, current=self.VOXLEVEL_LIST[cur.voxlevel]
            ),
        )
        basic.append(rs)
        rs = RadioSetting(
            ...
            "VOX delay",
            RadioSettingValueList(
                options=self.VOXDELAY_LIST, current=self.VOXDELAY_LIST[cur.voxdelay]
            ),
        )
        basic.append(rs)
        basic.append(
        ...
        basic.append(
            RadioSetting("beep", "Beep", RadioSettingValueBoolean(current=cur.beep))
        )
    ...
    def _read_block(self, block_addr, block_size):
        ...
        cmd = struct.pack(">cHb", self.CMD_READ, block_addr, block_size)
        resp_prefix = self.CMD_WRITE + cmd[1:]

        try:
            msg = (
                "Failed to write command to radio for block "
                "read at %04x" % block_addr
            )
            self._write(cmd)

            msg = (
                "Failed to read response from radio for block "
                "read at %04x" % block_addr
            )
            ...
            if response[: len(cmd)] != resp_prefix:
                raise errors.RadioError(
                    "Error reading block %04x, " "Command not returned." % (block_addr)
                )

            msg = (
                "Failed to write ACK to radio after block read at " "%04x" % block_addr
            )
            self._write(self.CMD_ACK)

            msg = (
                "Failed to read ACK from radio after block read at " "%04x" % block_addr
            )
            ack = self._read(1)
        except Exception:
        ...
        if ack != self.CMD_ACK:
            raise errors.RadioError("No ACK reading block " "%04x." % (block_addr))
    ...
    def _write_block(self, block_addr, block_size):
        ...
        LOG.debug("Writing Data:\n%s%s", util.hexprint(cmd), util.hexprint(data))
    ...
    def _enter_programming_mode(self):
        ...
        try:
            ...
            msg = "Error communicating with radio while querying " "model identifier"
            self._write(self.CMD_ACK)
            ...
            if ack != self.CMD_ACK:
                raise errors.RadioError(
                    "Radio refused to enter programming "
