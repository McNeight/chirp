...
@directory.register
class QuanshengTGUV2P(chirp_common.CloneModeRadio, chirp_common.ExperimentalRadio):
    """Quansheng TG-UV2+"""
    ...
    def get_prompts(cls):
        ...
        rp.experimental = (
            "Experimental version for TG-UV2/2+ radios " "Proceed at your own risk!"
        )
        rp.pre_download = _(
            "1. Turn radio off.\n"
    ...
    def get_memory(self, number):
        ...
        if (_mem.freq.get_raw(asbytes=False)[0] == "\xff") or (_bf.band == "\x0f"):
            mem.empty = True
            return mem
        ...
        rs = RadioSetting(
            "not_scramble", "(not)SCRAMBLE", RadioSettingValueBoolean(_mem.not_scramble)
        )
        mem.extra.append(rs)
    ...
    def get_settings(self):
        ...
        rs = RadioSetting(
            "time_out_timer",
            "TX Time Out Timer",
            RadioSettingValueList(options, current_index=_settings.time_out_timer),
        )
        cfg_grp.append(rs)
        ...
        rs = RadioSetting(
            ...
            RadioSettingValueList(options, current_index=_settings.display),
        )
        cfg_grp.append(rs)
        ...
        rs = RadioSetting(
            "vox", "VOX Level", RadioSettingValueMap(options_map, _settings.vox)
        )
        cfg_grp.append(rs)
        ...
        rs = RadioSetting(
            "rxmode",
            "Dual Watch/CrossBand Monitor",
            RadioSettingValueList(options, current_index=_rxmode),
        )
        cfg_grp.append(rs)
        ...
        rs = RadioSetting(
            "busy_lockout",
            "Busy Channel Lock",
            RadioSettingValueBoolean(not _settings.busy_lockout),
        )
        cfg_grp.append(rs)
        ...
        rs = RadioSetting(
            "keyunlocked",
            "Keypad Lock",
            RadioSettingValueBoolean(not _settings.keyunlocked),
        )
        cfg_grp.append(rs)
        ...
        rs = RadioSetting(
            "not_end_tone_elim",
            "Tx End Tone Elimination",
            RadioSettingValueMap(options_map, _settings.not_end_tone_elim),
        )
        cfg_grp.append(rs)
        ...
        for idx, vfo_group in enumerate(vfo_groups):
            ...
            rs = RadioSetting(
                vfo_lower[idx] + "_mode",
                vfo_upper[idx] + " Mode",
                RadioSettingValueList(options, current_index=tempvar),
            )
            vfo_group.append(rs)
            ...
            if tempvar == 0:
                ...
                if offset > 70e6:
                    offset = 0
                rs = RadioSetting(
                    vfo_lower[idx] + "_offset",
                    vfo_upper[idx] + " Offset",
                    RadioSettingValueFloat(
                        0.0, 69.995, offset / 100000.0, resolution=0.005
                    ),
                )
                vfo_group.append(rs)

                rs = RadioSetting(
                    vfo_lower[idx] + "_duplex",
                    vfo_upper[idx] + " Shift",
                    RadioSettingValueList(
                ...
                rs = RadioSetting(
                    ...
                    vfo_upper[idx] + " Power",
                    RadioSettingValueList(
                        POWER_LEVELS_STR, current_index=_bandsettings[band_num].power
                    ),
                )
                vfo_group.append(rs)
                ...
                rs = RadioSetting(
                    vfo_lower[idx] + "_ttmode",
                    vfo_upper[idx] + " TX tone mode",
                    RadioSettingValueList(options, current_index=txtmode),
                )
                vfo_group.append(rs)
                if txtmode == 1:
                    rs = RadioSetting(
                ...
                options = ["None", "Tone", "DTCS-N", "DTCS-I"]
                rs = RadioSetting(
                    vfo_lower[idx] + "_rtmode",
                    vfo_upper[idx] + " RX tone mode",
                    RadioSettingValueList(options, current_index=rxtmode),
                )
                vfo_group.append(rs)
            ...
            if not isinstance(element, RadioSetting):
                ...
                try:
                    ...
                    if element.has_apply_callback():
                        ...
                        if (
                            getattr(obj, "keyunlocked")
                            and int(element.value)
                            and (getattr(obj, "rxmode") != 0x02)
                        ):
                            raise errors.InvalidValueError(
                                "Keypad lock not allowed in " "Dual-Watch or CrossBand"
                            )
                        LOG.debug("Setting %s = %s" % (setting, not int(element.value)))
                        setattr(obj, setting, not int(element.value))
                    elif setting == "rxmode":
    ...
    def match_model(cls, filedata, filename):
        return filedata.startswith(b"TG-UV2+ Radio Program Data") and len(filedata) == (
            cls._memsize + 0x30
        )
