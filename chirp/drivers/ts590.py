...
def _read_mem(radio):
    ...
    for chn in range(0, (radio._upper + 21)):  # Loop stops at +20
        ...
        while r0ch != chn:
            # simplex
            if chn < 100:
                result0 = command(radio.pipe, "MR0 %02i" % chn, mrlen, W8S)
                result0 += read_str(radio)
            else:
        ...
        while r1ch != chn:
            # split
            if chn < 100:
                result1 = command(radio.pipe, "MR1 %02i" % chn, mrlen, W8S)
...
def _read_settings(radio):
    ...
    for cmc in setc:
        ...
        if cmc == "AS0":
            ...
            skipme = True
            for ix in range(8):
                result0 = command(radio.pipe, cmc, 6, W8S, "0%1i" % ix)  # Tx eq
                setts += chr(int(result0[4:]))  # 'EQ13x", want the x
                result0 = command(radio.pipe, cmc, 6, W8S, "1%1i" % ix)  # Rx eq
                setts += chr(int(result0[4:]))
                nc += 1
                status.cur = nc
                radio.status_fn(status)
        elif ((not radio.SG) and (cmc == "EX087")) or (radio.SG and (cmc == "EX001")):
    ...
    for ix in range(2):
        for cmx in setc:
            if str(cmx)[0:2] == "MF":
                ...
                if (radio.SG and cmx == "EX001") or ((not radio.SG) and cmx == "EX087"):
                    scm = cmx + "0000"
                ...
                # Now for the other EX cmds
                    ...
                    if radio.SG:
                        ...
                        scm = "%s0000%i" % (
                            cmx,
                            getattr(_mex[ix], radio.EX_X[cmx].lower()),
                        )
            result0 = command(radio.pipe, scm, stlen, W8S)
            snx += 1
            status.cur = snx
...
class TS590Radio(chirp_common.CloneModeRadio):
    ...
    def get_settings(self):
        ...
        def my_fnctns(setting, obj, ndx, atrb):
            ...
            if self.SG:
                ...
                if (vx > 99 and vx < 120) or (vx > 170 and vx < 200):
                    raise errors.RadioError(" %i Change Ignored for %s." % (vx, atrb))
        ...
        for mx in range(2):  # A/B index
            ...
            rset.set_apply_callback(my_val_list, options, _mex, "ex010", 0, mx)
            if mx == 0:
                mena.append(rset)
            else:
                menb.append(rset)
            ...
            rset.set_apply_callback(my_val_list, options, _mex, "ex012", 0, mx)
            if mx == 0:
                mena.append(rset)
            else:
                menb.append(rset)
            ...
            rset.set_apply_callback(my_val_list, options, _mex, "ex013", 0, mx)
            if mx == 0:
                mena.append(rset)
            else:
                menb.append(rset)
            ...
            if self.SG:
                options = ["Off", "0.5", "0.5", "1.0", "2.5", "5.0", "10.0"]
            ...
            rset.set_apply_callback(my_val_list, options, _mex, "ex016", 0, mx)
            if mx == 0:
                mena.append(rset)
            else:
                menb.append(rset)
            ...
            if self.SG:  # this setting only for SG
                rx = RadioSettingValueList(options, current_index=_mex[mx].ex017)

                sx = my_labels(17)
                rset = RadioSetting("exset.ex017/%d" % mx, sx, rx)
                rset.set_apply_callback(my_val_list, options, _mex, "ex017", 0, mx)
                if mx == 0:
                    mena.append(rset)
                else:
                    menb.append(rset)
            ...
            rset.set_apply_callback(my_val_list, options, _mex, "ex018", 0, mx)
            if mx == 0:
                mena.append(rset)
            else:
                menb.append(rset)
            ...
            rset.set_apply_callback(my_val_list, options, _mex, "ex019", 0, mx)
            if mx == 0:
                mena.append(rset)
            else:
                menb.append(rset)
            ...
            rset.set_apply_callback(my_val_list, options, _mex, "ex021", 0, mx)
            if mx == 0:
                mena.append(rset)
            else:
                menb.append(rset)
            ...
            rset.set_apply_callback(my_val_list, options, _mex, "ex024", 0, mx)
            if mx == 0:
                mena.append(rset)
            else:
                menb.append(rset)
            ...
            rset.set_apply_callback(my_val_list, options, _mex, "ex026", 0, mx)
            if mx == 0:
                mena.append(rset)
            else:
                menb.append(rset)
            ...
            rset.set_apply_callback(my_val_list, options, _mex, "ex055", 0, mx)
            if mx == 0:
                mena.append(rset)
            else:
                menb.append(rset)
        ...
        tcurves = ["Off", "HB1", "HB2", "FP", "BB1", "BB2", "C", "U"]
        rcurves = ["Off", "HB1", "HB2", "FP", "BB1", "BB2", "FLAT", "U"]
        for ix in range(8):
            rx = RadioSettingValueList(tcurves, current_index=_eqx[ix].txeq)
            rset = RadioSetting("eqx.txeq/%d" % ix, "TX %s Equalizer" % mohd[ix], rx)
            rset.set_apply_callback(my_val_list, tcurves, _eqx, "txeq", 0, ix)
            equ.append(rset)

            rx = RadioSettingValueList(rcurves, current_index=_eqx[ix].rxeq)
            rset = RadioSetting("eqx.rxeq/%d" % ix, "RX %s Equalizer" % mohd[ix], rx)
            rset.set_apply_callback(my_val_list, rcurves, _eqx, "rxeq", 0, ix)
            equ.append(rset)
