...
def download(radio):
    ...
    LOG.debug(
        "%04i P<R: %s" % (len(buf), util.hexprint(buf).replace("\n", "\n          "))
    )
    return buf
...
class AP510Memory(object):
    ...
    def set_smartbeacon(self, d):
        self._memobj[self.ATTR_MAP["smartbeacon"]] = struct.pack(
            ">7H",
...
class AP510Memory20141215(AP510Memory):
    ...
    ATTR_MAP = dict(
        list(AP510Memory.ATTR_MAP.items())
        + list(
            {
                "tx_volume": "21",  # 1-6
                "rx_volume": "22",  # 1-9
...
PTT_DELAY = ["60 ms", "120 ms", "180 ms", "300 ms", "480 ms", "600 ms", "1000 ms"]
...
TF_CARD = ["WPL", "KML"]
POWER_LEVELS = [
    chirp_common.PowerLevel("0.5 watt", watts=0.50),
    ...
    def get_settings(self):
        ...
        aprs.append(
            RadioSetting(
                "ssid",
                "SSID",
                RadioSettingValueInteger(0, 15, ord(self._mmap.callsign[6]) - 0x30),
            )
        )
        pttdelay = PTT_DELAY[int(self._mmap.pttdelay) - 1]
        aprs.append(
            RadioSetting(
                "pttdelay", "PTT Delay", RadioSettingValueList(PTT_DELAY, pttdelay)
            )
        )
        output = OUTPUT[int(self._mmap.output) - 1]
        aprs.append(
            RadioSetting("output", "Output", RadioSettingValueList(OUTPUT, output))
        )
        aprs.append(
            RadioSetting(
                "mice", "Mic-E", RadioSettingValueBoolean(strbool(self._mmap.mice))
            )
        )
        try:
        ...
        try:
            aprs.append(
                RadioSetting(
                    ...
                    "Path 1",
                    RadioSettingValueString(
                        0, 6, self._mmap.path1[:6], autopad=True, charset=CHARSET
                    ),
                )
            )
            ssid1 = ord(self._mmap.path1[6]) - 0x30
            aprs.append(
                RadioSetting("ssid1", "SSID 1", RadioSettingValueInteger(0, 7, ssid1))
            )
            aprs.append(
                RadioSetting(
                    "path2",
                    "Path 2",
                    RadioSettingValueString(
                        0, 6, self._mmap.path2[:6], autopad=True, charset=CHARSET
                    ),
                )
            )
            ssid2 = ord(self._mmap.path2[6]) - 0x30
            aprs.append(
                RadioSetting("ssid2", "SSID 2", RadioSettingValueInteger(0, 7, ssid2))
            )
            aprs.append(
                RadioSetting(
                    "path3",
                    "Path 3",
                    RadioSettingValueString(
                        0, 6, self._mmap.path3[:6], autopad=True, charset=CHARSET
                    ),
                )
            )
            ssid3 = ord(self._mmap.path3[6]) - 0x30
            aprs.append(
                RadioSetting("ssid3", "SSID 3", RadioSettingValueInteger(0, 7, ssid3))
            )
        except NotImplementedError:
            aprs.append(
                RadioSetting(
        ...
        aprs.append(
            RadioSetting(
                "symbol", "Symbol", RadioSettingValueList(SYMBOL, self._mmap.symbol[0])
            )
        )
        aprs.append(
            RadioSetting(
                "beacon",
                "Beacon Mode",
                RadioSettingValueList(BEACON, current_index=int(self._mmap.beacon) - 1),
            )
        )
        aprs.append(
            RadioSetting(
        ...
        try:
            ...
            aprs.append(
                RadioSetting(
                    "voltage", "Voltage in comment", RadioSettingValueBoolean(voltage)
                )
            )
        ...
        try:
            ...
            aprs.append(
                RadioSetting(
                    "telemetry", "Telemetry", RadioSettingValueBoolean(telemetry)
                )
            )
        ...
        fields = [
            ...
            (
                "chinalat",
                "Lat",
                RadioSettingValueInteger(-45, 45, ord(self._mmap.chinamapfix[2]) - 80),
            ),
            (
                "chinalon",
                "Lon",
                RadioSettingValueInteger(-45, 45, ord(self._mmap.chinamapfix[1]) - 80),
            ),
        ]
        for field in fields:
        ...
        fields = [
            ...
            (
                "btext",
                "Static Position BTEXT",
                RadioSettingValueString(
                    0, 27, self._mmap.virtualgps[1:], autopad=False, charset=CHARSET
                ),
            ),
        ]
        for field in fields:
        ...
        for field in fields:
            smartbeacon.append(
                RadioSetting(
                    field[0], field[1], RadioSettingValueInteger(0, 9999, sb[field[0]])
                )
            )
        ...
        try:
            system.append(
                RadioSetting(
                    "tx_volume",
                    "Transmit volume",
                    RadioSettingValueList(map(str, range(1, 7)), self._mmap.tx_volume),
                )
            )
            system.append(
                RadioSetting(
                    "rx_volume",
                    "Receive volume",
                    RadioSettingValueList(map(str, range(1, 10)), self._mmap.rx_volume),
                )
            )
            system.append(
                RadioSetting(
                    ...
                    "Squelch",
                    RadioSettingValueList(
                        map(str, range(0, 9)), str(self._mmap.multiple["squelch"])
                    ),
                )
            )
            system.append(
                RadioSetting(
                    "tx_serial_ui_out",
                    "Tx serial UI out",
                    RadioSettingValueBoolean(strbool(self._mmap.tx_serial_ui_out)),
                )
            )
            system.append(
                RadioSetting(
                    "auto_on",
                    "Auto-on with 5V input",
            ...
            system.append(
                RadioSetting(
                    "tfx", "TF/X", RadioSettingValueBoolean(self._mmap.multiple["tfx"])
                )
            )
            system.append(
                RadioSetting(
                    "blueled",
                    "Light blue LED on GPS lock",
            ...
            system.append(
                RadioSetting(
                    ...
                    "TF card format",
                    RadioSettingValueList(
                        TF_CARD, current_index=(int(self._mmap.multiple["tf_card"]))
                    ),
                )
            )
        except NotImplementedError:
        ...
        for setting in settings:
            ...
            try:
                ...
                if name == "callsign":
                    ...
                    self._mmap.pttdelay = PTT_DELAY.index(str(setting.value)) + 1
                elif name == "output":
                    ...
                    multiple["mice_message"] = MICE_MESSAGE.index(str(setting.value))
                    self._mmap.multiple = multiple
                elif name == "path":
                    self._mmap.path = PATH.index(str(setting.value))
                elif name == "path1":
                    self._mmap.path1 = "%s%s" % (setting.value, self._mmap.path1[6])
                elif name == "ssid1":
                    self._mmap.path1 = "%s%s" % (self._mmap.path1[:6], setting.value)
                elif name == "path2":
                    self._mmap.path2 = "%s%s" % (setting.value, self._mmap.path2[6])
                elif name == "ssid2":
                    self._mmap.path2 = "%s%s" % (self._mmap.path2[:6], setting.value)
                elif name == "path3":
                    self._mmap.path3 = "%s%s" % (setting.value, self._mmap.path3[6])
                elif name == "ssid3":
                    self._mmap.path3 = "%s%s" % (self._mmap.path3[:6], setting.value)
                elif name == "table":
                    ...
                    self.set_digipeat(alias=str(ALIAS.index(str(setting.value)) + 1))
