...
POWER_LEVELS = [
    chirp_common.PowerLevel("Low", dBm=10),
...
class HobbyPCBRSUV3Radio(chirp_common.LiveRadio):
    ...
    def get_settings(self):
        ...
        cl = RadioSetting(
            "CL%15s", "CW Callsign", RadioSettingValueString(0, 15, _get("CL"))
        )
        cw.append(cl)
        ...
        cs = RadioSetting(
            "CS%02i", "CW Speed", RadioSettingValueInteger(5, 25, int(_get("CS")))
        )
        cw.append(cs)
        ...
        bm = RadioSetting(
            "BM%15s", "Beacon Message", RadioSettingValueString(0, 15, _get("BM"))
        )
        cw.append(bm)

        bt = RadioSetting(
            "BT%03i", "Beacon Timer", RadioSettingValueInteger(0, 600, int(_get("BT")))
        )
        cw.append(bt)

        it = RadioSetting(
            "IT%03i", "CW ID Timer", RadioSettingValueInteger(0, 500, int(_get("IT")))
        )
        cw.append(it)

        tg = RadioSetting(
            "TG%7s", "CW Timeout Message", RadioSettingValueString(0, 7, _get("TG"))
        )
        cw.append(tg)
        ...
        af = RadioSetting(
            "AF%i", "Arduino LPF", RadioSettingValueBoolean(_get("AF") == "ON")
        )
        io.append(af)
        ...
        ai = RadioSetting(
            "AI%i",
            "Arduino Input Pin",
            RadioSettingValueList(input_pin, current_index=int(_get("AI"))),
        )
        io.append(ai)

        output_pin = ["LOW", "SQ OPEN", "DTMF DETECT", "TX ON", "CTCSS DET", "HIGH"]
        ao = RadioSetting(
            "AO%i",
            "Arduino Output Pin",
            RadioSettingValueList(output_pin, current_index=int(_get("AO"))),
        )
        io.append(ao)
        ...
        b1 = RadioSetting(
            "B1%i",
            "Arduino Baudrate",
            RadioSettingValueList(bauds, current_index=int(_get("B1"))),
        )
        io.append(b1)
        ...
        b2 = RadioSetting(
            "B2%i",
            "Main Baudrate",
            RadioSettingValueList(bauds, current_index=int(_get("B2"))),
        )
        io.append(b2)
        ...
        dr = RadioSetting(
            "DR%i", "DTMF Tone Detector", RadioSettingValueBoolean(_get("DR") == "ON")
        )
        dtmf.append(dr)
        ...
        sd = RadioSetting(
            "SD%i", "DTMF/CW Side Tone", RadioSettingValueBoolean(_get("SD") == "ON")
        )
        dtmf.append(sd)
        ...
        dp = RadioSetting(
            "DP%i", "Pre-Emphasis", RadioSettingValueBoolean(_get("DP") == "ON")
        )
        general.append(dp)

        fw = RadioSetting(
            "_fw", "Firmware Version", RadioSettingValueString(0, 20, _get("FW"))
        )
        general.append(fw)

        gm = RadioSetting(
            "GM%02i", "Mic Gain", RadioSettingValueInteger(0, 15, int(_get("GM")))
        )
        general.append(gm)
        ...
        ht = RadioSetting(
            "HT%04i", "Hang Time", RadioSettingValueInteger(0, 5000, int(_get("HT")))
        )
        general.append(ht)
        ...
        ld = RadioSetting(
            "LD%i",
            "LED Mode",
            RadioSettingValueList(ledmode, current_index=int(_get("LD"))),
        )
        general.append(ld)

        sq = RadioSetting(
            "SQ%i", "Squelch Level", RadioSettingValueInteger(0, 9, int(_get("SQ")))
        )
        general.append(sq)

        to = RadioSetting(
            "TO%03i", "Timeout Timer", RadioSettingValueInteger(0, 600, int(_get("TO")))
        )
        general.append(to)
        ...
        rc = RadioSetting("RC%i", "Current Channel", RadioSettingValueInteger(0, 9, 0))
        rc.set_doc(
            "Choosing one of these values causes the radio "
