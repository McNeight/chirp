...
POWER_LEVELS = [
    chirp_common.PowerLevel("Low", watts=25),
...
class TKx160Radio(chirp_common.CloneModeRadio):
    ...
    def match_model(cls, filedata, filename):
        return filename.endswith(".dat") and filedata[0x1740:0x1746] == cls._model
    ...
    def _set_num_zones(self, count):
        ...
        for i in range(128):
            ...
            if index_obj.zone != 0xFF and index_obj.zone > count:
                LOG.debug(
                    "Deleting memory %i:%i to delete zone",
                    index_obj.zone,
                    index_obj.memory,
                )
    ...
    def get_raw_memory(self, number):
        ...
        return repr(self._memobj.index[index_entry]) + repr(self._memobj.memories[slot])
    ...
    def check_index(self, number):
        ...
        for i in range(128):
            ...
            if index_obj.zone == self._zone and index_obj.memory == number:
                LOG.debug("Found %i:%i in slot %i", self._zone, number, index_obj.slot)
    ...
    def next_slot(self, number):
        ...
        LOG.debug("Putting channel %i in index spot %i", number, next_index)
        index_obj = self._memobj.index[next_index]
    ...
    def _delete_entry_from_index(self, index_entry):
        ...
        if index_entry != last_entry:
            LOG.debug(
                "Moving index entry %i to %i for delete of %i:%i",
                last_entry,
                index_entry,
                dst_obj.zone,
                dst_obj.memory,
    ...
    def get_settings(self):
        ...
        keys = self.make_key_group(self._memobj.keys, KEY_NAMES, KEYS)
...
KEYS_7162 = {
    i: key for i, key in enumerate(_KEYS) if key in VALID_KEY_SELECTION_7162_TRIANGLE
}
...
class TK7162Radio(TKx160Radio):
    ...
    def get_settings(self) -> settings.RadioSettings:
        ...
        SEL_ID_LOOKUP = {n: i for i, n in enumerate(VALID_KEY_SELECTION_7162_TRIANGLE)}

        for desc, name in KEY_NAMES.items():
            # radio_idx -> key_name -> sel_idx
            options = (
                VALID_KEY_SELECTION_7162_TRIANGLE
                if name == "triangle"
        ...
        gen.append(
            settings.RadioSetting(
                ...
                settings.RadioSettingValueInteger(
                    minval=0, maxval=9, current=_settings.sql
                ),
            )
        )
    ...
    def set_settings(self, _settings):
        ...
        SEL_ID = {i: n for i, n in enumerate(VALID_KEY_SELECTION_7162_TRIANGLE)}
        KEYS_7162_LOOKUP = {n: i for i, n in KEYS_7162.items()}
