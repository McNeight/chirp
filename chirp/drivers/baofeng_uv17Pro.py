...
def _crypt(symbol_index, buffer):
    ...
    for index2 in range(len(buffer)):
        bool_encrypt_char = (
            (tbl_encrypt_symbols[index1] != 32)
            and (buffer[index2] != 0)
            and (buffer[index2] != 255)
            and (buffer[index2] != tbl_encrypt_symbols[index1])
            and (buffer[index2] != (tbl_encrypt_symbols[index1] ^ 255))
        )
    ...
    for ident in radio._idents:
        ...
        try:
            ...
            if not ack.startswith(radio._fingerprint):
                LOG.debug(
                    "Ack %r did not match fingerprint: %r", ack, radio._fingerprint
                )
                continue
...
class UV17Pro(bfc.BaofengCommonHT):
    ...
    VALID_BANDS = [_vhf_range, _vhf2_range, _uhf_range]
    ...
    def apply_codename(self, setting, obj):
        codename = str(setting.value).encode("gb2312")[:10].ljust(10, b"\xff")
        obj.name = codename
    ...
    def get_settings_common_dtmf(self, dtmfe, _mem):
        for i in range(0, len(self.SCODE_LIST)):
            ...
            rs = RadioSetting(
                ...
                RadioSettingValueString(
                    0, 10, self._filterCodeName(_nameobj.name), False, CHARSET_GB2312
                ),
            )
    ...
    def get_settings_pro_dtmf(self, dtmfe, _mem):
        ...
        rs = RadioSetting(
            "ani.dtmfon",
            "DTMF Speed (on)",
            RadioSettingValueList(self.LIST_DTMFSPEED, current_index=val),
        )
        dtmfe.append(rs)
        ...
        if self._has_when_to_send_aniid:
            rs = RadioSetting(
                "ani.aniid",
                "When to send ANI ID",
                RadioSettingValueList(LIST_PTTID, current_index=_mem.ani.aniid),
            )
            dtmfe.append(rs)
        ...
        rs = RadioSetting(
            "settings.hangup",
            "Hang-up time",
            RadioSettingValueList(LIST_HANGUPTIME, current_index=val),
        )
        dtmfe.append(rs)
    ...
    def get_settings_common_basic(self, basic, _mem):
        ...
        rs = RadioSetting(
            "settings.tot",
            "Timeout Timer",
            RadioSettingValueList(self.LIST_TIMEOUT, current_index=val),
        )
        basic.append(rs)
        ...
        rs = RadioSetting(
            "settings.dualstandby",
            "Dual Watch",
            RadioSettingValueList(self.LIST_DUAL_WATCH, current_index=val),
        )
        basic.append(rs)
        ...
        if self._has_voice:
            ...
            rs = RadioSetting(
                "settings.voice",
                "Voice Prompt",
                RadioSettingValueList(self.LIST_VOICE, current_index=val),
            )
            basic.append(rs)
        ...
        rs = RadioSetting(
            "settings.backlight",
            "Backlight Timer",
            RadioSettingValueList(self.LIST_BACKLIGHT_TIMER, current_index=val),
        )
        basic.append(rs)
        ...
        rs = RadioSetting(
            "settings.beep",
            "Beep",
            RadioSettingValueList(self.LIST_BEEP, current_index=_mem.settings.beep),
        )
        basic.append(rs)

        rs = RadioSetting(
            "settings.roger", "Roger", RadioSettingValueBoolean(_mem.settings.roger)
        )
        basic.append(rs)

        rs = RadioSetting(
            "settings.chadistype",
            "Channel A display type",
            RadioSettingValueList(
                self.LIST_MODE, current_index=_mem.settings.chadistype
            ),
        )
        basic.append(rs)
        ...
        rs = RadioSetting(
            "settings.chbdistype",
            "Channel B display type",
            RadioSettingValueList(
                self.LIST_MODE, current_index=_mem.settings.chbdistype
            ),
        )
        basic.append(rs)
    ...
    def get_settings_pro_basic(self, basic, _mem):
        ...
        rs = RadioSetting(
            "settings.savemode",
            "Save Mode",
            RadioSettingValueList(self.LIST_PW_SAVEMODE, current_index=val),
        )
        basic.append(rs)
        ...
        rs = RadioSetting(
            ...
            "Timeout Timer Alarm",
            RadioSettingValueList(
                LIST_TIMEOUT_ALARM, current_index=_mem.settings.totalarm
            ),
        )
        basic.append(rs)
        ...
        if self._has_pilot_tone:
            rs = RadioSetting(
                ...
                "Pilot Tone",
                RadioSettingValueList(
                    LIST_PILOT_TONE, current_index=_mem.settings.tone
                ),
            )
            basic.append(rs)
        ...
        rs = RadioSetting(
            "settings.sidetone",
            "Side Tone",
            RadioSettingValueList(LIST_SIDE_TONE, current_index=_mem.settings.sidetone),
        )
        basic.append(rs)
        ...
        rs = RadioSetting(
            ...
            "Alarm Mode",
            RadioSettingValueList(
                LIST_ALARMMODE, current_index=_mem.settings.alarmmode
            ),
        )
        basic.append(rs)
        ...
        if self._has_send_id_delay:
            rs = RadioSetting(
                ...
                "Send ID Delay",
                RadioSettingValueList(
                    self.LIST_ID_DELAY, current_index=_mem.settings.pttdly
                ),
            )
            basic.append(rs)

        rs = RadioSetting(
            "settings.ctsdcsscantype",
            "QT Save Mode",
            RadioSettingValueList(
                LIST_QT_SAVEMODE, current_index=_mem.settings.ctsdcsscantype
            ),
        )
        basic.append(rs)
        ...
        def make_apply_key(attr_name):
            def apply(setting, obj):
                ...
                setattr(obj, attr_name, KEY_NAME_TO_CODE.get(val, 0x07))
            ...
            if getattr(self, flag):
                rs = RadioSetting(
                    ...
                    RadioSettingValueList(
                        ...
                        current_index=get_key_index(getattr(_mem.settings, attr)),
                    ),
                )
        ...
        if self._has_skey_disable:
            rs = RadioSetting(
                ...
                "Side Key Disable",
                RadioSettingValueList(
                    self.LIST_SKEY_DISABLE, current_index=_mem.settings.skdisable
                ),
            )
            basic.append(rs)
        ...
        rs = RadioSetting(
            ...
            "Channel A work mode",
            RadioSettingValueList(
                LIST_WORKMODE, current_index=_mem.settings.chaworkmode
            ),
        )
        basic.append(rs)
        ...
        rs = RadioSetting(
            ...
            "Channel B work mode",
            RadioSettingValueList(
                LIST_WORKMODE, current_index=_mem.settings.chbworkmode
            ),
        )
        basic.append(rs)
        ...
        rs = RadioSetting(
            ...
            "Rpt Tail Clear",
            RadioSettingValueList(
                LIST_RPT_TAIL_CLEAR, current_index=_mem.settings.rpttailclear
            ),
        )
        basic.append(rs)
        ...
        rs = RadioSetting(
            ...
            "Rpt Tail Delay",
            RadioSettingValueList(
                LIST_RPT_TAIL_CLEAR, current_index=_mem.settings.rpttaildet
            ),
        )
        basic.append(rs)
        ...
        if self._has_voxsw:
            ...
            rs = RadioSetting(
                ...
                "Vox Level",
                RadioSettingValueList(
                    LIST_VOX_LEVEL_ALT, current_index=_mem.settings.vox
                ),
            )
            basic.append(rs)
        else:
            rs = RadioSetting(
                "settings.vox",
                "Vox Level",
                RadioSettingValueList(LIST_VOX_LEVEL, current_index=_mem.settings.vox),
            )
            basic.append(rs)
        ...
        rs = RadioSetting(
            ...
            "Vox Delay Time",
            RadioSettingValueList(
                LIST_VOX_DELAY_TIME, current_index=_mem.settings.voxdlytime
            ),
        )
        basic.append(rs)
        ...
        if self._has_gps:
            rs = RadioSetting(
                "settings.gpsw", "GPS On", RadioSettingValueBoolean(_mem.settings.gpsw)
            )
            basic.append(rs)
            ...
            rs = RadioSetting(
                ...
                "GPS Mode",
                RadioSettingValueList(
                    LIST_GPS_MODE, current_index=_mem.settings.gpsmode
                ),
            )
            basic.append(rs)

            rs = RadioSetting(
                "settings.gpstimezone",
                "GPS Timezone",
                RadioSettingValueList(
                    LIST_GPS_TIMEZONE, current_index=_mem.settings.gpstimezone
                ),
            )
            basic.append(rs)
        ...
        if self._has_bt:
            rs = RadioSetting(
                "settings.bluetooth",
                "Bluetooth",
                RadioSettingValueBoolean(_mem.settings.bluetooth),
            )
            basic.append(rs)
        ...
        if self._has_zone_linking:
            for i in range(1, 11):
                rs = RadioSetting(
                    f"settings.zn{i}linkd",
                    f"Scan Link Zone {i}",
                    RadioSettingValueBoolean(getattr(_mem.settings, f"zn{i}linkd")),
                )
                basic.append(rs)
    ...
    def get_settings_common_workmode(self, workmode, _mem):
        ...
        freqA = RadioSettingValueString(0, 10, bfc.bcd_decode_freq(_mem.vfo.a.freq))
        freqA.set_validate_callback(my_validate)
        rs = RadioSetting("vfo.a.freq", "Frequency", freqA)
        rs.set_apply_callback(apply_freq, _mem.vfo.a)
        vfoA.append(rs)

        freqB = RadioSettingValueString(0, 10, bfc.bcd_decode_freq(_mem.vfo.b.freq))
        ...
        if _mem.vfo.a.sftd >= len(LIST_SHIFTS):
            ...
            val = _mem.vfo.a.sftd
        rs = RadioSetting(
            "vfo.a.sftd", "Shift", RadioSettingValueList(LIST_SHIFTS, current_index=val)
        )
        vfoA.append(rs)
        ...
        if _mem.vfo.b.sftd >= len(LIST_SHIFTS):
            ...
            val = _mem.vfo.b.sftd
        rs = RadioSetting(
            "vfo.b.sftd", "Shift", RadioSettingValueList(LIST_SHIFTS, current_index=val)
        )
        vfoB.append(rs)
        ...
        rs = RadioSetting("vfo.a.offset", "Offset (0.0-999.999)", offA)
        ...
        rs = RadioSetting("vfo.b.offset", "Offset (0.0-999.999)", offB)
        ...
        rs = RadioSetting(
            "vfo.a.lowpower",
            "Power",
            RadioSettingValueList(POWER_LEVELS, current_index=val),
        )
        vfoA.append(rs)
        ...
        rs = RadioSetting(
            "vfo.a.wide",
            "Bandwidth",
            RadioSettingValueList(LIST_BANDWIDTH, current_index=val),
        )
        vfoA.append(rs)
        ...
        rs = RadioSetting(
            "vfo.a.scode",
            "Signal Code",
            RadioSettingValueList(self.SCODE_LIST, current_index=val),
        )
        vfoA.append(rs)
        ...
        rs = RadioSetting(
            "vfo.a.step",
            "Tuning Step",
            RadioSettingValueList(self.LIST_STEPS, current_index=val),
        )
        vfoA.append(rs)
        ...
        if self._has_support_for_banknames:
            for i in range(0, 10):
                ...
                rs = RadioSetting(
                    ...
                    "Bank name %i" % (i + 1),
                    RadioSettingValueString(
                        0, 16, _filterName(_nameobj.name), False, CHARSET_GB2312
                    ),
                )
                if self.MODEL == "BF-F8HP-PRO":
                    rs.set_doc(
                        "12 characters maximum (only the first 6 show "
        ...
        if self._is_txinh(_mem):
            ...
            if offset != 0:
                if chirp_common.is_split(
                    self.get_features().valid_bands, freq, int(_mem.txfreq) * 10
                ):
                    duplex = "split"
    ...
    def get_memory_common(self, _mem, name, mem):
        ...
        rs = RadioSetting(
            "scode",
            "S-CODE",
            RadioSettingValueList(self.SCODE_LIST, current_index=scode),
        )
        mem.extra.append(rs)
        ...
        if self._has_scramble:
            rs = RadioSetting(
                "scramble",
                "Scramble",
                RadioSettingValueList(self.SCRAMBLE_LIST, current_index=_mem.scramble),
            )
            mem.extra.append(rs)
        ...
        if self.MODEL in ["BF-F8HP-PRO"]:
            rs = RadioSetting(
                "sqmode",
                "RX DTMF",
                RadioSettingValueList(self.RXDTMF_LIST, current_index=_mem.sqmode),
            )
            mem.extra.append(rs)

            rs = RadioSetting("fhss", "Encryption", RadioSettingValueBoolean(_mem.fhss))
            mem.extra.append(rs)

        if self.MODEL in ["K6", "UV-5R Mini"]:
            rs = RadioSetting(
                "sqmode", "RX DTMF", RadioSettingValueBoolean(_mem.sqmode)
            )
            mem.extra.append(rs)

            rs = RadioSetting("fhss", "FHSS", RadioSettingValueBoolean(_mem.fhss))
            mem.extra.append(rs)
            ...
            for setting in mem.extra:
                if setting.get_name() == "scode":
                    setattr(
                        _mem,
                        setting.get_name(),
                        str(int(setting.value) + self._scode_offset),
                    )
                else:
...
class UV17ProGPSGMRS(UV17ProGPS):
    ...
    def validate_memory(self, mem):
        ...
        msg_low_power = "GMRS 467 MHz interstitial frequencies require low power"
        msg_nfm = "GMRS 467 MHz interstitial frequencies require narrowband (NFM)"

        gmrs_tx_freqs = set(bandplan_na.ALL_GMRS_FREQS)
        gmrs_tx_freqs.update(freq + 5000000 for freq in bandplan_na.GMRS_HIRPT)
...
class BaofengUV5RMPlus(BF5RM):
    ...
    _magics = [
        ...
        (
            ...
            + b"\x0e\x0e\x02\x0f\x08\x04\x11\x0c\x00\x0d\x0e\x00",
            1,
        ),
    ]
    _encrsym = 13
...
class F8HPPro(UV17Pro):
    ...
    _magics = [
        ...
        (
            ...
            + b"\x12\x0a\x11\x06\x04\x0e\x02\x09\x0d\x00\x00",
            1,
        ),
    ]
    _encrsym = 3
    ...
    LIST_STEPS = ["2.5", "5.0", "6.25", "10.0", "12.5", "20.0", "25.0", "50.0", "100.0"]
    ...
    def get_settings_pro_dtmf(self, dtmfe, _mem):
        ...
        rs = RadioSetting(
            ...
            "Separate Code",
            RadioSettingValueList(
                self.LIST_SEPARATE_CODE, current_index=_mem.ani.separatecode
            ),
        )
        dtmfe.append(rs)
        ...
        rs = RadioSetting(
            ...
            "Group Call Code",
            RadioSettingValueList(
                self.LIST_GROUP_CALL_CODE, current_index=_mem.ani.groupcallcode
            ),
        )
        dtmfe.append(rs)
    ...
    def get_settings_pro_basic(self, basic, _mem):
        ...
        rs = RadioSetting(
            ...
            "Power on Time",
            RadioSettingValueList(
                self.LIST_POWER_ON_TIME, current_index=_mem.settings.pontime
            ),
        )
        basic.append(rs)
        ...
        rs = RadioSetting(
            ...
            "GPS Speed Units",
            RadioSettingValueList(
                self.LIST_GPS_UNITS, current_index=_mem.settings.gpsunits
            ),
        )
        basic.append(rs)
        ...
        def apply_stationid(setting, obj):
            stationid = str(setting.value).encode("gb2312")[:8].ljust(8, b"\xff")
        ...
        rs = RadioSetting(
            ...
            "StationID",
            RadioSettingValueString(
                0, 8, _filterStationID(_nameobj.stationid), False, CHARSET_GB2312
            ),
        )
...
class BFK6(UV17Pro):
    ...
    LIST_BEEP = ["Off", "On"]
    LIST_ID_DELAY = ["Off", "100 ms", "200 ms"] + [
        "%s ms" % x for x in range(400, 1200, 200)
    ]
    LIST_SKEYS = ["Off", "Flashlight", "TX Power", "Scan", "VOX", "SOS", "FM Radio"]
    LIST_DUAL_WATCH = [
        "Single Display",
    ...
    def get_settings_pro_basic(self, basic, _mem):
        ...
        rs = RadioSetting(
            "settings.tdr_tx",
            "TDR TX",
            RadioSettingValueList(LIST_OFF_AB, current_index=val),
        )
        basic.append(rs)
        ...
        rs = RadioSetting(
            "settings.powerontone",
            "Power On Tone",
            RadioSettingValueList(LIST_POWERON_TONE, current_index=val),
        )
        basic.append(rs)
        ...
        rs = RadioSetting(
            "settings.lcdcontrast",
            "LCD Contrast",
            RadioSettingValueList(LIST_LCD_CONTRAST, current_index=val),
        )
        basic.append(rs)
        ...
        rs = RadioSetting(
            ...
            "PF1",
            RadioSettingValueList(
                self.LIST_SKEYS, current_index=_mem.settings.key1short
            ),
        )
        basic.append(rs)
        ...
        rs = RadioSetting(
            ...
            "PF1 Long",
            RadioSettingValueList(
                self.LIST_SKEYS, current_index=_mem.settings.key1long
            ),
        )
        basic.append(rs)
        ...
        rs = RadioSetting(
            ...
            "PF2",
            RadioSettingValueList(
                self.LIST_SKEYS, current_index=_mem.settings.key2short
            ),
        )
        basic.append(rs)
        ...
        def apply_stationid(setting, obj):
            stationid = str(setting.value).encode("gb2312")[:16].ljust(16, b"\xff")
            obj.stationid = stationid
        ...
        rs = RadioSetting(
            ...
            "StationID",
            RadioSettingValueString(
                0, 16, _filterStationID(_nameobj.stationid), False, CHARSET_GB2312
            ),
        )
        rs.set_apply_callback(apply_stationid, _nameobj)
        basic.append(rs)
        ...
        rs = RadioSetting(
            "modes.modetype",
            "Factory Modes",
            RadioSettingValueList(LIST_MODES, current_index=_mem.modes.modetype),
        )
        basic.append(rs)

        rs = RadioSetting(
            "modes.tx_220mhz", "220M TX", RadioSettingValueBoolean(_mem.modes.tx_220mhz)
        )
        basic.append(rs)

        rs = RadioSetting(
            "modes.tx_520mhz", "520M TX", RadioSettingValueBoolean(_mem.modes.tx_520mhz)
        )
        basic.append(rs)

        rs = RadioSetting(
            "modes.rx_am", "AM RX", RadioSettingValueBoolean(_mem.modes.rx_am)
        )
        basic.append(rs)
...
class UV5RMini(UV17Pro):
    ...
    LIST_BEEP = ["Off", "On"]
    LIST_DTMFSPEED = ["%s ms" % x for x in [50, 100, 200, 300, 400, 500]]
    LIST_SKEY2_SHORT = ["FM Radio", "Scan", "Search", "VOX", "Flashlight", "SOS"]
    ...
    def _upload(radio):
        ...
        data_addr = 0x00
        radio_mem = radio.get_mmap()
        radio.pipe.log("Uploading to radio using block size of 0x%04x" % _blocksize)
        for i in range(len(radio.MEM_SIZES)):
            MEM_SIZE = radio.MEM_SIZES[i]
            MEM_START = radio.MEM_STARTS[i]
            for addr in range(MEM_START, MEM_START + MEM_SIZE, _blocksize):
                ...
                if byte_count < _blocksize:  # does block need padding?
                    radio.pipe.log(
                        "Padding partial block with \\xff "
                        "by 0x%02x bytes" % (_blocksize - len(data))
                    )
    ...
    def get_memory(self, number):
        ...
        if mem.number >= 1 and mem.number <= 30:
            ...
            if mem.number >= 8 and mem.number <= 14:
                ...
                mem.immutable = ["freq", "mode", "power", "duplex", "offset"]
