...
def _enter_programming(radio):
    ...
    if ack != CMD_ACK:
        raise errors.RadioError("Radio did not respond to program mode request")
    ...
    if not ident.startswith(IDENT_PREFIX):
        raise errors.RadioError("Unsupported radio (ident %s)" % ident.hex())
...
def _exit_programming(radio):
    try:
        ...
        raise errors.RadioError("Failed to exit programming mode")
...
def _read_page(radio, addr):
    ...
    try:
        ...
        raise errors.RadioError("Failed to read from radio at 0x%04X" % addr)

    if len(response) != 4 + PAGE_SIZE:
        raise errors.RadioError(
            "Short read at 0x%04X (%d bytes)" % (addr, len(response))
        )

    expected = struct.pack(">cHB", b"R", addr, PAGE_SIZE)
    if response[:4] != expected:
        raise errors.RadioError("Bad read response at 0x%04X" % addr)
...
def _write_page(radio, addr, data):
    ...
    try:
        ...
        raise errors.RadioError("Failed to write to radio at 0x%04X" % addr)

    if ack != CMD_ACK:
        raise errors.RadioError("No ACK after write at 0x%04X" % addr)
...
def do_upload(radio):
    ...
    _write_page(
        radio, SETTINGS_ADDR, raw[SETTINGS_OFFSET : SETTINGS_OFFSET + PAGE_SIZE]
    )
    status.cur = status.max
    radio.status_fn(status)
...
def _encode_tone(mode, value, polarity):
    ...
    if mode == "":
        ...
        return result
    raise errors.RadioError("Unsupported tone mode: %s" % mode)
...
class RadioddityDB40G(chirp_common.CloneModeRadio):
    ...
    def process_mmap(self):
        self._memobj = bitwise.parse(
            MEM_FORMAT % (NUM_CHANNELS, SETTINGS_OFFSET), self._mmap
        )
    ...
    def validate_memory(self, mem):
        ...
        if tx_freq:
            rpt = [f + 5000000 for f in bandplan_na.GMRS_HIRPT]
            all_tx = list(bandplan_na.ALL_GMRS_FREQS) + rpt
            if tx_freq not in all_tx:
                msgs.append(
                    chirp_common.ValidationWarning(
                        "TX frequency %s is not a "
                        "standard GMRS frequency" % chirp_common.format_freq(tx_freq)
                    )
                )

            if tx_freq in bandplan_na.GMRS_HHONLY:
                if mem.power and mem.power != POWER_LEVELS[0]:
                    msgs.append(
                        chirp_common.ValidationWarning(
                ...
                if mem.mode != "NFM":
                    msgs.append(
                        chirp_common.ValidationWarning(
                            "GMRS 467 MHz channels " "require narrowband"
                        )
                    )
    ...
    def get_settings(self):
        ...
        rs = RadioSettingValueList(["Off", "On"], current_index=int(_s.busylock))
        basic.append(MemSetting("settings.busylock", "Busy Lock", rs))

        rs = RadioSettingValueList(["Off", "On"], current_index=int(_s.keytone))
        basic.append(MemSetting("settings.keytone", "Key Tone", rs))

        tot_val = int(_s.tot)
        if tot_val >= len(TOT_OPTIONS):
            tot_val = 0
        rs = RadioSettingValueList(TOT_OPTIONS, current_index=tot_val)
        basic.append(MemSetting("settings.tot", "TX Timeout", rs))

        rs = RadioSettingValueInteger(0, 9, int(_s.squelch))
        basic.append(MemSetting("settings.squelch", "Squelch Level", rs))
