...
def start_programming(serial):
    try:
        ...
        if r != b"\x06":
            err = f"Enter Programming failed " f"expected 0x06 got {hex(r[0])}"
            LOG.error(err)
            raise errors.RadioError(err)
    except errors.RadioError:
        raise
...
def write_data(serial, addr, data):
    ...
    if r != b"\x06":
        err = f"Received bad ACK from Radio " f"expect 0x06 got {hex(r[0])}"
        LOG.error(err)
        raise errors.RadioError(err)
    ...
    try:
        ...
        if mode != int(radio._memobj.radio_info.mode):
            raise errors.RadioError(
                "The Radio Mode does not match" " [Settings]->[Radio Info]->Radio Mode"
            )
...
class RA25UVRadio(chirp_common.CloneModeRadio, chirp_common.ExperimentalRadio):
    ...
    VALID_DCS = [i for i in range(0, 778) if "9" not in str(i) and "8" not in str(i)]
    VALID_TUNING_STEPS = [2.5, 5.0, 6.25, 10, 12.5, 20, 25, 30, 50]
    VALID_CHARSET = "".join(chr(i) for i in range(32, 127) if chr(i) not in "\\`~")
    VALID_DTMF = [str(i) for i in range(0, 10)] + ["A", "B", "C", "D", "*", "#"]
    ...
    def get_prompts(cls):
        ...
        rp.experimental = "This driver is experimental, " "please report issues"
        return rp
    ...
    def get_memory(self, number):
        ...
        if 0 < number <= 500:
            ...
            mem.empty = not self.get_chan_status(_mem_chans_present, mem.number - 1)
            mem.skip = (
                "" if self.get_chan_status(_mem_chans_scanned, mem.number - 1) else "S"
            )
            _name = "".join(c for c in str(_mem.name) if c in self.VALID_CHARSET)
        ...
        rs = RadioSetting("busychannellockout", "Busy Channel Lockout", rstype)
        mem.extra.append(rs)
    ...
    def get_settings(self):
        ...
        rs = RadioSettingValueInteger(
            minval=0, maxval=9, current=_func_settings.squelch, step=1
        )
        rset = RadioSetting("func_settings.squelch", "Squelch Level", rs)
        function.append(rset)

        rs = RadioSettingValueInteger(
            minval=0, maxval=9, current=_func_settings.vox_level, step=1
        )
        ...
        rs = RadioSettingValueList(main_options, current_index=_func_settings.main)
        ...
        rs = RadioSettingValueInteger(
            minval=0, maxval=30, current=_func_settings.tot, step=1
        )
        rset = RadioSetting("func_settings.tot", "Time Out Timer (minutes)", rs)
        function.append(rset)

        tbst_options = ["Off", "1750", "2100", "1000", "1450"]
        rs = RadioSettingValueList(tbst_options, current_index=_func_settings.tbst_freq)
        rset = RadioSetting("func_settings.tbst_freq", "TBST Frequency (Hz)", rs)
        function.append(rset)
        ...
        rset = RadioSetting("func_settings.tail_elim_type", "Tail Eliminator Type", rs)
        function.append(rset)
        ...
        rset = RadioSetting("func_settings.dcs_tail_elim", "DCS Tail Elimination", rs)
        function.append(rset)
        ...
        rset = RadioSetting(
            "func_settings.sql_tail_elim", "Squelch Tail Elimination (Hz)", rs
        )
        function.append(rset)

        disp_options = ["Frequency", "Channel", "Name"]
        rs = RadioSettingValueList(disp_options, current_index=_func_settings.dispmode)
        ...
        _current = "".join(
            c for c in str(_startup_display.message) if c in self.VALID_CHARSET
        )
        ...
        rset = RadioSetting("startup_display.message", "Startup Display Message", rs)
        function.append(rset)
        ...
        rs = RadioSettingValueBoolean(current=_func_settings.beep, mem_vals=(0, 1))
        rset = RadioSetting("func_settings.beep", "Beep", rs)
        function.append(rset)

        rs = RadioSettingValueBoolean(current=_func_settings.alarm_off, mem_vals=(0, 1))
        ...
        rset = RadioSetting("func_settings.init_inhib", "Initialization Inhibit", rs)
        function.append(rset)

        lang_options = ["Simplified Chinese", "English", "Traditional Chinese"]
        rs = RadioSettingValueList(lang_options, current_index=_func_settings.language)
        ...
        rs = RadioSettingValueList(_options, current_index=_scan_menu.prio_chan)
        ...
        rs = RadioSettingValueList(_options, current_index=_scan_menu.revert_chan)
        rset = RadioSetting("scan_menu.revert_chan", "Revert Channel", rs)
        scan.append(rset)

        _options = [str(i / 10) for i in range(5, 51, 1)]
        rs = RadioSettingValueList(_options, current_index=_scan_menu.lookback_a)
        rset = RadioSetting("scan_menu.lookback_a", "Look Back Time A (s)", rs)
        scan.append(rset)

        # Uses previous options
        rs = RadioSettingValueList(_options, current_index=_scan_menu.lookback_b)
        rset = RadioSetting("scan_menu.lookback_b", "Look Back Time B (s)", rs)
        scan.append(rset)

        _options = [str(i / 10) for i in range(1, 51, 1)]
        rs = RadioSettingValueList(_options, current_index=_scan_menu.dropout_delay)
        rset = RadioSetting("scan_menu.dropout_delay", "Dropout Delay (s)", rs)
        scan.append(rset)

        # Uses previous options
        rs = RadioSettingValueList(_options, current_index=_scan_menu.dwell)
        ...
        rs = RadioSettingValueString(minlength=0, maxlength=3, current=_current)
        ...
        rset = RadioSetting("func_settings.dtmf_trans", "DTMF Transmit Time (ms)", rs)
        dtmf.append(rset)
        ...
        rs = RadioSettingValueList(_options, current_index=_dtmf_menu.decoding)
        ...
        rset = RadioSetting(
            "dtmf_menu.timelapse_enc", "Time Lapse After Encode (ms)", rs
        )
        dtmf.append(rset)

        _options = [str(i) for i in range(0, 2510, 10)]
        rs = RadioSettingValueList(_options, current_index=_dtmf_menu.first_dig_tm)
        rset = RadioSetting("dtmf_menu.first_dig_tm", "First Digit Time (ms)", rs)
        dtmf.append(rset)

        _options = [str(i / 10) for i in range(0, 251, 1)]
        rs = RadioSettingValueList(_options, current_index=_dtmf_menu.autoreset_tm)
        rset = RadioSetting("dtmf_menu.autoreset_tm", "Auto Reset Time (ms)", rs)
        dtmf.append(rset)

        rs = RadioSettingValueBoolean(current=_dtmf_menu.side_tone, mem_vals=(0, 1))
        ...
        rset = RadioSetting("dtmf_menu.pttid_pause", "PTT ID Pause Time (s)", rs)
        dtmf.append(rset)
        ...
        _warning = (
            "To Upload to the Radio this setting must match "
        ...
        _current = "".join(
            str(_radio_info.model[i]) for i in range(1, len(_radio_info.model))
        )
        rs = RadioSettingValueString(minlength=1, maxlength=8, current=_current)
        rs.set_mutable(False)
        rset = RadioSetting("radio_info.model", "Model", rs)
        info_menu.append(rset)

        _current = "".join(str(_radio_info.ver[i]) for i in range(len(_radio_info.ver)))
        rs = RadioSettingValueString(minlength=4, maxlength=4, current=_current)
        rs.set_mutable(False)
        rset = RadioSetting("radio_info.ver", "Version", rs)
        info_menu.append(rset)

        _current = "".join(
            str(_immut_1.serial_number[i]) for i in range(len(_immut_1.serial_number))
        )
        rs = RadioSettingValueString(minlength=0, maxlength=16, current=_current)
        rs.set_mutable(False)
        rset = RadioSetting("immut_1.serial_number", "Serial Number", rs)
        info_menu.append(rset)

        _current = "".join(
            str(_immut_2.prod_date[i])
            for i in range(len(_immut_2.prod_date))
            if int(_immut_2.prod_date[i]) >= 0x20
        )
        rs = RadioSettingValueString(minlength=0, maxlength=10, current=_current)
        rs.set_mutable(False)
