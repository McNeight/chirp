...
def _send_command(serport, data: bytes):
    ...
    serport.log(
        "Sending command (unobfuscated) len=0x%4.4x:\n%s"
    ...
    command = (
        struct.pack(">HBB", 0xABCD, len(data), 0)
        + xorarr(data2)
...
def _receive_reply(serport):
    ...
    if not header:
        ...
        LOG.warning(
            "Header short read: [%s] len=%i", util.hexprint(header), len(header)
        )
        raise errors.RadioError("Header short read")
    elif header[0] != 0xAB or header[1] != 0xCD or header[3] != 0x00:
        LOG.warning(
            "Bad response header: %s len=%i", util.hexprint(header), len(header)
        )
    ...
    if len(footer) != 4:
        LOG.warning(
            "Footer short read: [%s] len=%i", util.hexprint(footer), len(footer)
        )
        raise errors.RadioError("Footer short read")

    if footer[2] != 0xDC or footer[3] != 0xBA:
        LOG.debug(
            "Reply before bad response footer (obfuscated)" "len=0x%4.4x:\n%s",
            len(cmd),
            util.hexprint(cmd),
        )
        LOG.warning(
            "Bad response footer: %s len=%i", util.hexprint(footer), len(footer)
        )
...
def _sayhello(serport):
    ...
    if rep.startswith(b"\x18\x05"):
        raise errors.RadioError(
            "Radio is in programming mode, " "restart radio into normal mode"
        )
...
def _readmem(serport, offset, length):
    serport.log("Sending readmem offset=0x%4.4x len=0x%4.4x" % (offset, length))
...
def _writemem(serport, data, offset):
    ...
    if rep[0] == 0x1E and rep[4] == (offset & 0xFF) and rep[5] == (offset >> 8) & 0xFF:
        return True
...
def do_download(radio):
    ...
    if not radio.k5_approve_firmware(f) and radio.VARIANT != "unsupported":
        raise errors.RadioError("Firmware version is not supported by this driver")
...
def do_upload(radio):
    ...
    if not radio.k5_approve_firmware(f):
        raise errors.RadioError("Firmware version is not supported by this driver")
    LOG.info(
        "Uploading image from firmware %r to radio with %r",
        radio.metadata.get("uvk5_firmware", "unknown"),
        f,
    )
    addr = start_addr
...
class UVK5RadioBase(chirp_common.CloneModeRadio):
    ...
    def _check_firmware_at_load(self):
        ...
        if not firmware:
            ...
            raise errors.RadioError(
                "Image firmware is %r but is not supported by " "this driver" % firmware
            )
    ...
    def get_raw_memory(self, number):
        return "\n".join(
            [
                repr(self._memobj.channel[number - 1]),
    ...
    def _get_tone(self, mem, _mem):
        ...
        chirp_common.split_tone_decode(
            mem, (tx_tmode, tx_tone, tx_pol), (rx_tmode, rx_tone, rx_pol)
        )
    ...
    def _get_mem_extra(self, mem, _mem):
        ...
        rs = RadioSetting(
            "pttid",
            "PTTID",
            RadioSettingValueList(self._pttid_list, current_index=_mem.dtmf_pttid),
        )
        mem.extra.append(rs)
        ...
        rs = RadioSetting(
            "dtmfdecode", _("DTMF decode"), RadioSettingValueBoolean(is_dtmf)
        )
        mem.extra.append(rs)
        ...
        rs = RadioSetting(
            "scrambler",
            _("Scrambler"),
            RadioSettingValueList(SCRAMBLER_LIST, current_index=enc),
        )
        mem.extra.append(rs)

        rs = RadioSetting(
            "scanlists", _("Scanlists"), RadioSettingValueList(SCANLIST_LIST, tmpscn)
        )
        mem.extra.append(rs)
    ...
    def set_settings(self, settings):
        ...
        for element in settings:
            ...
            if element.get_name() == "backlight_auto_mode":
                _mem.backlight_auto_mode = self._backlight.index(str(element.value))
            ...
            if element.get_name() == "dtmf_decode_response":
                _mem.dtmf_settings.decode_response = DTMF_DECODE_RESPONSE_LIST.index(
                    str(element.value)
                )

            if element.get_name() == "dtmf_auto_reset_time":
                _mem.dtmf_settings.auto_reset_time = int(int(element.value) / 10)
            ...
            if element.get_name() == "dtmf_first_code_persist_time":
                _mem.dtmf_settings.first_code_persist_time = int(
            ...
            if element.get_name() == "dtmf_permit_remote_kill":
                _mem.dtmf_settings.permit_remote_kill = element.value and 1 or 0
            ...
            if element.get_name() == "scanlist1_priority_scan":
                _mem.scanlist1_priority_scan = element.value and 1 or 0

            if element.get_name() == "scanlist2_priority_scan":
                _mem.scanlist2_priority_scan = element.value and 1 or 0

            if (
                element.get_name() == "scanlist1_priority_ch1"
                or element.get_name() == "scanlist1_priority_ch2"
                or element.get_name() == "scanlist2_priority_ch1"
                or element.get_name() == "scanlist2_priority_ch2"
            ):
    ...
    def get_settings(self):
        ...
        top = RadioSettings(basic, keya, dtmf, dtmfc, scanl, unlock, fmradio, roinfo)
        ...
        rs = RadioSetting(
            ...
            RadioSettingValueList(KEYACTIONS_LIST, current_index=tmpval),
        )
        keya.append(rs)
        ...
        rs = RadioSetting(
            "dtmf_side_tone", "DTMF Sidetone", RadioSettingValueBoolean(tmppr)
        )
        dtmf.append(rs)
        ...
        rs = RadioSetting(
            ...
            "Decode Response",
            RadioSettingValueList(DTMF_DECODE_RESPONSE_LIST, current_index=tmpval),
        )
        dtmf.append(rs)
        ...
        tmpval = (
            str(_mem.dtmf_settings_numbers.dtmf_local_code)
            .upper()
            .strip("\x00\xff\x20")
        )
        for i in tmpval:
        ...
        rs = RadioSetting("dtmf_dtmf_local_code", "Local code (3 chars 0-9 ABCD)", val)
        dtmf.append(rs)

        tmpval = (
            str(_mem.dtmf_settings_numbers.dtmf_up_code).upper().strip("\x00\xff\x20")
        )
        for i in tmpval:
        ...
        rs = RadioSetting("dtmf_dtmf_up_code", "Up code (1-16 chars 0-9 ABCD*#)", val)
        dtmf.append(rs)

        tmpval = (
            str(_mem.dtmf_settings_numbers.dtmf_down_code).upper().strip("\x00\xff\x20")
        )
        for i in tmpval:
        ...
        val.set_charset(DTMF_CHARS_UPDOWN)
        rs = RadioSetting(
            "dtmf_dtmf_down_code", "Down code (1-16 chars 0-9 ABCD*#)", val
        )
        dtmf.append(rs)

        tmpval = str(_mem.dtmf_settings_numbers.kill_code).upper().strip("\x00\xff\x20")
        for i in tmpval:
        ...
        rs = RadioSetting("dtmf_kill_code", "Kill code (5 chars 0-9 ABCD)", val)
        dtmf.append(rs)

        tmpval = (
            str(_mem.dtmf_settings_numbers.revive_code).upper().strip("\x00\xff\x20")
        )
        for i in tmpval:
            if i in DTMF_CHARS_KILL:
                continue
        ...
        val = RadioSettingValueString(5, 5, tmpval)
        val.set_charset(DTMF_CHARS_KILL)
        rs = RadioSetting("dtmf_revive_code", "Revive code (5 chars 0-9 ABCD)", val)
        dtmf.append(rs)
        ...
        for i in range(1, 17):
            ...
            dtmfc.append(rs)
            rs.set_doc(
                "DTMF Contacts are 3 codes (valid: 0-9 * # ABCD), " "or an empty string"
            )
        ...
        if tmptot > 10:
            tmptot = 10
        rs = RadioSetting(
            "tot", "Max talk time [min]", RadioSettingValueInteger(0, 10, tmptot)
        )
        basic.append(rs)
        ...
        rs = RadioSetting(
            "noaa_autoscan",
            "NOAA Autoscan",
            RadioSettingValueBoolean(bool(_mem.noaa_autoscan > 0)),
        )
        basic.append(rs)
        ...
        rs = RadioSetting(
            "vox_switch",
            "VOX enabled",
            RadioSettingValueBoolean(bool(_mem.vox_switch > 0)),
        )
        basic.append(rs)
        ...
        if tmpvox > 10:
            tmpvox = 10
        rs = RadioSetting(
            "vox_level", "VOX Level", RadioSettingValueInteger(1, 10, tmpvox)
        )
        basic.append(rs)
        ...
        if tmpmicgain > 4:
            tmpmicgain = 4
        rs = RadioSetting(
            "mic_gain", "Mic Gain", RadioSettingValueInteger(0, 4, tmpmicgain)
        )
        basic.append(rs)
        ...
        rs = RadioSetting(
            ...
            "Channel display mode",
            RadioSettingValueList(CHANNELDISP_LIST, current_index=tmpchdispmode),
        )
        basic.append(rs)
        ...
        rs = RadioSetting(
            ...
            RadioSettingValueList(CROSSBAND_LIST, current_index=tmpcross),
        )
        basic.append(rs)
        ...
        rs = RadioSetting(
            "battery_save",
            "Battery Save",
            RadioSettingValueList(BATSAVE_LIST, current_index=tmpbatsave),
        )
        basic.append(rs)
        ...
        rs = RadioSetting(
            "dualwatch",
            "Dual Watch",
            RadioSettingValueList(DUALWATCH_LIST, current_index=tmpdual),
        )
        basic.append(rs)
        ...
        rs = RadioSetting(
            "backlight_auto_mode",
            "Backlight auto mode",
            RadioSettingValueList(self._backlight, current_index=tmpback),
        )
        basic.append(rs)
        ...
        # VFO open
        rs = RadioSetting(
            "vfo_open", "VFO open", RadioSettingValueBoolean(bool(_mem.vfo_open > 0))
        )
        basic.append(rs)
        ...
        rs = RadioSetting(
            "scan_resume_mode",
            "Scan resume mode",
            RadioSettingValueList(SCANRESUME_LIST, current_index=tmpscanres),
        )
        basic.append(rs)

        # Keypad locked
        rs = RadioSetting(
            "key_lock", "Keypad lock", RadioSettingValueBoolean(bool(_mem.key_lock > 0))
        )
        basic.append(rs)
        ...
        rs = RadioSetting(
            "welcome_mode",
            "Power on display mode",
            RadioSettingValueList(WELCOME_LIST, current_index=tmpdispmode),
        )
        basic.append(rs)
        ...
        rs = RadioSetting(
            "keypad_tone",
            "Keypad tone",
            RadioSettingValueList(KEYPADTONE_LIST, current_index=tmpkeypadtone),
        )
        basic.append(rs)
        ...
        rs = RadioSetting(
            "language",
            "Language",
            RadioSettingValueList(self._langs, current_index=tmplanguage),
        )
        if self._langs:
            basic.append(rs)
        ...
        rs = RadioSetting(
            "alarm_mode",
            "Alarm mode",
            RadioSettingValueList(ALARMMODE_LIST, current_index=tmpalarmmode),
        )
        basic.append(rs)
        ...
        rs = RadioSetting(
            ...
            "Reminding of end of talk",
            RadioSettingValueList(REMENDOFTALK_LIST, current_index=tmpalarmmode),
        )
        basic.append(rs)
        ...
        for i in range(1, 21):
            ...
            if fmfreq < FMMIN or fmfreq > FMMAX:
                rs = RadioSetting(freqname, freqname, RadioSettingValueString(0, 5, ""))
            else:
                rs = RadioSetting(
                    freqname, freqname, RadioSettingValueString(0, 5, str(fmfreq))
                )
        ...
        if tmpflock >= len(FLOCK_LIST):
            tmpflock = 0
        rs = RadioSetting(
            "flock", "F-LOCK", RadioSettingValueList(FLOCK_LIST, current_index=tmpflock)
        )
        unlock.append(rs)
        ...
        rs = RadioSetting(
            "tx350",
            "350TX - unlock 350-400 MHz TX",
            RadioSettingValueBoolean(bool(_mem.lock.tx350 > 0)),
        )
        unlock.append(rs)
        ...
        rs = RadioSetting(
            "Killed",
            "KILLED Device was disabled (via DTMF)",
            RadioSettingValueBoolean(bool(_mem.lock.killed > 0)),
        )
        unlock.append(rs)
        ...
        rs = RadioSetting(
            "tx200",
            "200TX - unlock 174-350 MHz TX",
            RadioSettingValueBoolean(bool(_mem.lock.tx200 > 0)),
        )
        unlock.append(rs)
        ...
        rs = RadioSetting(
            "tx500",
            "500TX - unlock 500-600 MHz TX",
            RadioSettingValueBoolean(bool(_mem.lock.tx500 > 0)),
        )
        unlock.append(rs)
        ...
        rs = RadioSetting(
            "en350",
            "350EN - unlock 350-400 MHz RX",
            RadioSettingValueBoolean(bool(_mem.lock.en350 > 0)),
        )
        unlock.append(rs)
        ...
        rs = RadioSetting(
            "scrambler",
            "SCREN - scrambler enable",
            RadioSettingValueBoolean(bool(_mem.lock.enscramble > 0)),
        )
        unlock.append(rs)
    ...
    def set_memory(self, mem):
        ...
        if _mem.get_raw(asbytes=False)[0] == "\xff":
            ...
            _mem.set_raw(
                b"\x00" * 10
                + bytes([prev_0a, prev_0b, prev_0c, prev_0d, prev_0e, prev_0f])
            )
...
class UVK5RestrictedRadio(UVK5RadioBase):
    ...
    def process_mmap(self):
        firmware = self.metadata.get("uvk5_firmware", "<unknown>")
        LOG.warning(
            "Firmware %s is not supported by CHIRP. " "Image data will be read-only.",
            firmware,
        )
        self._memobj = bitwise.parse(MEM_FORMAT, self._mmap)
    ...
    def validate_memory(self, mem):
        return [
            chirp_common.ValidationError(
                _(
                    "This image is read-only due to being from a radio with "
