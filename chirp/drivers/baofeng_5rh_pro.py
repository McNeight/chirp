...
from chirp import bitwise, chirp_common, directory, errors, kenwood_tone, memmap
from chirp.settings import (
    RadioSetting,
    RadioSettingGroup,
    RadioSettings,
    RadioSettingValueBoolean,
    RadioSettingValueFile,
...
ADDR_CHAN_EMPTY = 0x7A20  # 80 bytes  (640 / 8)
...
ADDR_SCAN_SKIP = 0x81A0  # 80 bytes  (640 / 8)
...
def _bs_scale_nn(rows, src_w, src_h, dst_w, dst_h):
    """Nearest-neighbor scale a rows grid to (dst_w, dst_h)."""
    out = []
...
def _bs_image_to_rgb565(img_path):
    ...
    if ext == ".bmp":
        ...
        if not _PIL_AVAILABLE:
            raise errors.RadioError(
                "Boot screen: unsupported format — only .bmp files are supported."
            )
        img = _PILImage.open(img_path).convert("RGB")
        if img.width != _BS_WIDTH or img.height != _BS_HEIGHT:
            img = img.resize((_BS_WIDTH, _BS_HEIGHT), _PILImage.Resampling.LANCZOS)
...
def _xread(pipe, n, timeout=2.0):
    ...
    while len(buf) < n and time.time() < deadline:
        ...
        if chunk:
            buf += chunk
    if len(buf) < n:
        raise errors.RadioError("Timeout: got %d of %d bytes" % (len(buf), n))
    return buf
...
def _wait_ack(pipe, seed, label, timeout=2.0):
    ...
    if decoded != _ACK:
        LOG.debug(
            "%s: raw=0x%02x seed=0x%02x decoded=0x%02x "
        ...
        raise errors.RadioError(
            "%s: expected ACK 0x41, got 0x%02x (raw 0x%02x)" % (label, decoded, b[0])
        )
...
class BaofengUV5RMPlusGPS(chirp_common.CloneModeRadio):
    ...
    # RX adds: 68-108 MHz Broadcast FM, 108-136 MHz airband AM, 350-390 MHz,
    #          and UHF up to 520 MHz
    VALID_BANDS = [
        (68_000_000, 108_000_000),  # FM Broadcast (RX only)
    ...
    def get_features(self):
        ...
        rf.valid_tones = chirp_common.TONES
    ...
    def sync_out(self):
        if self._boot_image_data:
            try:
                ...
                raise errors.RadioError(f"Boot screen upload failed: {e}")
            finally:
                try:
    ...
    def get_memory(self, number):
        ...
        if _mem.txfreq.get_raw() == b"\xff\xff\xff\xff":
            ...
            if tx_freq == mem.freq:
                ...
                if chirp_common.is_split(
                    self.get_features().valid_bands, mem.freq, tx_freq
                ):
                    mem.duplex = "split"
                    mem.offset = tx_freq
                elif diff > 0:
        ...
        # Skip flag
        scan_flag = self._memobj.scan_skip.bitfield[_index]
        mem.skip = "" if (scan_flag) == 0 else "S"
        ...
        _exa(
            ...
            RadioSettingValueList(LIST_BCL, current_index=min(int(_mem.bcl), 2)),
        )
        _exa("txdis", "TX Disable", RadioSettingValueBoolean(bool(_mem.txdis)))
        _exa(
            "talkaround", "Talk Around", RadioSettingValueBoolean(bool(_mem.talkaround))
        )
        _exa("compand", "Compander", RadioSettingValueBoolean(bool(_mem.compand)))
        _exa("scram", "Scramble", RadioSettingValueBoolean(bool(_mem.scram)))
        _exa(
            "cepin24bit",
            "Freq. Measuring 24-bit",
        ...
        _exa(
            ...
            "Signal Type",
            RadioSettingValueList(
                LIST_SIGNAL, current_index=min(int(_mem.signaltype), 4)
            ),
        )
        _exa(
            "sqtype",
            "Squelch Type",
            RadioSettingValueList(LIST_SQTYPE, current_index=min(int(_mem.sqtype), 3)),
        )
        _exa(
            "fivetoneptt",
            "5-Tone PTT ID",
            RadioSettingValueList(
                LIST_PTTID, current_index=min(int(_mem.fivetoneptt), 3)
            ),
        )
        _exa(
            "dtmfptt",
            "DTMF PTT ID",
            RadioSettingValueList(LIST_PTTID, current_index=min(int(_mem.dtmfptt), 3)),
        )
        _exa(
            "jumpfreq",
            "Jump Frequency",
        ...
        _exa(
            "dtmfidx", "DTMF Index", RadioSettingValueInteger(0, 15, int(_mem.dtmfidx))
        )
        _exa(
            "twotoneidx",
            "2-Tone Index",
        ...
        _exa("mdcidx", "MDC Index", RadioSettingValueInteger(0, 7, int(_mem.mdcidx)))
        _exa(
            "scanlist", "Scan List", RadioSettingValueInteger(0, 9, int(_mem.scanlist))
        )
        _exa(
            "emerglist",
            "Emergency List",
    ...
    def set_memory(self, mem):
        _index = mem.number - 1
        if mem.empty:
            self._memobj.memory[_index].set_raw(b"\x00" * _CHN_SIZE)
        ...
        if mem.skip == "S":
            ...
            scan_flag = False
        self._memobj.scan_skip.bitfield[_index] = scan_flag
    ...
    def get_settings(self):
        ...
        for _zi in range(10):
            ...
            _zone_labels.append(
                f"Zone {_zi + 1}: {_zname}" if _zname else f"Zone {_zi + 1}"
            )
        ...
        _add(
            ...
            "settings.chanum",
            "Channel A Number",
            RadioSettingValueInteger(1, _MAX_CHN, max(1, min(cha_num, _MAX_CHN))),
        )
        _add(
            basic,
            "settings.chbnum",
            "Channel B Number",
            RadioSettingValueInteger(1, _MAX_CHN, max(1, min(chb_num, _MAX_CHN))),
        )

        _add(
            basic,
            "settings.chazone",
            "Channel A Active Zone",
            RadioSettingValueList(_zone_labels, current_index=min(s[6], 9)),
        )
        _add(
            basic,
            "settings.chbzone",
            "Channel B Active Zone",
            RadioSettingValueList(_zone_labels, current_index=min(s[7], 9)),
        )
        ...
        _add(
            ...
            "settings.backlight",
            "Backlight Time",
            RadioSettingValueList(LIST_BACKLIGHT, current_index=blight_idx),
        )
        _add(
            basic,
            "settings.blightlv",
            "Backlight Level",
            RadioSettingValueList(
                LIST_BLIGHT_LV, current_index=max(0, min(s[9] - 1, 4))
            ),
        )
        ...
        _add(
            ...
            "settings.mainband",
            "Main Band",
            RadioSettingValueList(LIST_MAIN_BAND, current_index=min(s[12], 1)),
        )
        _add(
            basic,
            "settings.squelch",
        ...
        _add(
            ...
            "settings.posavedly",
            "Power Save Delay",
            RadioSettingValueList(LIST_POSAVE_DLY, current_index=min(s[17], 4)),
        )
        ...
        _add(
            basic,
            "settings.loneworktim",
            "Lone Worker Timer (min)",
            RadioSettingValueInteger(0, 99, s[18]),
        )
        _add(
            basic,
            "settings.loneworkrsp",
            "Lone Worker Response",
            RadioSettingValueList(LIST_LONEWORK_RSP, current_index=min(s[19], 3)),
        )
        ...
        _add(
            ...
            "settings.voice",
            "Voice Prompt",
            RadioSettingValueList(LIST_VOICE, current_index=min((b32 >> 2) & 0x03, 2)),
        )
        _add(
            basic,
            "settings.busylockglobal",
            "Busy Channel Lockout",
        ...
        _add(
            ...
            "End Tone",
            RadioSettingValueList(
                LIST_END_TONE, current_index=min((b34 >> 5) & 0x03, 3)
            ),
        )
        ...
        _add(
            ...
            "Record Mode",
            RadioSettingValueList(
                LIST_RECORD_MODE, current_index=min((b37 >> 5) & 0x03, 2)
            ),
        )
        _add(
            basic,
        ...
        _add(
            ...
            "settings.langsel",
            "Language",
            RadioSettingValueList(["English", "Chinese"], current_index=(b37 >> 2) & 1),
        )
        _add(
            basic,
            "settings.pownface",
            "Power-On Screen",
            RadioSettingValueList(LIST_POWON_FACE, current_index=min(b37 & 0x03, 2)),
        )
        ...
        _add(
            ...
            "Tail Tone Freq",
            RadioSettingValueList(
                LIST_TAIL_FREQ, current_index=min((b38 >> 5) & 0x07, 4)
            ),
        )
        _add(
            basic,
        ...
        # Passwords
        pow_pwd = bytes(b for b in s[64:72] if b not in (0, 0xFF)).decode(
            "ascii", errors="replace"
        )
        _add(
            basic,
            "settings.powpassword",
            "Power-On Password",
            RadioSettingValueString(0, 8, pow_pwd, False, chirp_common.CHARSET_ASCII),
        )
        wr_pwd = bytes(b for b in s[72:80] if b not in (0, 0xFF)).decode(
            "ascii", errors="replace"
        )
        _add(
            basic,
            "settings.wrpassword",
            "Write Password",
            RadioSettingValueString(0, 8, wr_pwd, False, chirp_common.CHARSET_ASCII),
        )
        ...
        _add(
            display,
            "settings.chadispmode",
            "Channel A Display Mode",
            RadioSettingValueList(
                LIST_DISP_MODE, current_index=min((b10 >> 4) & 0x0F, 3)
            ),
        )
        _add(
            display,
            "settings.chbdispmode",
            "Channel B Display Mode",
            RadioSettingValueList(LIST_DISP_MODE, current_index=min(b10 & 0x0F, 3)),
        )
        ...
        for zi in range(10):
            ...
            entry_str = (
                "%s: %s" % (zname, ", ".join(chn_ids)) if (zname or chn_ids) else ""
            )
            _add(
                zones,
                "zones.zone%d" % zi,
                "Zone %d" % (zi + 1),
                RadioSettingValueString(
                    0, 270, entry_str, False, chirp_common.CHARSET_ASCII + ":, "
                ),
            )
            ...
            for _cid_str in chn_ids:
                try:
                    _cid = int(_cid_str)
                    if self._memobj.chan_empty.bitfield[_cid - 1] == 0:
                        _cname = _decode_name(
                            self._memobj.memory[_cid - 1].name.get_raw()
                        ).strip()
                        _preview_parts.append(
                            "%d:%s" % (_cid, _cname) if _cname else _cid_str
                        )
                    else:
                        _preview_parts.append(_cid_str)
                except Exception:
                    _preview_parts.append(_cid_str)
            _preview = ", ".join(_preview_parts) if _preview_parts else "(empty)"
            _add(
                zones,
                "zones.zone%d_preview" % zi,
                "Zone %d — channel list (read-only)" % (zi + 1),
                RadioSettingValueString(
                    0, 500, _preview, False, chirp_common.CHARSET_ASCII + ",:"
                ),
            )
        ...
        _add(
            ...
            "scan.scanmode",
            "Scan Mode",
            RadioSettingValueList(LIST_SCAN_MODE, current_index=min(sp[0], 2)),
        )
        _add(
            scan,
            "scan.backscantim",
            "Back Scan Time (s)",
            RadioSettingValueInteger(0, 250, max(0, sp[1] - 5) if sp[1] > 5 else 0),
        )
        _add(
            scan,
            "scan.rxresume",
            "RX Resume (s)",
            RadioSettingValueInteger(0, 254, max(0, sp[2] - 1) if sp[2] > 1 else 0),
        )
        _add(
            scan,
            "scan.txresume",
            "TX Resume (s)",
            RadioSettingValueInteger(0, 254, max(0, sp[3] - 1) if sp[3] > 1 else 0),
        )
        _add(
            scan,
            "scan.rtnchtype",
        ...
        _add(
            ...
            "scan.priochannel",
            "Priority Channel",
            RadioSettingValueInteger(1, _MAX_CHN, max(1, min(prio_ch, _MAX_CHN))),
        )
        _add(
            scan,
            "scan.scanrange",
        ...
        _add(dtmf, "dtmf.dtmfsw", "DTMF Switch", RadioSettingValueBoolean(bool(dp[0])))
        _add(
            dtmf,
            "dtmf.codespeed",
            "Code Speed",
        ...
        _add(dtmf, "dtmf.dtmftone", "DTMF Tone", RadioSettingValueBoolean(bool(dp[6])))
        _add(
            dtmf,
            "dtmf.resettime",
            "Reset Time",
            RadioSettingValueInteger(0, 50, max(0, dp[7] - 10) if dp[7] >= 10 else 0),
        )
        ...
        _add(
            ...
            "BOT Code",
            RadioSettingValueString(
                0, 16, _decode_dtmf_bytes(dp[24:40]), False, chirp_common.CHARSET_ASCII
            ),
        )
        _add(
            dtmf,
            ...
            "EOT Code",
            RadioSettingValueString(
                0, 16, _decode_dtmf_bytes(dp[40:56]), False, chirp_common.CHARSET_ASCII
            ),
        )
        _add(
            dtmf,
            ...
            "Stun Code",
            RadioSettingValueString(
                0, 16, _decode_dtmf_bytes(dp[56:72]), False, chirp_common.CHARSET_ASCII
            ),
        )
        _add(
            dtmf,
            ...
            "Kill Code",
            RadioSettingValueString(
                0, 16, _decode_dtmf_bytes(dp[72:88]), False, chirp_common.CHARSET_ASCII
            ),
        )
        ...
        for di in range(16):
            ...
            enc_str = _decode_dtmf_bytes(enc_data) if (use_flg >> di) & 1 else ""
            _add(
                dtmf,
                "dtmf_enc.%d" % di,
                "DTMF Encode %d" % (di + 1),
                RadioSettingValueString(
                    0, 16, enc_str, False, chirp_common.CHARSET_ASCII
                ),
            )
            _add(
                dtmf,
        ...
        for ti in range(16):
            ...
            tname = tname_raw.split(b"\x00")[0].decode("ascii", errors="replace")
            ...
            _add(
                twotone,
                "twotone_enc.%d_name" % ti,
                "2-Tone %d Name" % (ti + 1),
                RadioSettingValueString(
                    0, 12, tname[:12], False, chirp_common.CHARSET_ASCII
                ),
            )
        ...
        _add(
            ...
            "twotone.decodersp",
            "Decode Response",
            RadioSettingValueList(LIST_2TONE_DEC_RSP, current_index=min(td[0], 2)),
        )
        _add(
            twotone,
            "twotone.resetim",
            "Reset Time",
            RadioSettingValueInteger(0, 50, max(0, td[1] - 10) if td[1] >= 10 else 0),
        )
        _add(
            twotone,
            "twotone.decformat",
        ...
        for fi in range(100):
            ...
            _add(
                fivetone,
                "fivetone_enc.%d_stand" % fi,
                "5-Tone %d Standard" % (fi + 1),
                RadioSettingValueList(LIST_5TONE_STAND, current_index=stand),
            )
            _add(
                fivetone,
                "fivetone_enc.%d_codetime" % fi,
                "5-Tone %d Code Time" % (fi + 1),
                RadioSettingValueList(LIST_5TONE_CODETIME, current_index=ct_idx),
            )
            _add(
                fivetone,
                "fivetone_enc.%d_codelen" % fi,
                "5-Tone %d Code Length" % (fi + 1),
                RadioSettingValueInteger(0, 24, code_len),
            )
            _add(
                fivetone,
                "fivetone_enc.%d_id" % fi,
                "5-Tone %d Code IDs" % (fi + 1),
                RadioSettingValueString(
                    0, 40, id_str, False, chirp_common.CHARSET_ASCII
                ),
            )
            _add(
                fivetone,
                "fivetone_enc.%d_scall" % fi,
                "5-Tone %d Special Call" % (fi + 1),
                RadioSettingValueList(LIST_5TONE_SCALL, current_index=scall),
            )
            _add(
                fivetone,
                "fivetone_enc.%d_name" % fi,
                "5-Tone %d Name" % (fi + 1),
                RadioSettingValueString(
                    0, 8, fname[:8], False, chirp_common.CHARSET_ASCII
                ),
            )
            _add(
                fivetone,
                "fivetone_enc.%d_en" % fi,
        ...
        _add(
            ...
            "fivetone.decstand",
            "Decode Standard",
            RadioSettingValueList(LIST_5TONE_STAND, current_index=min(fd[1], 13)),
        )
        _add(
            fivetone,
            "fivetone.dectim",
        ...
        _add(
            ...
            "fivetone.resettime",
            "Reset Time",
            RadioSettingValueInteger(0, 50, max(0, fd[14] - 10) if fd[14] >= 10 else 0),
        )
        _add(
            fivetone, "fivetone.fiveani", "ANI", RadioSettingValueBoolean(bool(fd[16]))
        )
        ...
        for pi in range(4):
            ...
            _add(
                fivetone,
                "fivetone_pttid.%d_len" % pi,
                "5-Tone PTT ID %d Length" % (pi + 1),
                RadioSettingValueInteger(0, 12, plen),
            )
            _add(
                fivetone,
                "fivetone_pttid.%d_id" % pi,
                "5-Tone PTT ID %d Code" % (pi + 1),
                RadioSettingValueString(
                    0, 16, pid_str, False, chirp_common.CHARSET_ASCII
                ),
            )
        ...
        for ic in range(8):
            ...
            ic_id = (
                bytes(icp[1:6]).split(b"\x00")[0].decode("ascii", errors="replace")[:5]
            )
            ic_func = min(icp[6], 4)
            ic_rsp = min(icp[7], 2)
            ic_name = (
                bytes(icp[8:14]).split(b"\x00")[0].decode("ascii", errors="replace")[:6]
            )
            _add(
                fivetone,
                "fivetone_info.%d_id" % ic,
                "5-Tone Info %d Code ID" % (ic + 1),
                RadioSettingValueString(0, 5, ic_id, False, "0123456789"),
            )
            _add(
                fivetone,
                "fivetone_info.%d_func" % ic,
                "5-Tone Info %d Function" % (ic + 1),
                RadioSettingValueList(LIST_5TONE_INFO_FUNC, current_index=ic_func),
            )
            _add(
                fivetone,
                "fivetone_info.%d_rsp" % ic,
                "5-Tone Info %d Response" % (ic + 1),
                RadioSettingValueList(LIST_5TONE_INFO_RSP, current_index=ic_rsp),
            )
            _add(
                fivetone,
                "fivetone_info.%d_name" % ic,
                "5-Tone Info %d Name" % (ic + 1),
                RadioSettingValueString(
                    0, 6, ic_name, False, chirp_common.CHARSET_ASCII
                ),
            )
        ...
        for mi in range(5):
            ...
            _add(
                mdc,
                "mdc_para.%d_dectone" % mi,
                "MDC Sys %d Dec Tone" % (mi + 1),
                RadioSettingValueBoolean(bool(dec_tone)),
            )
            _add(
                mdc,
                "mdc_para.%d_encsync" % mi,
                "MDC Sys %d Enc Sync" % (mi + 1),
                RadioSettingValueList(LIST_MDC_SYNC, current_index=min(encsync, 3)),
            )
            _add(
                mdc,
                "mdc_para.%d_decsync" % mi,
                "MDC Sys %d Dec Sync" % (mi + 1),
                RadioSettingValueList(LIST_MDC_SYNC, current_index=min(decsync, 3)),
            )
            _add(
                mdc,
                "mdc_para.%d_encid" % mi,
        ...
        for mi in range(100):
            ...
            _add(
                ...
                "MDC Dec %d ID" % (mi + 1),
                RadioSettingValueString(
                    0, 4, "%04d" % dec_id, False, chirp_common.CHARSET_ASCII
                ),
            )
            _add(
                mdc,
        ...
        for ei in range(10):
            ...
            _add(
                ...
                "emerg.%d_mode" % ei,
                "Emerg %d Mode" % (ei + 1),
                RadioSettingValueList(LIST_EMERG_MODE, current_index=min(ep[6], 3)),
            )
            _add(
                emerg,
                "emerg.%d_type" % ei,
                "Emerg %d Type" % (ei + 1),
                RadioSettingValueList(LIST_EMERG_TYPE, current_index=min(ep[7], 2)),
            )
            _add(
                emerg,
                "emerg.%d_chn" % ei,
                "Emerg %d Channel" % (ei + 1),
                RadioSettingValueInteger(1, _MAX_CHN, max(1, min(ep[14], _MAX_CHN))),
            )
            _add(
                emerg,
                "emerg.%d_zone" % ei,
        ...
        desno = bytes(ap[0:6]).rstrip(b"\xff\x00\x20").decode("ascii", errors="replace")
        srcno = (
            bytes(ap[8:14]).rstrip(b"\xff\x00\x20").decode("ascii", errors="replace")
        )
        _add(
            aprs,
            "aprs.desno",
            "Destination Callsign",
            RadioSettingValueString(0, 6, desno[:6], False, chirp_common.CHARSET_ASCII),
        )
        _add(
            aprs,
            "aprs.desid",
        ...
        _add(
            ...
            "aprs.srcno",
            "Source Callsign",
            RadioSettingValueString(0, 6, srcno[:6], False, chirp_common.CHARSET_ASCII),
        )
        _add(
            aprs,
            "aprs.srcid",
        ...
        _add(
            ...
            "TX Power",
            RadioSettingValueList(
                LIST_APRS_POWER, current_index=min((b15 >> 4) & 0x03, 2)
            ),
        )
        _add(aprs, "aprs.band", "Band", RadioSettingValueBoolean(bool((b15 >> 3) & 1)))
        _add(
            aprs,
            "aprs.beeptone",
            "Beep Tone",
        ...
        _add(
            aprs,
            "aprs.longdir",
            "Longitude Direction (1=West)",
            RadioSettingValueBoolean(bool((b15 >> 1) & 1)),
        )
        _add(
            aprs,
            "aprs.latdir",
            "Latitude Direction (1=South)",
            RadioSettingValueBoolean(bool(b15 & 1)),
        )

        _add(aprs, "aprs.pretime", "Pre Time", RadioSettingValueInteger(0, 255, ap[16]))
        _add(
            aprs, "aprs.codedly", "Code Delay", RadioSettingValueInteger(0, 255, ap[17])
        )
        ctdcs_mode, ctdcs_val, ctdcs_pol = _decode_tone(ap[18], ap[19])
        _add(
            aprs,
            "aprs.ctdcs",
            "RX CTCSS/DCS Filter (Off / Hz / DXXXN)",
            RadioSettingValueString(
                0,
                10,
                kenwood_tone.format_qtdqt(ctdcs_mode, ctdcs_val, ctdcs_pol),
                False,
                chirp_common.CHARSET_ASCII,
            ),
        )
        ...
        _add(
            ...
            "aprs.pttid",
            "PTT ID",
            RadioSettingValueList(LIST_PTTID, current_index=min((b92 >> 4) & 0x03, 3)),
        )
        _add(
            aprs,
            "aprs.encodetype",
        ...
        _add(
            ...
            "aprs.micetype",
            "Mic-E Type",
            RadioSettingValueList(LIST_MICETYPE, current_index=min(b92 & 0x07, 7)),
        )
        ...
        aprs_txt = "".join(
            c
            for c in bytes(ap[108 : 108 + txtlen]).decode("ascii", errors="replace")
            if c in _valid_aprs
        )
        _add(
            aprs,
            ...
            "APRS Text (max 60)",
            RadioSettingValueString(
                0, 60, aprs_txt[:60], False, chirp_common.CHARSET_ASCII + " .,!?-+/"
            ),
        )
        ...
        _add(
            ...
            "Position Table",
            RadioSettingValueString(
                0, 1, posicon_tbl, False, chirp_common.CHARSET_ASCII
            ),
        )
        _add(
            aprs,
            ...
            "Position Icon",
            RadioSettingValueString(
                0, 1, posicon_sym, False, chirp_common.CHARSET_ASCII
            ),
        )
        ...
        _add(
            ...
            "aprs.lon",
            "Longitude (×100000)",
            RadioSettingValueInteger(-18000000, 18000000, int(aprs_lon * 100000)),
        )
        _add(
            aprs,
            "aprs.lat",
            "Latitude (×100000)",
            RadioSettingValueInteger(-9000000, 9000000, int(aprs_lat * 100000)),
        )
        _add(
            aprs,
            "aprs.height",
        ...
        for ai in range(8):
            base_a = 24 + ai * 8
            cs = (
                bytes(ap[base_a : base_a + 6])
                .rstrip(b"\xff\x00\x20")
                .decode("ascii", errors="replace")
            )
            ...
            _add(
                ...
                "TX Callsign %d" % (ai + 1),
                RadioSettingValueString(
                    0, 6, cs[:6], False, chirp_common.CHARSET_ASCII
                ),
            )
            _add(
                aprs,
        ...
        for ai in range(32):
            ...
            _add(
                ...
                "RX Callsign %d" % (ai + 1),
                RadioSettingValueString(
                    0, 6, rcs[:6], False, chirp_common.CHARSET_ASCII
                ),
            )
            _add(
                aprs,
        ...
        for ai in range(8):
            ...
            _add(
                ...
                RadioSettingValueString(0, 10, freq_mhz, False, "0123456789."),
            )
        ...
        _add(
            ...
            "GPS Mode",
            RadioSettingValueList(
                LIST_GPS_MODE, current_index=min((b35 >> 5) & 0x03, 2)
            ),
        )
        _add(
            gps,
        ...
        for gi in range(80):
            ...
            _add(
                ...
                "GPS Contact %d" % (gi + 1),
                RadioSettingValueString(
                    0, 12, entry_str, False, chirp_common.CHARSET_ASCII + ":"
                ),
            )
        ...
        if _PIL_AVAILABLE:
            ...
            label = "Boot screen image — auto-scaled to 160×128 (.bmp, .png, .jpg)"
        else:
            wildcard = "BMP files (*.bmp)|*.bmp|All files (*.*)|*.*"
            label = "Boot screen image — auto-scaled to 160×128 (.bmp only)"

        bval = RadioSettingValueFile(current=self._boot_image_path, wildcard=wildcard)
        ...
        def _apply_boot_image(setting, _obj):
            ...
            if path:
                if not os.path.isfile(path):
                    raise errors.RadioError("Boot screen image not found: %s" % path)
        ...
        _add(
            ...
            "Model",
            RadioSettingValueString(
                0, 32, self._model, False, chirp_common.CHARSET_ASCII
            ),
        )
        _add(
            devinfo,
            "devinfo.swver",
            "SW Version",
            RadioSettingValueString(0, 16, dev_sw, False, chirp_common.CHARSET_ASCII),
        )
        _add(
            devinfo,
            "devinfo.hwver",
            "HW Version",
            RadioSettingValueString(0, 16, dev_hw, False, chirp_common.CHARSET_ASCII),
        )
        _add(
            devinfo,
            "devinfo.date",
            "Production Date",
            RadioSettingValueString(0, 32, dev_date, False, chirp_common.CHARSET_ASCII),
        )
    ...
    def _add_vfo_settings(self, group, vfo_idx):
        ...
        tx_tone_str = kenwood_tone.format_qtdqt(tx_mode, tx_val, tx_pol)
        rx_tone_str = kenwood_tone.format_qtdqt(rx_mode, rx_val, rx_pol)
        _add(
            "%s.txtone" % prefix,
            "%s TX Tone" % label,
            RadioSettingValueString(
                0, 10, tx_tone_str, False, chirp_common.CHARSET_ASCII
            ),
        )
        _add(
            "%s.rxtone" % prefix,
            "%s RX Tone" % label,
            RadioSettingValueString(
                0, 10, rx_tone_str, False, chirp_common.CHARSET_ASCII
            ),
        )
        ...
        _add(
            "%s.power" % prefix,
            "%s Power" % label,
            RadioSettingValueList(LIST_POWER, current_index=min(power_idx, 2)),
        )
        is_narrow = bool((f16 >> 4) & 0x03)
        _add(
            "%s.bandwidth" % prefix,
            "%s Bandwidth" % label,
            RadioSettingValueList(LIST_BANDWIDTH, current_index=1 if is_narrow else 0),
        )
        offsetdir = (f16 >> 2) & 0x03
        _add(
            "%s.offsetdir" % prefix,
            "%s Offset Dir" % label,
            RadioSettingValueList(LIST_OFFSETDIR, current_index=min(offsetdir, 3)),
        )
    ...
    def _apply_setting(self, rs):  # noqa: C901
        ...
        if name == "settings.chaworkmode":
            ...
            enc = str(val).encode("ascii", errors="replace")[:8].ljust(8, b"\xff")
        ...
        # ── VFO A / B ────────────────────────────────────────────────────────
            ...
            if field == "rxfreq":
                ...
                m, v, p = kenwood_tone.parse_qtdqt(str(val))
                ...
                _mwb(self._mmap, base + 16, (f16 & 0x3F) | ((int(val) & 0x03) << 6))
            elif field == "bandwidth":
                f16 = _mb(self._mmap, base + 16)
                _mwb(self._mmap, base + 16, (f16 & 0xCF) | (0x10 if int(val) else 0))
            elif field == "offsetdir":
                f16 = _mb(self._mmap, base + 16)
                _mwb(self._mmap, base + 16, (f16 & 0xF3) | ((int(val) & 0x03) << 2))
            elif field == "step":
        ...
        # ── Zones ────────────────────────────────────────────────────────────
            ...
            if suffix.isdigit():
                ...
                if raw and ":" in raw:
                    ...
                    if len(ids) > 64:
                        LOG.warning(
                            "5RH: zone %d: %d channels specified, truncating to 64",
                            zi + 1,
                            len(ids),
                        )
                        ids = ids[:64]
                ...
                cnt = sum(
                    1 for _z in range(10) if _mb(self._mmap, ADDR_ZONES + _z * 152) > 0
                )
        ...
        # ── Scan ─────────────────────────────────────────────────────────────
            ...
            _mwrite(self._mmap, ADDR_SCAN_FREQ + si * 8, _encode_freq_le(int(val)))
        elif name.startswith("scan.freq") and "_dw" in name:
            si = int(name.split("scan.freq")[1].split("_")[0])
            _mwrite(self._mmap, ADDR_SCAN_FREQ + si * 8 + 4, _encode_freq_le(int(val)))
        ...
        # ── DTMF ─────────────────────────────────────────────────────────────
            ...
            _mwrite(self._mmap, ADDR_DTMF + 16, _encode_dtmf_bytes(str(val), 3))
        elif name == "dtmf.bot":
            ...
            if name.startswith("dtmf_enc_en."):
                ...
                _mwrite(
                    self._mmap, ADDR_DTMF_ENC + di * 16, _encode_dtmf_bytes(str(val))
                )
        ...
        # ── MDC-1200 ─────────────────────────────────────────────────────────
            ...
            if field == "ctrl":
                ...
                _mwb(self._mmap, base, (b0 & 0x7F) | (0x80 if bool(val) else 0))
            elif field == "dectone":
                b0 = _mb(self._mmap, base)
                _mwb(self._mmap, base, (b0 & 0xBF) | (0x40 if bool(val) else 0))
            elif field == "encsync":
                b0 = _mb(self._mmap, base)
        ...
        # ── APRS ─────────────────────────────────────────────────────────────
            ...
            if field == "desno":
            ...
            elif field in ("beacon", "heighttype", "pttid", "encodetype", "micetype"):
                ...
                try:
                    ...
                    freq_hz = 0
                _mwrite(self._mmap, base + 168 + ai * 4, _encode_freq_le(freq_hz))
            elif field == "rxcscount":
                _mwb(self._mmap, base + 22, int(val))
            elif field.startswith("rxcsfilt"):
                ai = int(field[8:])
                _mwb(self._mmap, base + 200 + ai * 8 + 7, 1 if bool(val) else 0)
