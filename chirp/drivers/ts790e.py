...
class TS790ERadio(KenwoodLiveRadio):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.pipe:
            ...
            if RADIO_IDS[radio_id] != self.MODEL.split(" ")[0]:
    ...
    def get_memory(self, number):
        ...
        if number < 0 or number > (self._upper + len(TS790E_SPECIALS)):
            raise errors.InvalidMemoryLocation(
                "Number must be between 0 and %i" % (self._upper + len(TS790E_SPECIALS))
            )
    ...
    def set_memory(self, memory):
        ...
        if real_number < 0 or real_number > (self._upper + len(TS790E_SPECIALS)):
            raise errors.InvalidMemoryLocation(
                "Number must be between 0 and %i" % (self._upper + len(TS790E_SPECIALS))
            )
        ...
        if real_number >= 30 and memory.offset is not None:
            ...
            self.command_no_output(self.pipe, *self._cmd_set_split(real_number, spec))

        spec = self._make_base_spec(memory, memory.freq)
        self.command_no_output(self.pipe, *self._cmd_set_memory(real_number, spec))
    ...
    def _make_base_spec(self, mem, freq):
        if mem.mode == "FM" and mem.tmode == TS790E_TMODES[1]:
            tmode = "1"
