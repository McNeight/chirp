...
TOTPERMISSIONS_LIST = ["Always", "CTCSS/DCS Match", "Channel Free", "Receive Only"]
...
class HA1GBankModel(chirp_common.BankModel):
    ...
    def get_used_zone_index(self):
        return [x for x in self.zinfo.zoneindex[0 : self.zinfo.zonenum] if x != 0xFFFF]
    ...
    def _get_channel_numbers_in_bank(self, bank):
        ...
        return set(
            [
                int(ch) - self._radio._skip_vfoch_count + 1
                for ch in _members.chindex
                if ch != 0xFFFF and int(ch) - self._radio._skip_vfoch_count + 1 > 0
            ]
        )
    ...
    def _update_bank_with_channel_numbers(self, bank, channels_in_bank):
        ...
        for index, channel_number in enumerate(sorted(channels_in_bank)):
            _members.chindex[index] = channel_number + self._radio._skip_vfoch_count - 1
            empty = index + 1
    ...
    def remove_memory_from_mapping(self, memory, bank):
        ...
        if not channels_in_bank and bank.index in self.get_used_zone_index():
            ...
            _zone_index = [x for x in self.get_used_zone_index() if x != bank.index]
            self.zinfo.zoneindex = _zone_index + [0xFFFF] * (
...
def do_download(self):
    ...
    try:
        ...
        raise errors.RadioError(
            error_map.get(handshake_result, "Unknown error communicating with radio")
        )
...
def handshake(self, serial):
    ...
    for num in range(max_retries):
        flag, databytes = exchange_block_with_radio(
            get_handshake_bytes(self.current_model + " "), serial, self.read_packet_len
        )
...
def validate_packet(current_packet_Byte: bytes, chunk_size=1024):
    ...
    """
     ...
     performance boost!
    """
    data_len = current_packet_Byte[12] - 6 if current_packet_Byte[12] > 6 else 0
...
def get_read_current_packet_bytes(self, item: int, serial, status):
    ...
    while packet_index < packet_count:
        flag, new_data_bytes = exchange_block_with_radio(
            ...
            serial,
            self.read_packet_len,
        )
        if not flag or (len(new_data_bytes) > 14 and new_data_bytes[14] != item):
...
def write_item_current_page_bytes(self, serial, item_Bytes: bytes, item: int, status):
    ...
    for i in range(0, packet_count):
        ...
        flag, new_data_bytes = exchange_block_with_radio(
            current_packet_bytes, serial, self.read_packet_len
        )
        if not flag or (len(new_data_bytes) > 14 and new_data_bytes[14] != item):
            raise errors.RadioError("radio reported failure")
...
def copy_to_memory(
    all_bytes: bytearray, item_bytes: bytes, start_addr: int, item_len: int
):
    """
...
def exit_programming_mode(self):
    ...
    try:
        serial.write(
            get_send_packet_bytes(
                reboot_sign, packet_index, data_code, packet_count, data_buffer
            )
        )
    except Exception as e:
        LOG.debug(f"Radio refused to exit programming mode:{e}")
        raise errors.RadioError("Radio refused to exit programming mode")


def get_send_packet_bytes(
    data_type: int,
    packet_index: int,
    data_code: int,
    packet_count: int,
    ...
    if compare_version(ver, self.REQUIRED_VER) < 0:
        raise errors.RadioError(
            (
                ...
                "to be compatible with CHIRP"
            )
            % (ver, self.REQUIRED_VER)
        )
...
def _get_memory(self, mem, _mem, ch_index):
    ...
    mem.extra.append(
        RadioSetting(
            ...
            "TX Permissions",
            RadioSettingValueList(
                TOTPERMISSIONS_LIST, current_index=_mem.totpermissions
            ),
        )
    )
    mem.extra.append(
    ...
    mem.extra.append(
        RadioSetting(
            ...
            RadioSettingValueList(
                ...
                current_index=get_item_by_id(self._alarm_list, _mem.alarmlist),
            ),
        )
    )
    mem.extra.append(
        RadioSetting(
            ...
            RadioSettingValueList(
                ...
                current_index=get_item_by_id(self._dtmf_list, _mem.dtmfsignalinglist),
            ),
        )
    )
    mem.extra.append(
...
def get_dtmf_setting(self, dtmf):
    ...
    dtmf.append(
        get_radiosetting_by_key(
            self, _dtmf_comm, "stunmode", "Stun Type", _dtmf_comm.stunmode, opts
        )
    )
...
def _set_memory(self, mem, _mem, ch_index):
    ...
    for setting in mem.extra:
        if setting.get_name() == "tottime":
            ...
            _mem.alarmlist = get_item_by_name(self._alarm_list, setting.value)
        elif setting.get_name() == "dtmfsignalinglist":
            _mem.dtmfsignalinglist = get_item_by_name(self._dtmf_list, setting.value)
        else:
...
def set_ch_index(self, ch_index_list):
    ...
    for i in range(len(_ch_data.chindex)):
        _ch_data.chindex[i] = ch_index_list[i] if i < ch_num else 0xFFFF
...
def set_band_selection(
    set_item, obj, opts, home_select_field_name, home_index_field_name
):
...
class HA1G(chirp_common.CloneModeRadio):
    ...
    EXCLUDED_REGIONS = {"radioHead", "radioInfo", "radioVer", "scanData", "alarmData"}
    ...
    def validate_memory(self, mem):
        ...
        if (
            chirp_common.in_range(mem.freq, [self._airband])
    ...
    def read_items(self, serial):
        ...
        for item_name, (region_id, start_addr, length) in items:
            try:
                ...
                if item_bytes:
                    copy_to_memory(all_bytes, item_bytes, start_addr, length)
            except errors.RadioError:
                raise
            except Exception as e:
                LOG.error(f"read item_data error: {item_name} error_msg: {e}")
                continue
        return all_bytes
    ...
    def write_items(self, serial):
        ...
        item_bytes = get_read_current_packet_bytes(self, radiover_id, serial, status)
        if item_bytes:
            firmware_version = unpack_version(item_bytes[2:6])
            validate_version(self, firmware_version)
        items = self.MEMORY_REGIONS_RANGES.items()
        for item_name, (region_id, start_addr, length) in items:
            ...
            write_item_current_page_bytes(self, serial, item_bytes, region_id, status)
    ...
    def supports_airband(self):
        return (
            compare_version(self.metadata.get("ha1g_firmware", "0.0.0.0"), "1.1.12.5")
            >= 0
        )

    def supports_banks(self):
        return (
            compare_version(self.metadata.get("ha1g_firmware", "0.0.0.0"), "1.1.13.1")
            >= 0
        )
