...
class Platform:
    ...
    def os_version_string(self):
        ...
        u = platform.uname()
        return f'{u.system} {u.release} ({u.version}) on {u.machine}'
    ...
    def executable_path(self):
        ...
        if we_are_frozen():
            ...
            return os.path.dirname(
                os.path.abspath(os.path.join(_find_me(), '..'))
            )
    ...
    def find_resource(self, filename):
        ...
        pkgshare_candidates = [
            os.path.join(i, 'chirp') for i in share_candidates
        ]
...
class Win32Platform(Platform):
    ...
    def is_ble_serial(self, serial):
        ...
        LOG.debug(
            'Found BLE device %s, guessing pair is %s' % (device, pair_device)
        )
        try:
