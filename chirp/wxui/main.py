...
def get_stock_configs():
    default_dir = chirp_platform.get_platform().config_file("stock_configs")
...
class ChirpDropTarget(wx.DropTarget):
    ...
    def move_tab(self, source_window, index, target_index):
        ...
        self._main.add_editorset(
            eset, atindex=(target_index if target_index >= 0 else None)
        )
    ...
    def handle_tab_data(self, x, y):
        ...
        try:
            ...
            LOG.warning(
                "Unable to perform HitTest on target: " "no reordering possible"
            )
...
class ChirpEditorSet(wx.Panel):
    ...
    def tab_name(self):
        ...
        return "%s.%s" % (self.default_filename, self._radio.FILE_EXTENSION)
    ...
    def _editor_status(self, event):
        ...
        wx.PostEvent(self, common.StatusMessage(self.GetId(), message=event.message))
    ...
    def __init__(self, radio, filename, *a, **k):
        ...
        if len(radios) > 2 or features.has_dynamic_subdevices:
            LOG.info("Using TreeBook because radio has %i devices", len(radios))
            self._editors = wx.Treebook(self, style=wx.NB_RIGHT)
            self._editors.Bind(wx.EVT_TREEBOOK_PAGE_CHANGED, self._editor_selected)
        else:
        ...
        for radio in radios:
            ...
            self.add_editor(
                edit, format % {"type": _("Memories"), "variant": radio.VARIANT}
            )
            edit.refresh()
            ...
            if sub_features.has_bank and self.BANK_CLS:
                ...
                self.add_editor(
                    banks, format % {"type": _("Banks"), "variant": radio.VARIANT}
                )
        ...
        if developer.developer_mode() and isinstance(
    ...
    def default_filename(self):
        defname_format = (
            CONF.get("default_filename", "global") or "{vendor}_{model}_{date}"
        )
    ...
    def select_editor(self, index=None, name=None):
        if index is None and name:
            try:
                ...
                return
        if index is None:
            LOG.error("Unable to find editor to select (%r/%r", index, name)
            return
...
class ChirpLiveEditorSet(ChirpEditorSet):
    ...
    @property
    def tab_name(self):
        return "%s %s@%s" % (self._radio.VENDOR, self._radio.MODEL, _("LIVE"))
...
class ChirpWelcomePanel(wx.Panel):
    ...
    def __init__(self, *a, **k):
        ...
        welcome_file = locale_base_path.joinpath("welcome_screen_%s.png" % locale)
...
class ChirpMain(wx.Frame):
    def __init__(self, *args, **kwargs):
        ...
        self.SetSize(
            int(CONF.get("window_w", "state") or 1024),
        ...
        self.Bind(wx.aui.EVT_AUINOTEBOOK_PAGE_CHANGED, self._editor_page_changed)
        self._editors.Bind(wx.aui.EVT_AUINOTEBOOK_BEGIN_DRAG, self._tab_drag)
        self._editors.Bind(wx.aui.EVT_AUINOTEBOOK_TAB_RIGHT_DOWN, self._tab_rclick)
    ...
    def set_icon(self):
        if sys.platform == "win32":
            ...
            icon = "chirp.png"
        with resources.as_file(resources.files("chirp.share").joinpath(icon)) as path:
    ...
    def open_file(self, filename, exists=True, select=True, rclass=None):
        ...
        with common.expose_logs(logging.WARNING, "chirp.drivers", label, parent=self):
            if exists:
                LOG.debug("Doing open from %r" % filename)
                if not os.path.exists(filename):
                    raise FileNotFoundError(_("File does not exist: %s") % filename)
                if rclass is None:
        ...
        if not isinstance(radio, CSVRadio) or isinstance(
            radio, chirp_common.NetworkSourceRadio
        ):
    ...
    def add_editorset(self, editorset, select=True, atindex=None):
        ...
        if atindex is None:
            self._editors.AddPage(
                editorset, os.path.basename(editorset.filename), select=select
            )
        else:
            self._editors.InsertPage(
                atindex, editorset, os.path.basename(editorset.filename), select=select
            )
        ...
        editorset.Bind(
            common.EVT_EDITOR_REFRESH, lambda e: self._reload_editorset(e, editorset)
        )
    ...
    def add_stock_menu(self):
        ...
        dist_stock_confs = sorted(
            [
                (conf.name, hashlib.md5(conf.read_bytes()))
                for conf in resources.files("chirp.stock_configs").iterdir()
                if conf.is_file()
            ]
        )
        ...
        def add_stock(fn):
            ...
            self.Bind(wx.EVT_MENU, self._menu_open_stock_config, submenu_item)
        ...
        if sys.platform in ("darwin", "linux", "win32"):
            reveal = stock.Append(REVEAL_STOCK_DIR, _("Open stock config directory"))
        ...
        for fn, hash in dist_stock_confs:
            if os.path.basename(fn) in found:
                ...
                try:
                    user_fn = os.path.join(get_stock_configs(), os.path.basename(fn))
                    with open(user_fn, "rb") as f:
                        user_hash = hashlib.md5(f.read())
                    if hash.digest() == user_hash.digest():
                        LOG.info(
                            "Removing stale legacy stock config %s",
                            os.path.basename(fn),
                        )
                        os.remove(user_fn)
                    ...
                    LOG.info(
                        "Ignoring dist stock conf %s because same name "
                        "found in user dir: %s",
                        os.path.basename(fn),
                        e,
                    )
    ...
    def make_menubar(self):
        ...
        new_window.SetAccel(
            wx.AcceleratorEntry(wx.MOD_CONTROL | wx.ACCEL_SHIFT, ord("N"))
        )
        ...
        file_menu.AppendSubMenu(self.OPEN_STOCK_CONFIG_MENU, _("Open Stock Config"))

        self.OPEN_RECENT_MENU = wx.Menu()
        self.restore_tabs_item = wx.NewId()

        last_files = [x for x in (CONF.get("last_open", "state") or "").split("$") if x]
        ...
        if last_files:
            ...
            submenu_item.SetAccel(
                wx.AcceleratorEntry(wx.MOD_CONTROL | wx.ACCEL_SHIFT, ord("T"))
            )
        ...
        if developer.developer_mode():
            loadmod_item = file_menu.Append(
                wx.MenuItem(file_menu, wx.NewId(), _("Load Module..."))
            )
        ...
        print_preview_item = wx.MenuItem(
            file_menu, self._print_preview_item, _("Print Preview")
        )
        ...
        self.Bind(wx.EVT_MENU, functools.partial(self._menu_copy, cut=True), cut_item)
        ...
        if platform.system() == "Windows":
            ...
            findnextacc = wx.AcceleratorEntry(wx.MOD_CONTROL | wx.ACCEL_ALT, ord("F"))

        self._find_next_item = wx.NewId()
        find_next_item = edit_menu.Append(
            wx.MenuItem(edit_menu, self._find_next_item, _("Find Next"))
        )
        find_next_item.SetAccel(findnextacc)
        self.Bind(wx.EVT_MENU, self._menu_find, find_next_item, self._find_next_item)
        ...
        fixed_item = wx.MenuItem(
            view_menu, self._fixed_item, _("Use fixed-width font"), kind=wx.ITEM_CHECK
        )
        ...
        large_item = wx.MenuItem(
            view_menu, self._large_item, _("Use larger font"), kind=wx.ITEM_CHECK
        )
        ...
        restore_tabs = wx.MenuItem(
            view_menu, wx.NewId(), _("Restore tabs on start"), kind=wx.ITEM_CHECK
        )
        ...
        lang_item = wx.MenuItem(view_menu, wx.NewId(), _("Language") + "...")
        ...
        download_item = wx.MenuItem(
            radio_menu, self._download_menu_item, _("Download from radio...")
        )
        ...
        upload_item = wx.MenuItem(
            radio_menu, self._upload_menu_item, _("Upload to radio...")
        )
        ...
        query_rr_item = wx.MenuItem(source_menu, wx.NewId(), "RadioReference.com")
        ...
        query_prznet_item = wx.MenuItem(source_menu, wx.NewId(), "przemienniki.net")
        self.Bind(wx.EVT_MENU, self._menu_query_prznet, query_prznet_item)
        source_menu.Append(query_prznet_item)

        query_przeu_item = wx.MenuItem(source_menu, wx.NewId(), "przemienniki.eu")
        ...
        query_amsat_item = wx.MenuItem(
            source_menu, wx.NewId(), "Radio Amateur Satellites (GitHub Mirror)"
        )
        self.Bind(wx.EVT_MENU, self._menu_query_amsats, query_amsat_item)
        source_menu.Append(query_amsat_item)

        query_satnogs_item = wx.MenuItem(
            source_menu, wx.NewId(), "SatNOGS DB (Direct API)"
        )
        ...
        auto_edits = wx.MenuItem(
            radio_menu, wx.NewId(), _("Enable Automatic Edits"), kind=wx.ITEM_CHECK
        )
        ...
        select_bandplan = wx.MenuItem(radio_menu, wx.NewId(), _("Select Bandplan..."))
        ...
        if developer.developer_mode():
            ...
            reload_drv_item = wx.MenuItem(
                radio_menu, self._reload_driver_item, _("Reload Driver")
            )
            reload_drv_item.SetAccel(wx.AcceleratorEntry(wx.MOD_CONTROL, ord("R")))
            ...
            reload_both_item = wx.MenuItem(
                radio_menu, self._reload_both_item, _("Reload Driver and File")
            )
            reload_both_item.SetAccel(
                wx.AcceleratorEntry(wx.ACCEL_ALT | wx.MOD_CONTROL, ord("R"))
            )
            self.Bind(
                wx.EVT_MENU,
            ...
            interact_drv_item = wx.MenuItem(
                radio_menu, self._interact_driver_item, _("Interact with driver")
            )
            self.Bind(wx.EVT_MENU, self._menu_interact_driver, interact_drv_item)
        ...
        developer_menu = wx.MenuItem(
            help_menu, wx.NewId(), _("Developer Mode"), kind=wx.ITEM_CHECK
        )
        self.Bind(
            wx.EVT_MENU,
        ...
        reporting_menu = wx.MenuItem(
            help_menu, wx.NewId(), _("Reporting enabled"), kind=wx.ITEM_CHECK
        )
        self.Bind(
            wx.EVT_MENU,
        ...
        if developer.developer_mode():
            trace_item = wx.MenuItem(help_menu, wx.NewId(), _("Open last serial trace"))
    ...
    def _menu_print(self, event):
        p = printing.MemoryPrinter(
            self, self.current_editorset.radio, self.current_editorset.current_editor
        )
    ...
    def make_toolbar(self):
        ...
        def bm(stock):
            return wx.ArtProvider.GetBitmap(stock, wx.ART_TOOLBAR, (32, 32))

        tbopen = tb.AddTool(
            wx.NewId(), _("Open"), bm(wx.ART_FILE_OPEN), _("Open a file")
        )
        tb.AddTool(wx.NewId(), _("Save"), bm(wx.ART_FILE_SAVE), _("Save file"))
        tb.AddTool(wx.NewId(), _("Close"), bm(wx.ART_CLOSE), _("Close file"))
        tb.AddTool(
            wx.NewId(), _("Download"), bm(wx.ART_GO_DOWN), _("Download from radio")
        )
        tb.AddTool(wx.NewId(), "Upload", bm(wx.ART_GO_UP), _("Upload to radio"))
    ...
    def adj_menu_open_recent(self, filename):
        ...
        if filename:
            ...
            if (
                stock_dir
                and os.path.exists(stock_dir)
                and this_dir
                and os.path.samefile(stock_dir, this_dir)
            ):
                return

        # Make a list of recent files in config
        recent = [
            CONF.get("recent%i" % i, "state")
        ...
        while self.OPEN_RECENT_MENU.GetMenuItemCount():
            self.OPEN_RECENT_MENU.Delete(self.OPEN_RECENT_MENU.FindItemByPosition(0))
    ...
    def _reload_editorset(self, event, editorset):
        ...
        # fundamental change
        LOG.info("Editor %s set %s needs refresh" % (event.GetEventObject(), editorset))
        ...
        new_eset = ChirpEditorSet(editorset.radio, editorset.filename, self._editors)
        ...
        for i, (name, editor) in enumerate(new_eset.iter_editors()):
            if i >= selected_editor_idx and isinstance(editor, selected_editor_cls):
                LOG.debug(
                    "Selecting editor %i type %s name %r",
                    i,
                    selected_editor_cls.__name__,
                    name,
                )
    ...
    def _editor_cross_action(self, event):
        ...
        for i in range(self._editors.GetPageCount()):
            ...
            for title, editor in page.iter_editors(onlycls=memedit.ChirpMemEdit):
                tabs["%s::%s" % (os.path.basename(page.filename), title)] = editor
        ...
        label = wx.StaticText(
            d,
            label=_("Select a tab and memory to diff"),
    ...
    def _update_window_for_editor(self):
        ...
        if eset is not None:
            ...
            is_network = isinstance(eset.radio, base.NetworkResultRadio)
            can_close = True
        ...
        for menu, items in self.editor_menu_items.items():
            for item in items:
                if item.GetId() in (wx.ID_UNDO, wx.ID_REDO):
                    ...
                    continue
                editor_match = (
                    eset and isinstance(eset.current_editor, item.editor_class) or False
                )
    ...
    def _window_close(self, event):
        ...
        CONF.set("last_dir", chirp_platform.get_platform().get_last_dir(), "state")
        ...
        CONF.set(
            "last_open",
            "$".join(os.path.abspath(fn) for fn in open_files if fn),
            "state",
        )
        config._CONFIG.save()
    ...
    def _update_editorset_title(self, editorset):
        ...
        self._editors.SetPageText(
            index,
            "%s%s"
            % (os.path.basename(editorset.filename), editorset.modified and "*" or ""),
        )
    ...
    def _do_open(self):
        all_extensions = ["*.img"]
        formats = [
            _("Chirp Image Files") + " (*.img)|*.img",
            _("All Files") + " (*.*)|*.*",
        ]
        for name, pattern, readonly in directory.AUX_FORMATS:
            formats.insert(1, "%s %s (%s)|%s" % (name, _("Files"), pattern, pattern))
        ...
        if CONF.get_bool("open_default_all_formats", "prefs", True):
            ...
            i_index = len(formats)
        formats.insert(
            i_index, (_("All supported formats") + "|" + ";".join(all_extensions))
        )
    ...
    def _menu_open_stock_config(self, event):
        ...
        fn = self.OPEN_STOCK_CONFIG_MENU.FindItemById(event.GetId()).GetItemLabelText()
        ...
        if os.path.exists(user_stock_conf):
            ...
            LOG.error("Unable to find %s or %s" % (user_stock_conf, dist_stock_conf))
            common.error_proof.show_error(_("Unable to find stock config %r") % fn)
            return
    ...
    def _menu_open_recent(self, event):
        filename = self.OPEN_RECENT_MENU.FindItemById(event.GetId()).GetItemLabelText()
        self.open_file(filename)

    def _menu_save_as(self, event):
        eset = self.current_editorset
        wildcard = "CHIRP %(vendor)s %(model)s %(files)s (*.%(ext)s)|*.%(ext)s" % {
        ...
        for name, pattern, readonly in directory.AUX_FORMATS:
            ...
            if name in eset.radio.FORMATS:
                wildcard += "|%s %s (%s)|%s" % (name, _("Files"), pattern, pattern)
    ...
    def _menu_import(self, event):
        ...
        d = wx.MessageDialog(
            self,
            _(
                "The recommended procedure for importing memories is to open "
        ...
        if r == wx.ID_YES:
            with common.expose_logs(
                logging.WARNING, "chirp.drivers", _("Import messages"), parent=self
            ):
    ...
    def _menu_export(self, event):
        ...
        defcsv = (
            os.path.splitext(os.path.basename(self.current_editorset.filename))[0]
            + ".csv"
        )
        style = wx.FD_SAVE | wx.FD_OVERWRITE_PROMPT | wx.FD_CHANGE_DIR
        with wx.FileDialog(
            self, _("Export to CSV"), defaultFile=defcsv, wildcard=wildcard, style=style
        ) as fd:
            if fd.ShowModal() == wx.ID_CANCEL:
                return
    ...
    def _prompt_to_close_editor(self, editorset):
        ...
        answer = wx.MessageBox(
            _("%s has not been saved. Save before closing?") % (editorset.filename),
    ...
    def _menu_copy(self, event, cut=False):
        try:
            ...
            LOG.warning(
                "Attempt to cut/copy from %s not supported",
    ...
    def _menu_paste(self, event):
        try:
            ...
            LOG.warning(
                "Attempt to paste to %s not supported",
                self.current_editorset.current_editor,
            )
    ...
    def _menu_find(self, event):
        if event.GetId() == wx.ID_FIND:
            search = wx.GetTextFromUser(
                _("Find") + ":", _("Find"), self._last_search_text, self
            )
    ...
    def _menu_language(self, event):
        def fmt_lang(lang):
            return "%s - %s" % (lang.DescriptionNative, lang.Description.split(" ")[0])

        trans = wx.Translations.Get()
        langs = {
            fmt_lang(wx.Locale.FindLanguageInfo(code)): code
        ...
        choices = [_("Automatic from system") + " " * 30] + sorted(
            langs.keys(), key=str.casefold
        )
        try:
            current = wx.Locale.FindLanguageInfo(CONF.get("force_language", "prefs"))
            ...
            LOG.debug(
                "Unable to find current language selection; " "defaulting to auto"
            )
            initial = 0
        ...
        try:
            LOG.debug("User chose override language %r (%r)", choice, langs[choice])
        ...
        if initial != choices.index(choice):
            wx.MessageBox(
                _("CHIRP must be restarted for the new selection " "to take effect"),
                _("Restart Required"),
            )
    ...
    def _make_backup(self, radio, backup_type):
        ...
        backup_fn = os.path.join(
            ...
            "%s_%s_%s"
            % (
                directory.radio_class_id(radio.__class__),
                backup_type,
        ...
        try:
            ...
            bydate = [(os.stat(os.path.join(backup_dir, f)).st_mtime, f) for f in files]
            now = time.time()
            for mtime, fn in sorted(bydate):
                age = (now - mtime) // 86400
                if keep_days > 0 and age > keep_days:
                    os.remove(os.path.join(backup_dir, fn))
                    LOG.warning("Pruned backup %s older than %i days", fn, keep_days)
    ...
    def _menu_upload(self, event):
        ...
        if isinstance(radio, chirp_common.LiveRadio):
            msg = _(
                "This is a live-mode radio, which means changes are "
                "sent to the radio in real-time as you make them. Upload "
                "is not necessary!"
            )
            d = wx.MessageDialog(self, msg, _("Live Radio"), wx.ICON_INFORMATION)
        elif isinstance(radio, (CSVRadio, base.NetworkResultRadio)):
            msg = _(
                "This is a radio-independent file and cannot be uploaded "
            ...
            d = wx.MessageDialog(
                self, msg, _("Unable to upload this file"), wx.ICON_INFORMATION
            )
        else:
    ...
    def _menu_reload_driver(self, event, andfile=False):
        ...
        LOG.info(
            "Reloaded radio driver%s in place; good luck!"
    ...
    def load_module(self, filename):
        ...
        modname = "chirp.loaded.%s" % os.path.splitext(os.path.basename(filename))[0]
    ...
    def _menu_load_module(self, event):
        ...
        r = wx.MessageDialog(
            self,
            _(
                "Loading modules can be extremely dangerous, "
    ...
    def _menu_about(self, event):
        ...
        vbox.Add(label, 0, border=10, flag=wx.ALIGN_CENTER | wx.ALL)

        details = (
            "Python %s.%s.%s" % (pyver.major, pyver.minor, pyver.micro),
            "wxPython %s" % wx.version(),
        )
        for detail in details:
            label = wx.StaticText(d, label=detail)
            vbox.Add(label, 0, border=10, flag=wx.ALIGN_CENTER)

        documentation_link = wx.adv.HyperlinkCtrl(
            d,
            label=_("Click here for online documentation"),
...
GNU General Public License for more details."""
        ...
        license_link = wx.adv.HyperlinkCtrl(
            d,
            label=_("Click here for full license text"),
            url=(
                "https://www.chirpmyradio.com/projects/chirp/repository/"
    ...
    def _menu_developer(self, menuitem, event):
        ...
        if menuitem.IsChecked():
            ...
            d = wx.MessageDialog(
                self, msg, _("Danger Ahead"), style=wx.ICON_WARNING | wx.YES_NO
            )
        ...
        wx.MessageBox(
            _("Developer state is now %s. " "CHIRP must be restarted to take effect")
            % state,
            _("Restart Required"),
            wx.OK,
        )
    ...
    def _menu_backup_loc(self, event):
        ...
        try:
            ...
            LOG.warning("Failed to create backup directory %s: %s" % (backup_dir, e))
            return
    ...
    def _menu_load_from_issue(self, event):
        if self.current_editorset:
            r = wx.MessageBox(
                _(
                    "Loading a module will not affect open tabs. "
        ...
        if module:
            if self.load_module(module):
                wx.MessageBox(
                    _("Module loaded successfully"), _("Success"), wx.ICON_INFORMATION
                )
    ...
    def _menu_last_trace(self, event):
        try:
            ...
            common.error_proof.show_error(_("No traces stored"), parent=self)
    ...
    def _menu_select_bandplan(self, event):
        ...
        plans = sorted(
            [(shortname, details[0]) for shortname, details in bandplans.plans.items()],
            key=lambda x: x[1],
        )
        d = wx.SingleChoiceDialog(
            self, _("Select a bandplan"), _("Bandplan"), [x[1] for x in plans]
        )
    ...
    def _do_network_query(self, query_cls):
        ...
        if r == wx.ID_OK:
            ...
            editorset = ChirpEditorSet(d.result_radio, None, self._editors)
            self.add_editorset(editorset)
...
def display_update_notice(version):
    ...
    if (
        CONF.is_defined("last_update_check", "state")
        and (time.time() - CONF.get_int("last_update_check", "state")) < intv
    ):
        return
    ...
    msg = _(
        "A new CHIRP version is available. Please visit the "
    ...
    if visit == wx.ID_OK:
        wx.MessageBox(
            _("Please be sure to quit CHIRP before installing " "the new version!")
        )
        webbrowser.open(url)
