...
def open_serial(port, rclass):
    ...
    if "://" in port:
        ...
        pipe = serialtrace.SerialTrace(
            baudrate=rclass.BAUD_RATE, rtscts=rclass.HARDWARE_FLOW, timeout=0.25
        )
        pipe.rts = rclass.WANTS_RTS
        pipe.dtr = rclass.WANTS_DTR
        pipe.port = port
    ...
    LOG.debug("Serial opened: %s (rts=%s dtr=%s)", pipe, pipe.rts, pipe.dtr)
    return pipe
...
class ChirpRadioPromptDialog(wx.Dialog):
    def __init__(self, *a, **k):
        ...
        vbox.Add(instructions, border=20, flag=wx.ALL)
        ...
        self.cb = wx.CheckBox(
            self,
            label=_("Do not prompt again for %s")
...
def port_label(port):
    ...
    if not port.description:
        ...
        return label_fmt % ({"desc": port.description, "dev": dev})
...
def detected_value(parent_rclass, rclass):
    ...
    if not label:
        LOG.error(
            "Calculated blank detected value of %s from %s", rclass, parent_rclass
        )
    return label
...
class ChirpCloneDialog(wx.Dialog):
    def __init__(self, *a, **k):
        ...
        def _add_grid(label, control):
            ...
            grid.Add(
                control, proportion=1, border=10, flag=wx.EXPAND | wx.RIGHT | wx.LEFT
            )
        ...
        vbox = wx.BoxSizer(wx.VERTICAL)
        vbox.Add(grid, proportion=1, flag=wx.TOP | wx.BOTTOM | wx.EXPAND, border=20)
        self.model_msg = wx.StaticText(
            self,
            label="",
            style=(wx.ALIGN_CENTER_HORIZONTAL | wx.ST_NO_AUTORESIZE | wx.ELLIPSIZE_END),
        )
        ...
        vbox.Add(self.status_msg, border=5, proportion=0, flag=wx.EXPAND | wx.BOTTOM)
        vbox.Add(
            self.gauge, flag=wx.EXPAND | wx.RIGHT | wx.LEFT, border=10, proportion=0
        )
        vbox.Add(self.model_msg, border=5, proportion=0, flag=wx.EXPAND | wx.BOTTOM)
        ...
        for rclass in directory.DRV_TO_RADIO.values():
            if not issubclass(rclass, chirp_common.CloneModeRadio) and not issubclass(
                rclass, chirp_common.LiveRadio
            ):
                continue
            if getattr(rclass, "_DETECTED_BY", None) and not allow_detected_models:
                continue
        ...
        try:
            self.select_vendor_model(
                CONF.get("last_vendor", "state"), CONF.get("last_model", "state")
            )
        except ValueError:
            LOG.warning(
                "Last vendor/model (%s/%s) not found",
    ...
    def set_ports(self, system_ports=None, select=None):
        ...
        self.ports = [
            (port.device, port_label(port))
            for port in sorted(system_ports, key=port_sort_key)
        ...
        self._port.SetItems([x[1] for x in self.ports] + [CUSTOM, HELPME])
        for device, name in self.ports:
            ...
            LOG.warning("Last port %r is unavailable, defaulting to %s", select, port)
    ...
    def _port_assist(self, event):
        ...
        if not changed:
            wx.MessageBox(
                _(
                    "Unable to determine port for your cable. "
            ...
            wx.MessageBox(
                "%s\n%s" % (_("Your cable appears to be on port:"), port_label(found)),
                _("USB Port Finder"),
                parent=self,
            )
        else:
    ...
    def _prolific_assist(self, event):
        r = wx.MessageBox(
            _(
                "Your Prolific-based USB device will not work without "
    ...
    def _selected_port(self, event):
        ...
        if self._port.GetStringSelection() == CUSTOM:
            port = wx.GetTextFromUser(
                _("Enter custom port:"), _("Custom Port"), parent=self
            )
            if port:
    ...
    def _do_recent(self):
        ...
        d = wx.SingleChoiceDialog(
            self, _("Choose a recent model"), _("Recent"), recent_strs
        )
        box = d.GetSizer()
        panel = wx.Panel(d)
        hbox = wx.BoxSizer(wx.HORIZONTAL)
        ...
        def remove_selected(event):
            listbox = [x for x in d.GetChildren() if isinstance(x, wx.ListBox)][0]
    ...
    def _select_port_for_model(self):
        ...
        if last_port and self.set_selected_port(last_port):
            LOG.debug(
                "Automatically chose last-used port %s for %s %s",
                last_port,
                vendor,
                model,
            )
    ...
    def _remember_port_for_selected(self):
        ...
        LOG.debug(
            "Recorded last-used port %s for %s %s", port, rclass.VENDOR, rclass.MODEL
        )
    ...
    def fail(self, error):
        def safe_fail():
            common.error_proof.show_error(
                error, parent=self, title=_("Error communicating with radio")
            )
...
class ChirpDownloadDialog(ChirpCloneDialog):
    def __init__(self, *a, **k):
        super().__init__(*a, **k)
        if CONF.get_bool("always_start_recent", "state") and CONF.get(
            "recent_models", "state"
        ):
            self._do_recent()
    ...
    def _selected_model(self, event):
        ...
        if prompts.experimental:
            ...
            r = d.ShowModal()
            if r == wx.ID_CANCEL:
                LOG.info("User did not accept experimental risk for %s", rclass)
                self.FindWindowById(wx.ID_OK).Disable()
            else:
                LOG.info("User accepted experimental risk for %s", rclass)
                self.FindWindowById(wx.ID_OK).Enable()
    ...
    def _actual_action(self, event):
        ...
        if prompts.info:
            d = ChirpRadioPromptDialog(
                self, title=_("Radio information"), radio=rclass, prompt="info"
            )
        ...
        self.model_msg.SetLabel(
            "%s %s %s" % (rclass.VENDOR, rclass.MODEL, rclass.VARIANT)
        )
    ...
    def _persist_choices(self):
        ...
        CONF.set("last_model", self._model_choices[self._model.GetSelection()], "state")
        ...
        if recent:
            ...
            recent = []
        modelstr = "%s:%s" % (
            self._vendor.GetStringSelection(),
...
class ChirpUploadDialog(ChirpCloneDialog):
    def __init__(self, radio, *a, **k):
        super(ChirpUploadDialog, self).__init__(*a, allow_detected_models=True, **k)
        self._radio = radio
    ...
    def _actual_action(self, event):
        ...
        if prompts.info:
            d = ChirpRadioPromptDialog(
                self, title=_("Radio information"), radio=self._radio, prompt="info"
            )
            if d.ShowModal() != wx.ID_OK:
                self.cancel_action()
                return
