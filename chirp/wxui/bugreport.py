...
_ = wx.GetTranslation
CONF = config.get()
BASE = CONF.get("baseurl", "chirpmyradio") or "https://data.chirpmyradio.com/redmine"
...
def get_macos_system_info(manifest):
    try:
        sp = subprocess.check_output(
            "system_profiler SPSoftwareDataType SPUSBDataType", shell=True
        )
...
def get_linux_system_info(manifest):
    try:
        sp = subprocess.check_output("lsusb", shell=True)
    except Exception as e:
...
def get_windows_system_info(manifest):
    try:
        sp = subprocess.check_output("pnputil /enum-devices /connected", shell=True)
    except Exception as e:
        sp = "Error getting system data: %s" % e
...
def prepare_report(chirpmain):
    ...
    LOG.debug(
        "Capturing config file %s stamped %s",
        conf_fn,
        datetime.datetime.fromtimestamp(os.stat(conf_fn).st_mtime).isoformat(),
    )
    ...
    if editor and isinstance(editor._radio, chirp_common.FileBackedRadio):
        ...
        LOG.debug(
            "Capturing focused open file %s from %s", editor.filename, editor._radio
        )
...
class BugReportPage(wx.adv.WizardPage):
    ...
    def __init__(self, context):
        ...
        inst = wx.StaticText(self, label=self.INST, style=wx.TE_CENTER)
...
class Start(BugReportPage):
    ...
    def _build(self, vbox):
        ...
        self.newbug = wx.RadioButton(
            choices, label=_("File a new bug"), style=wx.RB_GROUP
        )
        self.newbug.Bind(wx.EVT_RADIOBUTTON, self.validate_next)
        cvbox.Add(self.newbug, flag=wx.ALIGN_LEFT)

        self.existbug = wx.RadioButton(choices, label=_("Update an existing bug"))
        ...
        if not logger.Logger.instance.has_debug_log_file:
            wx.MessageDialog(
                self,
                _(
                    "The debug log file is not available when CHIRP is run "
...
class GetCreds(BugReportPage):
    INST = _(
        "Enter your username and password for chirpmyradio.com. "
    ...
    def _build(self, vbox):
        vbox.Add(
            wx.adv.HyperlinkCtrl(
                self, label=BASE + "/account/register", url=BASE + "/account/register"
            ),
            0,
            border=20,
        ...
        grid.Add(
            wx.StaticText(panel, label=_("Username")),
            border=20,
            flag=wx.ALIGN_CENTER | wx.RIGHT | wx.LEFT,
        )
        self.username = wx.TextCtrl(
            panel, value=CONF.get("chirp_user", "chirpmyradio") or ""
        )
        self.username.Bind(wx.EVT_TEXT, self.validate_next)
        grid.Add(self.username, 1, border=20, flag=wx.EXPAND | wx.RIGHT | wx.LEFT)

        grid.Add(
            wx.StaticText(panel, label=_("Password")),
            border=20,
        ...
        self.password = wx.TextCtrl(
            ...
            value=CONF.get_password("chirp_password", "chirpmyradio") or "",
        )
        self.password.Bind(wx.EVT_TEXT, self.validate_next)
        grid.Add(self.password, 1, border=20, flag=wx.EXPAND | wx.RIGHT | wx.LEFT)
        panel.SetSizer(grid)
        self.SetSizer(vbox)
    ...
    def _validate_next(self):
        return len(self.username.GetValue()) > 2 and len(self.password.GetValue()) > 2
    ...
    def _check_limit(self, uid):
        ...
        if not known_user and open_issue_count > issue_limit:
            LOG.warning(
                "User %i is not known and has %i open recent issues",
                uid,
                open_issue_count,
            )
            wx.MessageDialog(
                self,
                _(
                    "You have opened multiple issues within the last week. "
    ...
    def validate_success(self, event):
        ...
        r = self.context.session.get(BASE + "/my/account.json", auth=self.context.auth)
        if r.status_code != 200:
            LOG.error("Login auth check failed: %s", r.reason)
            wx.MessageDialog(
                self,
                _("Login failed: " "Check your username and password"),
        ...
        CONF.set_password("chirp_password", self.password.GetValue(), "chirpmyradio")
...
class NewBugInfo(BugReportPage):
    INST = _(
        "Enter information about the bug including a short but "
    ...
    def _build(self, vbox):
        ...
        grid.Add(
            wx.StaticText(panel, label=_("Bug subject:")),
            border=20,
            flag=wx.ALIGN_CENTER | wx.RIGHT | wx.LEFT,
        )
        ...
        grid.Add(
            wx.StaticText(panel, label=_("Radio model:")),
            border=20,
            flag=wx.ALIGN_CENTER | wx.RIGHT | wx.LEFT,
        )
        self.model = wx.TextCtrl(panel)
        grid.Add(self.model, 1, border=20, flag=wx.EXPAND | wx.RIGHT | wx.LEFT)
        ...
        panel = wx.Panel(self)
        panel.SetSizer(hbox)
        self.searched_dupes = wx.CheckBox(panel, label=_("I have"))
    ...
    def _validate_next(self):
        return len(self.subj.GetValue()) > 10 and self.searched_dupes.GetValue()
...
class ExistingBugInfo(BugReportPage):
    ...
    def _build(self, vbox):
        ...
        hbox.Add(
            wx.StaticText(panel, label=_("Bug number:")),
            0,
            border=20,
        ...
        self.bugnum.SetToolTip(
            _(
                "This is the ticket number for an already-created issue on the "
    ...
    def validate_success(self, event):
        ...
        r = self.context.session.get(
            BASE + "/issues/%s.json" % self.context.bugnum, auth=self.context.auth
        )
        if r.status_code != 200:
            LOG.error(
                "Failed to access issue %s: %i %s",
                self.context.bugnum,
                r.status_code,
                r.reason,
            )
            ...
            wx.MessageDialog(
                self,
                _(
                    "Please check the bug number and try again. If you do "
...
class BugUpdateInfo(BugReportPage):
    INST = _(
        "Enter details about this update. Be descriptive about what "
    ...
    DEFAULT = "\n".join(
        [
            ...
            "",
            _(
                "(Has this ever worked before? New radio? "
    ...
    def _build(self, vbox):
        ...
        self.details = wx.TextCtrl(self, style=wx.TE_MULTILINE, value=self.DEFAULT)
        try:
        ...
        vbox.Add(self.details, 1, border=20, flag=wx.EXPAND | wx.LEFT | wx.RIGHT)
...
class SubmitPage(BugReportPage):
    ...
    def update(self):
        ...
        text = "\n".join(
            [
                (
                    _("Reporting a new bug: %r") % self.context.bugsubj
                    if self.context.is_new
                    else _("Updating bug %s") % self.context.bugnum
                ),
                _("Files:") + " " + ",".join(self.context.manifest["files"].keys()),
...
class ResultPage(BugReportPage):
    def _build(self, vbox):
        ...
        self.result = wx.StaticText(self, label="Sending report...", style=wx.TE_CENTER)
    ...
    def start_thread(self):
        ...
        self.thread = threading.Thread(
            target=self.send_report, args=(self.context.manifest,)
        )
        self.thread.start()
    ...
    def send_report(self, manifest):
        LOG.info("Report thread running for issue %s", manifest.get("issue", "(new)"))
        try:
            ...
            failed = True
        wx.PostEvent(
            self, ReportThreadEvent(self.GetId(), result=result, failed=failed)
        )
    ...
    def _create_bug(self, manifest):
        ...
        r = self.context.session.post(
            BASE + "/issues.json", json=issue_data, auth=self.context.auth
        )
    ...
    def _upload_file(self, manifest, fn):
        for i in range(3):
            ...
            if r.status_code >= 400:
                LOG.error("Failed to upload %s: %s %s", fn, r.status_code, r.reason)
                time.sleep(2 + (2 * i))
                continue
    ...
    def _send_report(self, manifest):
        ...
        for fn in manifest["files"]:
            ...
            token_info = {"token": token, "filename": fn, "content_type": ct}
        ...
        if r.status_code != 204:
            LOG.error(
                "Failed to update issue %s with tokens %s: %s %s",
                manifest["issue"],
                tokens,
                r.status_code,
                r.reason,
    ...
    def report_done(self, event):
        ...
        if event.failed:
            self.result.SetLabel(_("Failed to send bug report:") + "\n" + event.result)
        else:
            self.result.SetLabel(_("Successfully sent bug report:"))
...
def do_bugreport(parent, event):
    ...
    wizard.Bind(wx.adv.EVT_WIZARD_PAGE_CHANGED, lambda e: e.GetPage().validate_next())
    wizard.Bind(
        wx.adv.EVT_WIZARD_PAGE_CHANGING,
        lambda e: (e.GetPage().validate_success(e) if e.GetDirection() else None),
    )
