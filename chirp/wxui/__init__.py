...
def maybe_install_desktop(args, parent):
    ...
    desktop_path = os.path.join(local, "share", "applications", "chirp.desktop")
    with resources.as_file(
        resources.files("chirp.share").joinpath("chirp.desktop")
    ) as desktop_src:
        with open(desktop_src) as f:
            desktop_content = f.readlines()
    with resources.as_file(resources.files("chirp.share").joinpath("chirp.ico")) as p:
        icon_path = str(p)
    ...
    if CONF.get_bool("offered_desktop", "state") and not args.install_desktop_app:
    ...
    with open(desktop_path, "w") as f:
        for line in desktop_content:
            if line.startswith("Exec"):
                ...
                f.write(line)
        LOG.debug("Wrote %s with exec=%r icon=%r" % (f.name, exec_path, icon_path))
...
def chirpmain():
    ...
    parser.add_argument(
        "--onlydriver", nargs="+", help="Include this driver while loading"
    )
    parser.add_argument(
        "--inspect", action="store_true", help="Show wxPython inspector"
    )
    parser.add_argument(
        "--page", default=None, help="Select this page of the default editor at start"
    )
    parser.add_argument(
        "--action", default=None, choices=actions, help="Start UI action immediately"
    )
    parser.add_argument(
        "--restore", default=None, action="store_true", help="Restore previous tabs"
    )
    parser.add_argument(
        "--force-language", default=None, help="Force locale to this ISO language code"
    )
    parser.add_argument(
        "--config-dir",
        help=("Use this alternate directory for config and " "other profile data"),
    )
    if sys.platform == "linux":
        ...
        desktop.add_argument(
            ...
            action="store_true",
            default=False,
            help=("Install a desktop icon even if it was " "previously refused"),
        )
        desktop.add_argument(
    ...
    if args.config_dir:
        try:
            ...
            pass
        assert os.path.isdir(args.config_dir), "--config must specify directory"
    ...
    if args.force_language:
        ...
        if force_lang is None:
            ...
            return 1
        LOG.info(
            "Forcing locale to %r (%s)" % (args.force_language, force_lang.Description)
        )
        ...
        if force_lang is None:
            LOG.warning(
                "Config prefs.force_language specifies unknown " "language %r",
                prefs_lang,
            )
            lang = wx.Locale.GetSystemLanguage()
        else:
            LOG.info(
                "Forcing locale to %r (%s) via config",
                prefs_lang,
    ...
    LOG.debug("Using locale: %s (%i)", app._lc.GetCanonicalName(), lang)
    LOG.debug(
        "Translation loaded=%s for CHIRP: %s (%s) from %s",
        app._lc.IsLoaded("CHIRP"),
        wx.Translations.Get().GetBestTranslation("CHIRP"),
        ",".join(wx.Translations.Get().GetAvailableTranslations("CHIRP")),
        localedir,
    )
    LOG.debug(
        "Translation loaded=%s for wxstd: %s (%s)",
        app._lc.IsLoaded("wxstd"),
        wx.Translations.Get().GetBestTranslation("wxstd"),
        ",".join(wx.Translations.Get().GetAvailableTranslations("wxstd")),
    )
    ...
    for fn in args.files:
        if os.path.abspath(fn) in restored:
            LOG.info("File %s on the command line is already being restored", fn)
            continue
    ...
    report.check_for_updates(lambda ver: wx.CallAfter(main.display_update_notice, ver))
