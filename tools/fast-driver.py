...
def find_imports(of_modules):
    ...
    for fn in glob.glob("chirp/drivers/*.py"):
        with open(fn) as f:
            lines = [ln for ln in f.readlines() if "import " in ln and "drivers" in ln]
        imports = []
        for line in lines:
...
base_modules = [mod for parent, mod in files_by_module if parent not in exclude_mods]
...
if base_modules:
    ...
    print("No driver tests necessary for changes in %s" % ",".join(files))
    sys.exit(0)
