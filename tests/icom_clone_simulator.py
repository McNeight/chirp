...
class FakeIcomRadio(object):
    ...
    def load_from_file(self, filename):
        ...
        LOG.debug("Initialized %i bytes from %s" % (len(self._memory), filename))
    ...
    def do_clone_out(self):
        ...
        for addr in range(0, self._radio.get_memsize(), size):
            if len(self._memory[addr:]) < 4:
                ...
                break
            header = bytes(struct.pack(">%sB" % self.address_fmt, addr, size))
            chunk = []
            cs = 0
            for byte in header:
    ...
    def do_clone_data(self, payload_hex):
        ...
        payload_bytes = bytes(
            [hex_to_byte(payload_hex[i : i + 2]) for i in range(0, len(payload_hex), 2)]
        )
        ...
        if len(data) != size:
            LOG.debug("Invalid frame size: expected %i, but got %i" % (size, len(data)))

        expected_addr = len(self._memory)
        if addr < expected_addr:
            LOG.debug("Frame goes back to %04x from %04x" % (addr, expected_addr))
        if len(self._memory) != addr:
            LOG.debug("Filling gap between %04x and %04x" % (expected_addr, addr))
    ...
    def write(self, data):
        ...
        if cmd == 0xE0:  # Ident
            # FIXME
            self.queue(self.make_response(0x01, model))  # Model
