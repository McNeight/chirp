...
class TestCaseCopyAll(base.DriverTest):
    ...
    def setUp(self):
        super().setUp()
        csvfile = os.path.join(os.path.dirname(self.TEST_IMAGE), "Generic_CSV.csv")
    ...
    def test_copy(self):
        ...
        for dst_number in range(
            self.rf.memory_bounds[0],
            min(self.rf.memory_bounds[0] + 10, self.rf.memory_bounds[1]),
        ):
        ...
        for number in range(bounds[0], bounds[1]):
            ...
            try:
                dst_mem = import_logic.import_mem(
                    self.radio, src_rf, src_mem, overrides={"number": dst_number}
                )
                import_logic.import_bank(self.radio, self.src_radio, dst_mem, src_mem)
