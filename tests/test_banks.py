...
class TestCaseBanks(base.DriverTest):
    ...
    def test_bank_names(self, testname="T"):
        ...
        for bank in banks:
            self.assertIsInstance(
                bank.get_name(), str, "Bank model returned non-string name"
            )
    ...
    def test_bank_store(self):
        ...
        if channelAlwaysHasBank:
            self.assertNotEqual(
                0,
                len(mem_banks),
                "Freshly-created memory has no banks " "but it should",
            )
        else:
            self.assertEqual(
                0, len(mem_banks), "Freshly-created memory has banks " "and should not"
            )
        ...
        self.assertIn(
            loc,
            [x.number for x in model.get_mapping_memories(banks[0])],
        ...
        if not channelAlwaysHasBank:
            ...
            self.assertRaises(
                Exception, model.remove_memory_from_mapping, mem, banks[0]
            )
    ...
    def test_bank_index(self):
        ...
        for i in range(*index_bounds):
            ...
            self.assertEqual(
                i, model.get_memory_index(mem, banks[0]), "Bank index not persisted"
            )

        suggested_index = model.get_next_mapping_index(banks[0])
        self.assertIn(
            suggested_index,
            list(range(*index_bounds)),
