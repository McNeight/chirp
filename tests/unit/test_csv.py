...
CHIRP_CSV_LEGACY = """Location,Name,Frequency,Duplex,Offset,Tone,rToneFreq,cToneFreq,DtcsCode,DtcsPolarity,Mode,TStep,Skip,Comment,URCALL,RPT1CALL,RPT2CALL
1,FRS 1,462.562500,,5.000000,,88.5,88.5,023,NN,NFM,12.50,,,,,
2,FRS 2,462.587500,,5.000000,,88.5,88.5,023,NN,NFM,12.50,,,,,
"""
CHIRP_CSV_MINIMAL = """Location,Frequency
1,146.520
2,446.000
"""
CHIRP_CSV_MODERN = """Location,Name,Frequency,Duplex,Offset,Tone,rToneFreq,cToneFreq,DtcsCode,DtcsPolarity,RxDtcsCode,CrossMode,Mode,TStep,Skip,Power,Comment,URCALL,RPT1CALL,RPT2CALL,DVCODE
...
"""  # noqa
CHIRP_CSV_MODERN_QUOTED_HEADER = """"Location","Name","Frequency","Duplex","Offset","Tone","rToneFreq","cToneFreq","DtcsCode","DtcsPolarity","RxDtcsCode","CrossMode","Mode","TStep","Skip","Power","Comment","URCALL","RPT1CALL","RPT2CALL","DVCODE"
0,Nat Simplex,146.520000,,0.600000,TSQL,88.5,88.5,023,NN,023,Tone->Tone,FM,5.00,,50W,This is the national calling frequency on 2m,,,,
1,"National Simp",446.000000,-,5.000000,DTCS,88.5,88.5,023,NN,023,Tone->Tone,FM,5.00,,5.0W,This is NOT the UHF calling frequency,,,,
"""  # noqa
CHIRP_TSV = """Location	Name	Frequency	Duplex	Offset	Tone	rToneFreq	cToneFreq	DtcsCode	DtcsPolarity	RxDtcsCode	CrossMode	Mode	TStep	Skip	Power	Comment	URCALL	RPT1CALL	RPT2CALL	DVCODE
...
class TestCSV(unittest.TestCase):
    ...
    def test_parse_modern(self, output_encoding="utf-8", data=None):
        ...
        with open(self.testfn, "rb") as f:
            self.assertTrue(generic_csv.CSVRadio.match_model(f.read(), self.testfn))
        csv = generic_csv.CSVRadio(self.testfn)
        mem = csv.get_memory(1)
        self.assertEqual(1, mem.number)
    ...
    def test_parse_modern_quoted_header_bom(self):
        self.test_parse_modern(
            output_encoding="utf-8-sig", data=CHIRP_CSV_MODERN_QUOTED_HEADER
        )
