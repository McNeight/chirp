...
class TestRepeaterbook(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.patches = []
        self.patches.append(
            mock.patch("chirp.platform.Platform.config_file", self.fake_config)
        )
        for p in self.patches:
            p.start()

        self.testfile = os.path.join(
            os.path.dirname(__file__), "rb-united_states-oregon.json"
        )
    ...
    def test_distance(self):
        ...
        self.assertLess(
            sum(rb2.get_features().memory_bounds), sum(rb1.get_features().memory_bounds)
        )
    ...
    def test_get_data_500(self):
        ...
        with mock.patch("requests.get") as mock_get:
            ...
            self.assertEqual([], os.listdir(os.path.join(self.tempdir, "repeaterbook")))
    ...
    def test_get_data_json_fail(self):
        ...
        with mock.patch("requests.get") as mock_get:
            ...
            r = rb.get_data(status, "US", "OR", "")
            self.assertIsNone(r)
            files = os.listdir(os.path.join(self.tempdir, "repeaterbook"))
    ...
    def test_get_data_honors_cache_rules(self):
        ...
        cache_file = os.path.join(
            self.tempdir, "repeaterbook", os.path.basename(self.testfile)
        )
        ...
        with mock.patch("requests.get") as mock_get:
            ...
            with mock.patch.object(repeaterbook, "datetime") as mock_dt:
                ...
                mock_get.assert_called_once_with(
                    mock.ANY, headers=expected_headers, stream=True
                )
