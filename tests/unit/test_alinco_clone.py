    ...
    def _handle_rw(self, data):
        ...
        if cmd == b"R":
            ...
            if len(chunk) != block_size:
                raise Exception("Expected %i, got %i" % (block_size, len(chunk)))
...
class AlincoCloneTest(unittest.TestCase):
    ...
    def _test_alinco_upload(self, rclass, image):
        ...
        self.assertEqual(
            ref_image[0x200 : rclass._memsize],
    ...
    def _test_alinco(self, rclass):
        ...
        image = os.path.join(
            os.path.dirname(__file__), "..", "images", "%s.img" % ident
        )
    ...
    def test_all_alinco_identify(self):
        ...
        alincos = [x for x in directory.DRV_TO_RADIO.values() if x.VENDOR == "Alinco"]
