...
class TestUtilityFunctions(base.BaseTest):
    ...
    def test_parse_freq_bad(self):
        ...
        self.assertRaises(ValueError, chirp_common.parse_freq, "1.0000001")
...
class TestSplitTone(base.BaseTest):
    ...
    def test_split_tone_decode_none(self):
        self._test_split_tone_decode((None, None, None), (None, None, None), tmode="")

    def test_split_tone_decode_tone(self):
        self._test_split_tone_decode(
            ("Tone", 100.0, None), ("", 0, None), tmode="Tone", rtone=100.0
        )

    def test_split_tone_decode_tsql(self):
        self._test_split_tone_decode(
            ("Tone", 100.0, None), ("Tone", 100.0, None), tmode="TSQL", ctone=100.0
        )

    def test_split_tone_decode_dtcs(self):
        self._test_split_tone_decode(
            ("DTCS", 23, None), ("DTCS", 23, None), tmode="DTCS", dtcs=23
        )
    ...
    def split_tone_encode_test_none(self):
        self.assertEqual(self._set_mem(tmode=""), (("", None, None), ("", None, None)))

    def split_tone_encode_test_tone(self):
        self.assertEqual(
            self._set_mem(tmode="Tone", rtone=100.0),
            (("Tone", 100.0, None), ("", None, None)),
        )

    def split_tone_encode_test_tsql(self):
        self.assertEqual(
            self._set_mem(tmode="TSQL", ctone=100.0),
            (("Tone", 100.0, None), ("Tone", 100.0, None)),
        )

    def split_tone_encode_test_dtcs(self):
        self.assertEqual(
            self._set_mem(tmode="DTCS", dtcs=23, dtcs_polarity="RN"),
            (("DTCS", 23, "R"), ("DTCS", 23, "N")),
        )
    ...
    def split_tone_encode_test_cross_tone_tone(self):
        self.assertEqual(
            self._set_mem(
                tmode="Cross", cross_mode="Tone->Tone", rtone=100.0, ctone=123.0
            ),
            (("Tone", 100.0, None), ("Tone", 123.0, None)),
        )

    def split_tone_encode_test_cross_tone_dtcs(self):
        self.assertEqual(
            self._set_mem(
                tmode="Cross", cross_mode="Tone->DTCS", rtone=100.0, rx_dtcs=25
            ),
            (("Tone", 100.0, None), ("DTCS", 25, "N")),
        )

    def split_tone_encode_test_cross_dtcs_tone(self):
        self.assertEqual(
            self._set_mem(tmode="Cross", cross_mode="DTCS->Tone", ctone=100.0, dtcs=25),
            (("DTCS", 25, "N"), ("Tone", 100.0, None)),
        )

    def split_tone_encode_test_cross_none_dtcs(self):
        self.assertEqual(
            self._set_mem(tmode="Cross", cross_mode="->DTCS", rx_dtcs=25),
            (("", None, None), ("DTCS", 25, "N")),
        )

    def split_tone_encode_test_cross_none_tone(self):
        self.assertEqual(
            self._set_mem(tmode="Cross", cross_mode="->Tone", ctone=100.0),
            (("", None, None), ("Tone", 100.0, None)),
        )
...
class TestStepFunctions(base.BaseTest):
        ...
        for step, freqs in list(steps.items()):
            for freq in freqs:
                ...
                if step == 5.0:
                    ...
                    self.assertEqual(step, chirp_common.required_step(freq, allowed))

    def test_required_step_finds_suitable(self):
        # If we support 5.0, we should get it as the step for those
        self.assertEqual(
            5.0, chirp_common.required_step(self._005[0], allowed=[5.0, 2.5])
        )
        # If we support 2.5 and not 5.0, then we should find 2.5 as a suitable
        # alternative
        self.assertEqual(2.5, chirp_common.required_step(self._005[0], allowed=[2.5]))
    ...
    def test_fix_rounded_step_no_change(self):
        self.assertEqual(146520000, chirp_common.fix_rounded_step(146520000))
    ...
    def test_fix_rounded_step_833(self):
        ...
        self.assertFalse(chirp_common.is_8_33(chirp_common.to_MHz(108)))
        self.assertEqual(
            chirp_common.to_MHz(108),
            chirp_common.fix_rounded_step(chirp_common.to_MHz(108)),
        )

        # 108.025 MHz should be left unchanged
        self.assertFalse(chirp_common.is_8_33(chirp_common.to_MHz(108.025)))
        self.assertEqual(
            chirp_common.to_MHz(108.025),
            chirp_common.fix_rounded_step(chirp_common.to_MHz(108.025)),
        )

        # Even though 146.008333 is a valid 8.33kHz step, we should
        # not align it as such since it is not in airband
        self.assertTrue(chirp_common.is_8_33(chirp_common.to_MHz(146.008333)))
        ...
        self.assertEqual(
            int(132050000), chirp_common.fix_rounded_step(chirp_common.to_MHz(132.055))
        )
        self.assertEqual(
        ...
        self.assertEqual(
            int(132075000), chirp_common.fix_rounded_step(chirp_common.to_MHz(132.080))
        )
...
class TestImageMetadata(base.BaseTest):
    ...
    def test_strip_metadata(self):
        ...
        r._metadata["someextra"] = "foo"
        raw_metadata = r._make_metadata()
        raw_data = b"foooooooooooooooooooooo" + TestRadio.MAGIC + raw_metadata
...
class TestOverrideRules(base.BaseTest):
    ...
    def _test_radio_override_immutable_policy(self, rclass):
        self.assertEqual(
            ...
            "check_set_memory_immutable_policy" % (directory.radio_class_id(rclass)),
        )
    ...
    def _test_radio_override_calls_super(self, rclass):
        ...
        self.assertIn(
            ...
            "%s.%s must call super() but does not" % (rclass.__name__, method.__name__),
        )
...
class TestMemory(base.BaseTest):
    def test_pickle_with_extra(self):
        ...
        m.extra.append(
            settings.RadioSetting(
                "test", "test", settings.RadioSettingValueString(1, 32, current="foo")
            )
        )
        n = pickle.loads(pickle.dumps(m))
        self.assertEqual(str(n.extra["test"].value), str(m.extra["test"].value))
    ...
    def test_frozen_dupe_unfrozen(self):
        FrozenMemory = chirp_common.FrozenMemory(chirp_common.Memory()).__class__
    ...
    def test_frozen_modifications(self):
        ...
        orig.extra = [
            settings.RadioSetting(
                "foo", "Foo", settings.RadioSettingValueBoolean(False)
            )
        ]
    ...
    def test_repr_dump(self):
        ...
        self.assertEqual(
            ...
            "tuning_step=5.0,comment='',empty=False,immutable=[]>",
            repr(m),
        )
        ...
        m.extra.append(
            settings.RadioSetting(
                "test1", "Test Setting 1", settings.RadioSettingValueBoolean(False)
            )
        )
        m.extra.append(
            settings.RadioSetting(
                "test2",
                "Test Setting 2",
                settings.RadioSettingValueList(["foo", "bar"], "foo"),
            )
        )
        self.assertEqual(
            "<Memory Call(101): freq=146520000,name='',vfo=0,rtone=107.2,"
    ...
    def test_debug_diff(self):
        ...
        m2.tmode = "TSQL"
        self.assertEqual("freq=146520000/446000000,tmode=''/'TSQL'", m1.debug_diff(m2))
