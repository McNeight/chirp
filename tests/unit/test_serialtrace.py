...
class TestSerialTrace(unittest.TestCase):
    ...
    def test_purge_trace_files(self, mock_remove):
        ...
        self.assertEqual(
            ["test_trace_%i.txt" % (i + 5) for i in range(10)], serialtrace.TRACEFILES
        )
        ...
        mock_remove.assert_has_calls([mock.call(fn) for fn in files], any_order=True)
    ...
    def test_log_write_fail(self, mock_read, mock_write, mock_open, mock_tf):
        mock_tf.return_value.writelines.side_effect = [None, Exception("Write error")]
        trace = serialtrace.SerialTrace()
        trace.open()
    ...
    @mock.patch("serial.Serial.write")
    @mock.patch("serial.Serial.read")
    def test_timeout_no_warning(self, mock_read, mock_write, mock_open, mock_tf):
        mock_tf.return_value.writelines.side_effect = [None]
    ...
    @mock.patch("serial.Serial.write")
    @mock.patch("serial.Serial.read")
    def test_timeout_write_read_warning(
        self, mock_read, mock_write, mock_open, mock_tf
    ):
        mock_tf.return_value.writelines.side_effect = [None]
        trace = serialtrace.SerialTrace(timeout=2, baudrate=9600)
        trace.open()
