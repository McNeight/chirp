...
class FakeRadio(chirp_common.Radio):
    def __init__(self, arg):
        self.POWER_LEVELS = list(
            [
                chirp_common.PowerLevel("lo", watts=5),
...
class DstarTests(base.BaseTest):
    def _test_ensure_has_calls(
        self, mem, ini_urcalls, ini_rptcalls, exp_urcalls, exp_rptcalls
    ):
        radio = FakeDstarRadio(None)
        with mock.patch.multiple(
            radio, get_urcall_list=mock.DEFAULT, get_repeater_call_list=mock.DEFAULT
        ) as mocks:
    ...
    def test_ensure_has_calls_empty(self):
        ...
        self._test_ensure_has_calls(
            mem, ini_urcalls, ini_rptcalls, exp_urcalls, exp_rptcalls
        )
...
class ImportFieldTests(base.BaseTest):
    ...
    def test_import_mode_invalid(self):
        ...
        mem.freq = 1800000
        self.assertRaises(
            import_logic.DestNotCompatible, import_logic._import_mode, radio, None, mem
        )
    ...
    def test_import_mem_with_class(self):
        ...
        mem = chirp_common.Memory()
        mem.freq = 146520000
        mem2 = import_logic.import_mem(
            radio, src_rf, mem, mem_cls=chirp_common.DVMemory
        )
    ...
    def test_import_mem_dstar_logic(self):
        ...
        radio.MODES = ["FM"]
        self.assertRaises(
            import_logic.DestNotCompatible, import_logic.import_mem, radio, src_rf, mem
        )
    ...
    def test_import_bank(self):
        ...
        @mock.patch.object(dst_radio, "get_mapping_models", return_value=[dst_bm])
        @mock.patch.object(src_radio, "get_mapping_models", return_value=[src_bm])
        @mock.patch.object(dst_bm, "get_mappings", return_value=dst_banks)
        @mock.patch.object(src_bm, "get_mappings", return_value=src_banks)
        @mock.patch.object(src_bm, "get_memory_mappings", return_value=[src_banks[0]])
        @mock.patch.object(dst_bm, "get_memory_mappings", return_value=[dst_banks[1]])
